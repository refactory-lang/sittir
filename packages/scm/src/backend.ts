...
/**
 * Backend selection. Loads the grammar-local `sittir-scm` native build
...
/**
 * Structural shape of the grammar-local `sittir-scm` native module. We
...
const PACKAGE_ID = 'sittir/scm';

/** Workspace-local native module path for the grammar-owned binary. */
const NATIVE_MODULE_PATH = fileURLToPath(new URL('../../../rust/crates/sittir-scm/index.js', import.meta.url));
...
function computeBackend(): BackendStatus {
	...
	if (forced === 'wasm') {
		return createJsStatus('forced by SITTIR_BACKEND=wasm, but @sittir/scm does not package a WASM backend yet');
	}
