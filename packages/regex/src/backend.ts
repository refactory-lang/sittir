...
/**
 * Backend selection. Loads the grammar-local `sittir-regex` native build
...
/**
 * Structural shape of the grammar-local `sittir-regex` native module. We
...
const PACKAGE_ID = 'sittir/regex';

/** Workspace-local native module path for the grammar-owned binary. */
const NATIVE_MODULE_PATH = fileURLToPath(new URL('../../../rust/crates/sittir-regex/index.js', import.meta.url));
...
function computeBackend(): BackendStatus {
	...
	if (forced === 'wasm') {
		return createJsStatus('forced by SITTIR_BACKEND=wasm, but @sittir/regex does not package a WASM backend yet');
	}
