...
function tryLoadNative() {
    try {
        ...
        const mod = req(NATIVE_MODULE_PATH);
        return mod;
    }
    catch (err) {
...
function computeBackend() {
    ...
    if (!hashMatch) {
        if (forced === 'native') {
            throw new Error(`SITTIR_BACKEND=native but template-bundle hash mismatch (native=${nativeHash}, ts=${TEMPLATE_BUNDLE_HASH})`);
        }
        return createJsStatus('template-bundle hash mismatch', false);
    }
    if (nativeRenderTransportAbi !== NATIVE_RENDER_TRANSPORT_ABI) {
        if (forced === 'native') {
            throw new Error(`SITTIR_BACKEND=native but native render transport ABI mismatch (native=${String(nativeRenderTransportAbi)}, ts=${NATIVE_RENDER_TRANSPORT_ABI})`);
        }
