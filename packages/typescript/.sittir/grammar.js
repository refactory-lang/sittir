  ...
  wire(
    {
      ...
      expectTestFailures: {
        ...
        "export_statement.defaultFromArm": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "import_clause.identifier": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "import_clause.type": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "variable_declarator.identifier": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "variable_declarator.objectPattern": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "variable_declarator.arrayPattern": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "call_expression.import": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "ambient_declaration.semi": "sub-factory derivation is visiting-context-sensitive: this flattened name resolves unambiguously only under some cyclic candidate reachability contexts and not others, so whichever context the overlay emitter caches first determines whether the wire exists \u2014 the wired overlay never gets this name",
        "switch_case.sequenceExpression": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference and Omits the choice-slot key off the parent's own Loose union config type \u2014 when the child factory's own parameter is itself loose-union-shaped or overloaded, this degrades the position to `never` regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "parenthesized_expression.sequenceExpression": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference \u2014 when that factory's own parameter is loose-union-shaped or overloaded, `Parameters<CF>` degrades to `never` here regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "object_assignment_pattern.objectPattern": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference and Omits the choice-slot key off the parent's own Loose union config type \u2014 when the child factory's own parameter is itself loose-union-shaped or overloaded, this degrades the position to `never` regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "object_assignment_pattern.arrayPattern": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference and Omits the choice-slot key off the parent's own Loose union config type \u2014 when the child factory's own parameter is itself loose-union-shaped or overloaded, this degrades the position to `never` regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "class_heritage.implementsClause": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference \u2014 when that factory's own parameter is loose-union-shaped or overloaded, `Parameters<CF>` degrades to `never` here regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "assignment_expression.sequenceExpression": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference and Omits the choice-slot key off the parent's own Loose union config type \u2014 when the child factory's own parameter is itself loose-union-shaped or overloaded, this degrades the position to `never` regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "augmented_assignment_expression.sequenceExpression": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference and Omits the choice-slot key off the parent's own Loose union config type \u2014 when the child factory's own parameter is itself loose-union-shaped or overloaded, this degrades the position to `never` regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "template_substitution.sequenceExpression": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference \u2014 when that factory's own parameter is loose-union-shaped or overloaded, `Parameters<CF>` degrades to `never` here regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "rest_pattern.objectPattern": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference \u2014 when that factory's own parameter is loose-union-shaped or overloaded, `Parameters<CF>` degrades to `never` here regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "rest_pattern.arrayPattern": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference \u2014 when that factory's own parameter is loose-union-shaped or overloaded, `Parameters<CF>` degrades to `never` here regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "tuple_parameter.objectPattern": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference and Omits the choice-slot key off the parent's own Loose union config type \u2014 when the child factory's own parameter is itself loose-union-shaped or overloaded, this degrades the position to `never` regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter",
        "tuple_parameter.arrayPattern": "the wire's declared parameter type derives Parameters<CF> directly off the child's coerce/strict factory reference and Omits the choice-slot key off the parent's own Loose union config type \u2014 when the child factory's own parameter is itself loose-union-shaped or overloaded, this degrades the position to `never` regardless of the args this test builds; a pre-existing overlay-typing limitation in overlays/polymorphs.ts's wire generation, not the test emitter"
      },
      rules: {
