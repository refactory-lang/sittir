...
describe("typescript is / isTree / isNode composition", () => {
	...
	it("isNode returns true for NodeData shapes", () => {
		expect(
			isNode({ $type: "identifier", $text: "foo" } as unknown as { readonly $type: string }),
		).toBe(true);
		expect(
			isNode({ $type: "class_declaration", $fields: {} } as unknown as { readonly $type: string }),
		).toBe(true);
		expect(isNode({ $type: "function_declaration" } as unknown as { readonly $type: string })).toBe(
			false,
		);
	});
