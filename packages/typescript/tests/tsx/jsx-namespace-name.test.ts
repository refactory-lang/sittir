...
describe('jsx_namespace_name', () => {
  it('should build with correct kind', () => {
    const builder = ir.jsx_namespace_name(ir.identifier('test'), ir.identifier('test'));
    const node = builder.build();
    expect(node.kind).toBe('jsx_namespace_name');
  ...
  it('should produce a valid CST node', () => {
    ...
    expect(cst.type).toBe('jsx_namespace_name');
