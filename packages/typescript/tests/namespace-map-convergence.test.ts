...
describe('typescript NamespaceMap access-path convergence', () => {
	it('ClassDeclaration three-path convergence', () => {
		expectTrue<
			Equals<ClassDeclaration.Config, ConfigFor<'class_declaration'>>
		>();
		expectTrue<
			Equals<
				ConfigFor<'class_declaration'>,
				NamespaceMap['class_declaration']['Config']
			>
		>();
		expectTrue<
			Equals<
				ClassDeclaration.Config,
				NamespaceMap['class_declaration']['Config']
			>
		>();
	});
	...
	it('Fluent / Loose / Tree / Kind each converge', () => {
		expectTrue<
			Equals<ClassDeclaration.Fluent, FluentFor<'class_declaration'>>
		>();
