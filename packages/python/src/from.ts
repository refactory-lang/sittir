...
export function splatPatternIdentifierFrom(input: RuntimeNodeOf<SplatPatternIdentifier> | SplatPatternIdentifierFromInput) {
  if (isNodeData<'splat_pattern'>(input)) {
    return splat_pattern__identifier_({
      identifier: input.fields?.identifier,
    } as SplatPatternIdentifierConfig);
  }
  const obj = input as SplatPatternIdentifierFromInput;
  return splat_pattern__identifier_({
    identifier: _resolveNamedExpressionLhs(obj.identifier),
  } as SplatPatternIdentifierConfig);
}

export function splatPattern_From(input: RuntimeNodeOf<SplatPattern_> | SplatPattern_FromInput) {
  if (isNodeData<'splat_pattern'>(input)) {
    return splat_pattern____({
    } as SplatPattern_Config);
  }
  const obj = input as SplatPattern_FromInput;
  return splat_pattern____({
  } as SplatPattern_Config);
}
