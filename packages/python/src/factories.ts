...
export function futureImportStatement(
	config: ConfigOf<T.FutureImportStatement>
) {
	const fields = {
		name: config.name
	};
...
export function expressionStatementUFormTuple(
	...
	return {
		...
		replace(
			this: AnyNodeData,
			target: T.ExpressionStatementUFormTupleTree
		): Edit {
			const r = target.range();
			return toEdit(this, r);
		}
		...
		toEdit: (s: number | ByteRange, e?: number) =>
			typeof s === 'number'
				...
				: {
						startPos: s.start.index,
						endPos: s.end.index,
						insertedText: 'continue' as const
					},
...
export function withClause(
	...
	switch (config.$variant) {
		case 'bare':
			return withClauseUFormBare(
				config as Parameters<typeof withClauseUFormBare>[0]
			);
		case 'paren':
			return withClauseUFormParen(
				config as Parameters<typeof withClauseUFormParen>[0]
			);
	}
	throw new Error(
...
export function withClauseUFormBare(
	_config?: Omit<ConfigOf<T.WithClauseUFormBare>, '$variant'>
) {
	return {
...
export function withClauseUFormParen(
	config?: Omit<ConfigOf<T.WithClauseUFormParen>, '$variant'>
) {
...
export function parenthesizedListSplat(
	child: T.ParenthesizedListSplat | T.ListSplat
) {
	const children = [child];
	return {
...
export function casePattern(
	child: T._AsPattern | T.KeywordPattern | T.SimplePattern
) {
	const children = [child];
	return {
...
export function typedDefaultParameter(
	config: ConfigOf<T.TypedDefaultParameter>
) {
	const fields = {
		name: config.name,
		type: config.type,
...
export function lambdaWithinForInClause(
	config: ConfigOf<T.LambdaWithinForInClause>
) {
	const fields = {
		parameters: config.parameters,
		body: config.body
	...
	switch (config.$variant) {
		case 'eq':
			return assignmentUFormEq(
				config as Parameters<typeof assignmentUFormEq>[0]
			);
		case 'type':
			return assignmentUFormType(
				config as Parameters<typeof assignmentUFormType>[0]
			);
		case 'typed':
			return assignmentUFormTyped(
				config as Parameters<typeof assignmentUFormTyped>[0]
			);
	}
	throw new Error(
...
export function assignmentUFormEq(
	config: Omit<ConfigOf<T.AssignmentUFormEq>, '$variant'>
) {
	const fields = {
		left: config.left
	};
...
export function assignmentUFormType(
	config: Omit<ConfigOf<T.AssignmentUFormType>, '$variant'>
) {
	const fields = {
		left: config.left
	};
...
export function assignmentUFormTyped(
	config: Omit<ConfigOf<T.AssignmentUFormTyped>, '$variant'>
) {
	const fields = {
		left: config.left
	};
...
export function typedParameter(config: ConfigOf<T.TypedParameter>) {
	...
	return {
		...
		child(
			value?: T.Identifier | T.ListSplatPattern | T.DictionarySplatPattern
		) {
			if (value === undefined) return children[0];
...
export function dictionaryComprehension(
	config: ConfigOf<T.DictionaryComprehension>
) {
	const fields = {
		body: config.body
	};
...
export function comprehensionClauses(
...
export function conditionalExpression(
	config: ConfigOf<T.ConditionalExpression>
) {
	const fields = {
		body: config.body,
		condition: config.condition,
...
export type FluentKindMap = {
	...
	_simple_statements: FluentNode<
		'_simple_statements',
		T.SimpleStatements.Config
	>;
	...
	future_import_statement: FluentNode<
		'future_import_statement',
		T.FutureImportStatement.Config
	>;
	import_from_statement: FluentNode<
		'import_from_statement',
		T.ImportFromStatement.Config
	>;
	...
	function_definition: FluentNode<
		'function_definition',
		T.FunctionDefinition.Config
	>;
	...
	nonlocal_statement: FluentNode<
		'nonlocal_statement',
		T.NonlocalStatement.Config
	>;
	exec_statement: FluentNode<'exec_statement', T.ExecStatement.Config>;
	type_alias_statement: FluentNode<
		'type_alias_statement',
		T.TypeAliasStatement.Config
	>;
	...
	parenthesized_list_splat: FluentNode<
		'parenthesized_list_splat',
		T.ParenthesizedListSplat.Config
	>;
	argument_list: FluentNode<'argument_list', T.ArgumentList.Config>;
	decorated_definition: FluentNode<
		'decorated_definition',
		T.DecoratedDefinition.Config
	>;
	...
	typed_default_parameter: FluentNode<
		'typed_default_parameter',
		T.TypedDefaultParameter.Config
	>;
	list_splat_pattern: FluentNode<
		'list_splat_pattern',
		T.ListSplatPattern.Config
	>;
	dictionary_splat_pattern: FluentNode<
		'dictionary_splat_pattern',
		T.DictionarySplatPattern.Config
	>;
	...
	_not_in: T.NotIn;
	_is_not: T.IsNot;
	comparison_operator: FluentNode<
		'comparison_operator',
		T.ComparisonOperator.Config
	>;
	...
	augmented_assignment: FluentNode<
		'augmented_assignment',
		T.AugmentedAssignment.Config
	>;
	...
	list_comprehension: FluentNode<
		'list_comprehension',
		T.ListComprehension.Config
	>;
	...
	generator_expression: FluentNode<
		'generator_expression',
		T.GeneratorExpression.Config
	>;
	_comprehension_clauses: FluentNode<
		'_comprehension_clauses',
		T.ComprehensionClauses.Config
	>;
	...
	conditional_expression: FluentNode<
		'conditional_expression',
		T.ConditionalExpression.Config
	>;
	concatenated_string: FluentNode<
		'concatenated_string',
		T.ConcatenatedString.Config
	>;
	...
	_as_pattern_target: FluentNode<
		'_as_pattern_target',
		T.AsPatternTarget.Config
	>;
	_format_expression: FluentNode<
		'_format_expression',
		T.FormatExpression.Config
	>;
	_assignment_eq: T.AssignmentEq;
	_assignment_type: T.AssignmentType;
	_assignment_typed: T.AssignmentTyped;
	_with_clause_paren: FluentNode<
		'_with_clause_paren',
		T._WithClauseParen.Config
	>;
	_match_block_block: T.MatchBlockBlock;
	_simple_pattern_negative: FluentNode<
