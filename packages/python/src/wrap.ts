...
const _overrides = {
  ...
  "if_clause": {"fields":{"if":{"types":[{"type":"if","named":false}],"multiple":false,"required":true,"position":0},"expression":{"types":[{"type":"expression","named":true}],"multiple":false,"required":true,"position":1}}},
  ...
  "await": {"fields":{"await":{"types":[{"type":"await","named":false}],"multiple":false,"required":true,"position":0},"primary_expression":{"types":[{"type":"primary_expression","named":true}],"multiple":false,"required":true,"position":1}}},
} as const;
export { _overrides };
...
export function wrapAssignment(data: _NodeData, tree: TreeHandle): WrappedNode<Assignment> {
  return {
    ...
    get typeField() { const c = data.children?.[0]; return c && typeof c === 'object' && 'fields' in (c as Record<string, unknown>) ? drillIn((c as any).fields?.['type'], tree) : undefined; },
