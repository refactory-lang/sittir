...
class MatchBuilder extends Builder<MatchStatement> {
...
export function match(...subject: Builder<Expression>[]): MatchBuilder {
...
export namespace match {
  export function from(options: MatchStatementOptions): MatchBuilder {
    const _ctor = options.subject;
    const _arr = Array.isArray(_ctor) ? _ctor : [_ctor];
    const b = new MatchBuilder(..._arr);
