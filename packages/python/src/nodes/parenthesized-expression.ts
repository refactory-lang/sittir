...
import type { Expression, ListSplat, ParenthesizedExpression } from '../types.js';


class ParenthesizedExpressionBuilder extends Builder<ParenthesizedExpression> {
  private _children: Builder<Expression | ListSplat | ParenthesizedExpression>[] = [];

  constructor(children: Builder<Expression | ListSplat | ParenthesizedExpression>) {
    super();
...
export function parenthesized_expression(children: Builder<Expression | ListSplat | ParenthesizedExpression>): ParenthesizedExpressionBuilder {
  return new ParenthesizedExpressionBuilder(children);
}

export interface ParenthesizedExpressionOptions {
  children: Builder<Expression | ListSplat | ParenthesizedExpression> | (Builder<Expression | ListSplat | ParenthesizedExpression>)[];
}
