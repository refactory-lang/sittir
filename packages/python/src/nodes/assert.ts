...
class AssertBuilder extends Builder<AssertStatement> {
...
export type { AssertBuilder };

export function assert(...children: Builder<Expression>[]): AssertBuilder {
  return new AssertBuilder(...children);
}
...
export namespace assert {
  export function from(options: AssertStatementOptions): AssertBuilder {
    const _children = options.children;
    const _arr = Array.isArray(_children) ? _children : [_children];
    const b = new AssertBuilder(..._arr);
    return b;
  }
}
