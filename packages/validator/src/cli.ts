...
async function grammarCounts(grammar: Grammar): Promise<string> {
...
async function grammarProbeFactory(grammar: Grammar): Promise<void> {
	const tp = defaultTemplatesPath(grammar);
...
export async function runCountsCli(args: string[]): Promise<void> {
...
export async function runProbeFactoryCli(args: string[]): Promise<void> {
	const grammars = resolveGrammars(args);
	for (const g of grammars) {
		try {
