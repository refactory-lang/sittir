 ...
 *
 * Always uses the in-process JS reader. Engine selection
 * (TS-engine vs native-engine) is the caller's job — `probe-kind`
 * does it via `--engine`, codemods can do it via separate entry
 * points that talk to the napi `SittirEngine` directly. wrap.ts
 * intentionally does NOT consult `getActiveBackend()` here so that
 * `--engine typescript` produces a deterministic JS-only run and
 * `--engine native` is a deliberate, explicit invocation of the
 * napi engine — no implicit / env-driven routing.
 */
