 ...
 *
 * Reads `getActiveBackend()` to decide engine: when the global
 * setting reports native AND the TreeHandle carries `source`, the
 * read dispatches through `@sittir/rust-native`'s
 * `SittirEngine.parseAndRead(source)` (zero web-tree-sitter,
 * `tree_sitter` Rust crate end-to-end). Otherwise the in-process
 * JS reader runs. Either way `wrapNode` (drillAs + lazy getters)
 * is applied on top, so consumers see one shape regardless of
 * which engine produced the bytes.
 *
 * The global is set via `SITTIR_BACKEND` env var (`typescript` |
 * `native`) — see backend.ts for the selection contract. Callers
 * that want to force an engine should set the env var before
 * importing this module so backend selection caches with the
 * intended value (the singleton is computed once per process).
 *
 * Per-instance native engine + initial root-NodeData are cached
 * on a WeakMap keyed by TreeHandle so repeated reads share state.
 */
...
function readViaActiveBackend(tree: TreeHandle, nodeId?: number): AnyNodeData {
  ...
  // Drill-in caveat: native uses sequential `$nodeId` counters,
  // legacy JS path passes tree-sitter pointer ids — the two id
  // spaces don't match, so the napi `readNode(pointerId)` would
  // panic. Serve drill-ins from the JS reader against already-
  // cached state. Follow-up: pointer-id → $nodeId mapping built
  // during the initial native walk OR sittir_core::read_node
  // accepting tree-sitter Node ids directly.
  return readNode(tree, nodeId);
}
