...
class FieldExpressionBuilder extends Builder<FieldExpression> {
  ...
  build(ctx?: RenderContext): FieldExpression {
    return {
      ...
      field: this._field ? this._field.build(ctx) : undefined,
    } as FieldExpression;
  }

  override get nodeKind(): 'field_expression' { return 'field_expression'; }
...
export namespace field_expression {
  export function from(options: Omit<FieldExpressionOptions, 'nodeKind'>): FieldExpressionBuilder {
    ...
    const b = new FieldExpressionBuilder(_ctor);
    if (options.field !== undefined) {
      const _v = options.field;
      if (_v instanceof Builder) {
        b.field(_v);
      } else {
        switch (_v.nodeKind) {
          case 'field_identifier': b.field(new LeafBuilder('field_identifier', (_v as LeafOptions).text!)); break;
          case 'integer_literal': b.field(new LeafBuilder('integer_literal', (_v as LeafOptions).text!)); break;
        }
      }
    }
    return b;
