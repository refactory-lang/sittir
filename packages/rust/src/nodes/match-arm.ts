...
class MatchArmBuilder extends Builder<MatchArm> {
  ...
  override get nodeKind(): 'match_arm' { return 'match_arm'; }
...
export interface MatchArmOptions {
  nodeKind: 'match_arm';
  pattern: Builder<MatchPattern> | Omit<MatchPatternOptions, 'nodeKind'>;
  ...
  children?: Builder<AttributeItem | InnerAttributeItem> | AttributeItemOptions | InnerAttributeItemOptions | (Builder<AttributeItem | InnerAttributeItem> | AttributeItemOptions | InnerAttributeItemOptions)[];
}

export namespace match_arm {
  export function from(options: Omit<MatchArmOptions, 'nodeKind'>): MatchArmBuilder {
    const _ctor = options.pattern;
    const b = new MatchArmBuilder(_ctor instanceof Builder ? _ctor : match_pattern.from(_ctor));
    ...
    if (options.children !== undefined) {
      ...
      b.children(..._arr.map(_v => { if (_v instanceof Builder) return _v; switch (_v.nodeKind) {   case 'attribute_item': return attribute.from(_v);   case 'inner_attribute_item': return inner_attribute.from(_v); } throw new Error('unreachable'); }));
    }
    return b;
  }
}
