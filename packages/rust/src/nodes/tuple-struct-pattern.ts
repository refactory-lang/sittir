...
class TupleStructPatternBuilder extends Builder<TupleStructPattern> {
  private _type: Builder<Identifier | ScopedIdentifier | GenericType>;
  ...
  constructor(type_: Builder<Identifier | ScopedIdentifier | GenericType>) {
    super();
    this._type = type_;
  }
  ...
  override get nodeKind(): 'tuple_struct_pattern' { return 'tuple_struct_pattern'; }
...
export namespace tuple_struct_pattern {
  export function from(options: Omit<TupleStructPatternOptions, 'nodeKind'>): TupleStructPatternBuilder {
    ...
    const b = new TupleStructPatternBuilder(_ctor);
    if (options.children !== undefined) {
      const _v = options.children;
      const _arr = Array.isArray(_v) ? _v : [_v];
      b.children(..._arr.map(_v => { if (_v instanceof Builder) return _v; switch (_v.nodeKind) {   case 'string_literal': return string_literal.from(_v);   case 'raw_string_literal': return raw_string_literal.from(_v);   case 'negative_literal': return negative_literal.from(_v);   case 'scoped_identifier': return scoped_identifier.from(_v);   case 'generic_pattern': return generic_pattern.from(_v);   case 'tuple_pattern': return tuple_pattern.from(_v);   case 'struct_pattern': return struct_pattern.from(_v);   case 'ref_pattern': return ref_pattern.from(_v);   case 'slice_pattern': return slice_pattern.from(_v);   case 'captured_pattern': return captured_pattern.from(_v);   case 'reference_pattern': return reference_pattern.from(_v);   case 'mut_pattern': return mut_pattern.from(_v);   case 'range_pattern': return range_pattern.from(_v);   case 'or_pattern': return or_pattern.from(_v);   case 'const_block': return const_block.from(_v);   case 'macro_invocation': return macro_invocation.from(_v); } throw new Error('unreachable'); }));
    }
    return b;
  }
}
