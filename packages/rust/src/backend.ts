 ...
 * (the grammar-local `sittir-rust` build) and the TypeScript fallback. See
...
/**
 * Structural shape of the grammar-local `sittir-rust` native module. Matches
...
const NATIVE_MODULE_PATH = fileURLToPath(new URL('../../../rust/crates/sittir-rust/index.js', import.meta.url));
...
function computeBackend(): BackendStatus {
	...
	if (forced === 'wasm') {
		return createJsStatus('forced by SITTIR_BACKEND=wasm, but @sittir/rust does not package a WASM backend yet');
	}
	...
	if (!hashMatch) {
		if (forced === 'native') {
			throw new Error(
				`SITTIR_BACKEND=native but template-bundle hash mismatch (native=${nativeHash}, js=${TEMPLATE_BUNDLE_HASH})`
			);
		}
