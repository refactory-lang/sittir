...
function getNativeBackendEngine(options?: EngineOptions): SittirEngineLike | null {
	...
	try {
		// Convert FormatRecord to JSON string for native engine
...
export function createEngine(options?: EngineOptions): SittirEngineLike {
	...
	const initParser = async () => {
		...
		const treeSitterRust = (await import('tree-sitter-rust')).default;
		...
		parserInstance.setLanguage(treeSitterRust);
