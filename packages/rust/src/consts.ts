// Auto-generated by ir-codegen — do not edit

/** All branch (non-leaf) node kind strings. */
export const NODE_KINDS = [
  'abstract_type',
  'arguments',
  'array_expression',
  'array_type',
  'assignment_expression',
  'associated_type',
  'async_block',
  'attribute',
  'attribute_item',
  'await_expression',
  'base_field_initializer',
  'binary_expression',
  'block',
  'block_comment',
  'bounded_type',
  'bracketed_type',
  'break_expression',
  'call_expression',
  'captured_pattern',
  'closure_expression',
  'closure_parameters',
  'compound_assignment_expr',
  'const_block',
  'const_item',
  'const_parameter',
  'continue_expression',
  'declaration_list',
  'dynamic_type',
  'else_clause',
  'enum_item',
  'enum_variant',
  'enum_variant_list',
  'expression_statement',
  'extern_crate_declaration',
  'extern_modifier',
  'field_declaration',
  'field_declaration_list',
  'field_expression',
  'field_initializer',
  'field_initializer_list',
  'field_pattern',
  'for_expression',
  'for_lifetimes',
  'foreign_mod_item',
  'function_item',
  'function_modifiers',
  'function_signature_item',
  'function_type',
  'gen_block',
  'generic_function',
  'generic_pattern',
  'generic_type',
  'generic_type_with_turbofish',
  'higher_ranked_trait_bound',
  'if_expression',
  'impl_item',
  'index_expression',
  'inner_attribute_item',
  'label',
  'let_chain',
  'let_condition',
  'let_declaration',
  'lifetime',
  'lifetime_parameter',
  'line_comment',
  'loop_expression',
  'macro_definition',
  'macro_invocation',
  'macro_rule',
  'match_arm',
  'match_block',
  'match_expression',
  'match_pattern',
  'mod_item',
  'mut_pattern',
  'negative_literal',
  'or_pattern',
  'ordered_field_declaration_list',
  'parameter',
  'parameters',
  'parenthesized_expression',
  'pointer_type',
  'qualified_type',
  'range_expression',
  'range_pattern',
  'raw_string_literal',
  'ref_pattern',
  'reference_expression',
  'reference_pattern',
  'reference_type',
  'removed_trait_bound',
  'return_expression',
  'scoped_identifier',
  'scoped_type_identifier',
  'scoped_use_list',
  'self_parameter',
  'shorthand_field_initializer',
  'slice_pattern',
  'source_file',
  'static_item',
  'string_literal',
  'struct_expression',
  'struct_item',
  'struct_pattern',
  'token_binding_pattern',
  'token_repetition',
  'token_repetition_pattern',
  'token_tree',
  'token_tree_pattern',
  'trait_bounds',
  'trait_item',
  'try_block',
  'try_expression',
  'tuple_expression',
  'tuple_pattern',
  'tuple_struct_pattern',
  'tuple_type',
  'type_arguments',
  'type_binding',
  'type_cast_expression',
  'type_item',
  'type_parameter',
  'type_parameters',
  'unary_expression',
  'union_item',
  'unsafe_block',
  'use_as_clause',
  'use_bounds',
  'use_declaration',
  'use_list',
  'use_wildcard',
  'variadic_parameter',
  'visibility_modifier',
  'where_clause',
  'where_predicate',
  'while_expression',
  'yield_expression',
] as const;

/** All leaf/terminal node kind strings. */
export const LEAF_KINDS = [
  'boolean_literal',
  'char_literal',
  'crate',
  'doc_comment',
  'empty_statement',
  'escape_sequence',
  'field_identifier',
  'float_literal',
  'fragment_specifier',
  'identifier',
  'inner_doc_comment_marker',
  'integer_literal',
  'metavariable',
  'mutable_specifier',
  'never_type',
  'outer_doc_comment_marker',
  'primitive_type',
  'remaining_field_pattern',
  'self',
  'shebang',
  'shorthand_field_identifier',
  'string_content',
  'super',
  'type_identifier',
  'unit_expression',
  'unit_type',
] as const;

/** All node kind strings (branch + leaf). */
export const ALL_KINDS = [...NODE_KINDS, ...LEAF_KINDS] as const;

/** Language keywords (alphabetic anonymous tokens). */
export const KEYWORDS = [
  '_',
  'as',
  'async',
  'await',
  'break',
  'const',
  'continue',
  'default',
  'dyn',
  'else',
  'enum',
  'expr',
  'extern',
  'false',
  'fn',
  'for',
  'gen',
  'ident',
  'if',
  'impl',
  'in',
  'item',
  'let',
  'literal',
  'loop',
  'match',
  'meta',
  'mod',
  'move',
  'pat',
  'pat_param',
  'path',
  'pub',
  'raw',
  'ref',
  'return',
  'static',
  'stmt',
  'struct',
  'trait',
  'true',
  'try',
  'tt',
  'ty',
  'type',
  'union',
  'unsafe',
  'use',
  'vis',
  'where',
  'while',
  'yield',
] as const;

/** Operator/punctuation tokens. */
export const OPERATORS = [
  '!',
  '!=',
  '#',
  '$',
  '%',
  '%=',
  '&',
  '&&',
  '&=',
  '(',
  ')',
  '*',
  '*/',
  '*=',
  '+',
  '+=',
  ',',
  '-',
  '-=',
  '->',
  '.',
  '..',
  '...',
  '..=',
  '/',
  '/*',
  '//',
  '/=',
  ':',
  '::',
  ';',
  '<',
  '<<',
  '<<=',
  '<=',
  '=',
  '==',
  '=>',
  '>',
  '>=',
  '>>',
  '>>=',
  '?',
  '@',
  '[',
  ']',
  '^',
  '^=',
  'expr_2021',
  'macro_rules!',
  '{',
  '|',
  '|=',
  '||',
  '}',
] as const;

export type NodeKind = (typeof NODE_KINDS)[number];
export type LeafKind = (typeof LEAF_KINDS)[number];
export type AnyKind = (typeof ALL_KINDS)[number];
export type Keyword = (typeof KEYWORDS)[number];
export type Operator = (typeof OPERATORS)[number];

/** Per-node-kind field metadata. */
export const FIELD_MAP: Record<NodeKind, ReadonlyArray<{
  name: string;
  required: boolean;
  multiple: boolean;
}>> = {
  'abstract_type': [
    { name: 'trait', required: true, multiple: false },
  ],
  'arguments': [
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'expression', required: false, multiple: false },
    { name: 'attributeItemOrExpression', required: false, multiple: true },
  ],
  'array_expression': [
    { name: 'length', required: false, multiple: false },
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'attributeItemOrExpression1', required: false, multiple: true },
    { name: 'expression', required: false, multiple: false },
    { name: 'attributeItemOrExpression2', required: false, multiple: true },
  ],
  'array_type': [
    { name: 'element', required: true, multiple: false },
    { name: 'length', required: false, multiple: false },
  ],
  'assignment_expression': [
    { name: 'left', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'associated_type': [
    { name: 'bounds', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
  ],
  'async_block': [
  ],
  'attribute': [
    { name: 'arguments', required: false, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'attribute_item': [
  ],
  'await_expression': [
  ],
  'base_field_initializer': [
  ],
  'binary_expression': [
    { name: 'left', required: true, multiple: false },
    { name: 'operator', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'block': [
    { name: 'label', required: false, multiple: false },
    { name: 'children2', required: false, multiple: true },
    { name: 'expression', required: false, multiple: false },
  ],
  'block_comment': [
    { name: 'doc', required: false, multiple: false },
    { name: 'inner', required: false, multiple: false },
    { name: 'outer', required: false, multiple: false },
  ],
  'bounded_type': [
    { name: 'lifetimeOrTypeOrUseBounds1', required: false, multiple: false },
    { name: 'lifetimeOrTypeOrUseBounds2', required: false, multiple: false },
  ],
  'bracketed_type': [
  ],
  'break_expression': [
    { name: 'label', required: false, multiple: false },
    { name: 'expression', required: false, multiple: false },
  ],
  'call_expression': [
    { name: 'arguments', required: true, multiple: false },
    { name: 'function', required: true, multiple: false },
  ],
  'captured_pattern': [
    { name: 'identifier', required: false, multiple: false },
    { name: 'pattern', required: false, multiple: false },
  ],
  'closure_expression': [
    { name: 'body', required: true, multiple: false },
    { name: 'parameters', required: true, multiple: false },
    { name: 'returnType', required: false, multiple: false },
  ],
  'closure_parameters': [
    { name: 'parameterOrPattern1', required: false, multiple: false },
    { name: 'parameterOrPattern2', required: false, multiple: true },
  ],
  'compound_assignment_expr': [
    { name: 'left', required: true, multiple: false },
    { name: 'operator', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'const_block': [
    { name: 'body', required: true, multiple: false },
  ],
  'const_item': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'const_parameter': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'continue_expression': [
  ],
  'declaration_list': [
  ],
  'dynamic_type': [
    { name: 'trait', required: true, multiple: false },
  ],
  'else_clause': [
  ],
  'enum_item': [
    { name: 'body', required: true, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'enum_variant': [
    { name: 'body', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'enum_variant_list': [
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'enumVariant', required: false, multiple: false },
    { name: 'attributeItemOrEnumVariant', required: false, multiple: true },
  ],
  'expression_statement': [
  ],
  'extern_crate_declaration': [
    { name: 'alias', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'crate', required: false, multiple: false },
  ],
  'extern_modifier': [
  ],
  'field_declaration': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'field_declaration_list': [
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'fieldDeclaration', required: false, multiple: false },
    { name: 'attributeItemOrFieldDeclaration', required: false, multiple: true },
  ],
  'field_expression': [
    { name: 'field', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'field_initializer': [
    { name: 'field', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'field_initializer_list': [
    { name: 'children1', required: false, multiple: false },
    { name: 'children2', required: false, multiple: true },
  ],
  'field_pattern': [
    { name: 'name', required: true, multiple: false },
    { name: 'pattern', required: false, multiple: false },
  ],
  'for_expression': [
    { name: 'body', required: true, multiple: false },
    { name: 'pattern', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'for_lifetimes': [
    { name: 'lifetime1', required: false, multiple: false },
    { name: 'lifetime2', required: false, multiple: true },
  ],
  'foreign_mod_item': [
    { name: 'body', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'externModifier', required: false, multiple: false },
  ],
  'function_item': [
    { name: 'body', required: true, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'parameters', required: true, multiple: false },
    { name: 'returnType', required: false, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'functionModifiers', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'function_modifiers': [
  ],
  'function_signature_item': [
    { name: 'name', required: true, multiple: false },
    { name: 'parameters', required: true, multiple: false },
    { name: 'returnType', required: false, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'functionModifiers', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'function_type': [
    { name: 'parameters', required: true, multiple: false },
    { name: 'returnType', required: false, multiple: false },
    { name: 'trait', required: false, multiple: false },
    { name: 'forLifetimes', required: false, multiple: false },
    { name: 'functionModifiers', required: false, multiple: false },
  ],
  'gen_block': [
  ],
  'generic_function': [
    { name: 'function', required: true, multiple: false },
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'generic_pattern': [
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'generic_type': [
    { name: 'type', required: true, multiple: false },
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'generic_type_with_turbofish': [
    { name: 'type', required: true, multiple: false },
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'higher_ranked_trait_bound': [
    { name: 'type', required: true, multiple: false },
    { name: 'typeParameters', required: true, multiple: false },
  ],
  'if_expression': [
    { name: 'alternative', required: false, multiple: false },
    { name: 'condition', required: true, multiple: false },
    { name: 'consequence', required: true, multiple: false },
  ],
  'impl_item': [
    { name: 'body', required: false, multiple: false },
    { name: 'trait', required: false, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
  ],
  'index_expression': [
    { name: 'object', required: false, multiple: false },
    { name: 'index', required: false, multiple: false },
  ],
  'inner_attribute_item': [
  ],
  'label': [
  ],
  'let_chain': [
    { name: 'conditions', required: false, multiple: false },
  ],
  'let_condition': [
    { name: 'pattern', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'let_declaration': [
    { name: 'alternative', required: false, multiple: false },
    { name: 'pattern', required: true, multiple: false },
    { name: 'type', required: false, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'lifetime': [
  ],
  'lifetime_parameter': [
    { name: 'bounds', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'line_comment': [
    { name: 'doc', required: false, multiple: false },
    { name: 'inner', required: false, multiple: false },
    { name: 'outer', required: false, multiple: false },
  ],
  'loop_expression': [
    { name: 'body', required: true, multiple: false },
  ],
  'macro_definition': [
    { name: 'name', required: true, multiple: false },
    { name: 'macroRule1', required: false, multiple: true },
    { name: 'macroRule2', required: false, multiple: false },
  ],
  'macro_invocation': [
    { name: 'macro', required: true, multiple: false },
  ],
  'macro_rule': [
    { name: 'left', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'match_arm': [
    { name: 'pattern', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'match_block': [
    { name: 'matchArm1', required: false, multiple: true },
    { name: 'matchArm2', required: false, multiple: false },
  ],
  'match_expression': [
    { name: 'body', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'match_pattern': [
    { name: 'condition', required: false, multiple: false },
  ],
  'mod_item': [
    { name: 'body', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'mut_pattern': [
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'pattern', required: false, multiple: false },
  ],
  'negative_literal': [
    { name: 'operator', required: false, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'or_pattern': [
    { name: 'left', required: false, multiple: false },
    { name: 'operator', required: false, multiple: false },
    { name: 'right', required: false, multiple: false },
  ],
  'ordered_field_declaration_list': [
    { name: 'type', required: false, multiple: true },
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'attributeItemOrVisibilityModifier', required: false, multiple: true },
  ],
  'parameter': [
    { name: 'pattern', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'parameters': [
    { name: 'attributeItem', required: false, multiple: false },
    { name: 'children2', required: false, multiple: false },
    { name: 'children3', required: false, multiple: true },
  ],
  'parenthesized_expression': [
  ],
  'pointer_type': [
    { name: 'type', required: true, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
  ],
  'qualified_type': [
    { name: 'alias', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'range_expression': [
    { name: 'start', required: false, multiple: false },
    { name: 'operator', required: false, multiple: false },
    { name: 'end', required: false, multiple: false },
  ],
  'range_pattern': [
    { name: 'left', required: false, multiple: false },
    { name: 'right', required: false, multiple: false },
  ],
  'raw_string_literal': [
  ],
  'ref_pattern': [
  ],
  'reference_expression': [
    { name: 'value', required: true, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
  ],
  'reference_pattern': [
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'pattern', required: false, multiple: false },
  ],
  'reference_type': [
    { name: 'type', required: true, multiple: false },
    { name: 'lifetime', required: false, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
  ],
  'removed_trait_bound': [
  ],
  'return_expression': [
    { name: 'value', required: false, multiple: false },
  ],
  'scoped_identifier': [
    { name: 'name', required: true, multiple: false },
    { name: 'path', required: false, multiple: false },
  ],
  'scoped_type_identifier': [
    { name: 'name', required: true, multiple: false },
    { name: 'path', required: false, multiple: false },
  ],
  'scoped_use_list': [
    { name: 'list', required: true, multiple: false },
    { name: 'path', required: false, multiple: false },
  ],
  'self_parameter': [
    { name: 'lifetime', required: false, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'self', required: false, multiple: false },
  ],
  'shorthand_field_initializer': [
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'identifier', required: false, multiple: false },
  ],
  'slice_pattern': [
  ],
  'source_file': [
    { name: 'shebang', required: false, multiple: false },
    { name: 'children2', required: false, multiple: true },
  ],
  'static_item': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'value', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
  ],
  'string_literal': [
  ],
  'struct_expression': [
    { name: 'body', required: true, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'struct_item': [
    { name: 'body', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'struct_pattern': [
    { name: 'type', required: true, multiple: false },
    { name: 'fieldPatternOrRemainingFieldPattern1', required: false, multiple: false },
    { name: 'fieldPatternOrRemainingFieldPattern2', required: false, multiple: true },
  ],
  'token_binding_pattern': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'token_repetition': [
  ],
  'token_repetition_pattern': [
  ],
  'token_tree': [
  ],
  'token_tree_pattern': [
  ],
  'trait_bounds': [
    { name: 'higherRankedTraitBoundOrLifetimeOrType1', required: false, multiple: false },
    { name: 'higherRankedTraitBoundOrLifetimeOrType2', required: false, multiple: true },
  ],
  'trait_item': [
    { name: 'body', required: true, multiple: false },
    { name: 'bounds', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'try_block': [
  ],
  'try_expression': [
    { name: 'value', required: false, multiple: false },
    { name: 'operator', required: false, multiple: false },
  ],
  'tuple_expression': [
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'expression1', required: false, multiple: false },
    { name: 'expression2', required: false, multiple: true },
    { name: 'expression3', required: false, multiple: false },
  ],
  'tuple_pattern': [
    { name: 'closureExpressionOrPattern1', required: false, multiple: false },
    { name: 'closureExpressionOrPattern2', required: false, multiple: true },
  ],
  'tuple_struct_pattern': [
    { name: 'type', required: true, multiple: false },
    { name: 'pattern1', required: false, multiple: false },
    { name: 'pattern2', required: false, multiple: true },
  ],
  'tuple_type': [
  ],
  'type_arguments': [
    { name: 'children1', required: false, multiple: false },
    { name: 'traitBounds', required: false, multiple: false },
    { name: 'children3', required: false, multiple: true },
  ],
  'type_binding': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'typeArguments', required: false, multiple: false },
  ],
  'type_cast_expression': [
    { name: 'type', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'type_item': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'whereClause1', required: false, multiple: false },
    { name: 'whereClause2', required: false, multiple: false },
  ],
  'type_parameter': [
    { name: 'bounds', required: false, multiple: false },
    { name: 'defaultType', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'type_parameters': [
    { name: 'attributeItem', required: false, multiple: true },
    { name: 'children2', required: false, multiple: false },
    { name: 'children3', required: false, multiple: true },
  ],
  'unary_expression': [
    { name: 'operator', required: false, multiple: false },
    { name: 'operand', required: false, multiple: false },
  ],
  'union_item': [
    { name: 'body', required: true, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'unsafe_block': [
  ],
  'use_as_clause': [
    { name: 'alias', required: true, multiple: false },
    { name: 'path', required: true, multiple: false },
  ],
  'use_bounds': [
    { name: 'lifetimeOrTypeIdentifier1', required: false, multiple: false },
    { name: 'lifetimeOrTypeIdentifier2', required: false, multiple: true },
  ],
  'use_declaration': [
    { name: 'argument', required: true, multiple: false },
  ],
  'use_list': [
  ],
  'use_wildcard': [
  ],
  'variadic_parameter': [
    { name: 'pattern', required: false, multiple: false },
  ],
  'visibility_modifier': [
  ],
  'where_clause': [
    { name: 'wherePredicate1', required: false, multiple: false },
    { name: 'wherePredicate2', required: false, multiple: true },
  ],
  'where_predicate': [
    { name: 'bounds', required: true, multiple: false },
    { name: 'left', required: true, multiple: false },
  ],
  'while_expression': [
    { name: 'body', required: true, multiple: false },
    { name: 'condition', required: true, multiple: false },
  ],
  'yield_expression': [
  ],
};

/** Valid values for `boolean_literal` nodes. */
export const BOOLEAN_LITERALS = [
  'false',
  'true',
] as const;
export type BooleanLiteralValue = (typeof BOOLEAN_LITERALS)[number];

/** Valid values for `fragment_specifier` nodes. */
export const FRAGMENT_SPECIFIERS = [
  'block',
  'expr',
  'expr_2021',
  'ident',
  'item',
  'lifetime',
  'literal',
  'meta',
  'pat',
  'pat_param',
  'path',
  'stmt',
  'tt',
  'ty',
  'vis',
] as const;
export type FragmentSpecifierValue = (typeof FRAGMENT_SPECIFIERS)[number];

/** Valid values for `primitive_type` nodes. */
export const PRIMITIVE_TYPES = [
  'bool',
  'char',
  'f32',
  'f64',
  'i128',
  'i16',
  'i32',
  'i64',
  'i8',
  'isize',
  'str',
  'u128',
  'u16',
  'u32',
  'u64',
  'u8',
  'usize',
] as const;
export type PrimitiveTypeValue = (typeof PRIMITIVE_TYPES)[number];
