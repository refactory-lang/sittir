// Auto-generated by @sittir/codegen — do not edit

/** All branch (non-leaf) node kind strings. */
export const NODE_KINDS = [
  '_closure_expression_expr',
  '_field_identifier',
  '_field_pattern_shorthand',
  '_function_type_fn_form',
  '_function_type_trait_form',
  '_impl_item_body',
  '_let_chain',
  '_macro_definition_brace',
  '_macro_definition_bracket',
  '_macro_definition_paren',
  '_mod_item_inline',
  '_non_special_token',
  '_range_expression_bare',
  '_reserved_identifier',
  '_type_identifier',
  'abstract_type',
  'arguments',
  'array_expression',
  'array_expression_list',
  'array_expression_semi',
  'array_type',
  'assignment_expression',
  'associated_type',
  'async_block',
  'attribute',
  'attribute_item',
  'await_expression',
  'base_field_initializer',
  'binary_expression',
  'block',
  'block_comment',
  'bounded_type',
  'bracketed_type',
  'break_expression',
  'call_expression',
  'captured_pattern',
  'closure_expression',
  'closure_expression_block',
  'closure_expression_expr',
  'closure_parameters',
  'comment',
  'compound_assignment_expr',
  'const_block',
  'const_item',
  'const_parameter',
  'continue_expression',
  'declaration_list',
  'delim_token_tree',
  'dynamic_type',
  'else_clause',
  'enum_item',
  'enum_variant',
  'enum_variant_list',
  'expression_statement',
  'extern_crate_declaration',
  'extern_modifier',
  'field_declaration',
  'field_declaration_list',
  'field_expression',
  'field_initializer',
  'field_initializer_list',
  'field_pattern',
  'field_pattern_named',
  'field_pattern_shorthand',
  'for_expression',
  'for_lifetimes',
  'foreign_mod_item',
  'function_item',
  'function_modifiers',
  'function_signature_item',
  'function_type',
  'function_type_fn_form',
  'function_type_trait_form',
  'gen_block',
  'generic_function',
  'generic_pattern',
  'generic_type',
  'generic_type_with_turbofish',
  'higher_ranked_trait_bound',
  'if_expression',
  'impl_item',
  'impl_item_body',
  'index_expression',
  'inner_attribute_item',
  'label',
  'last_match_arm',
  'let_chain',
  'let_condition',
  'let_declaration',
  'lifetime',
  'lifetime_parameter',
  'line_comment',
  'loop_expression',
  'macro_definition',
  'macro_definition_brace',
  'macro_definition_bracket',
  'macro_definition_paren',
  'macro_invocation',
  'macro_rule',
  'match_arm',
  'match_block',
  'match_expression',
  'match_pattern',
  'mod_item',
  'mod_item_inline',
  'mut_pattern',
  'negative_literal',
  'or_pattern',
  'or_pattern_binary',
  'or_pattern_prefix',
  'ordered_field_declaration_list',
  'parameter',
  'parameters',
  'parenthesized_expression',
  'pointer_type',
  'qualified_type',
  'range_expression',
  'range_expression_bare',
  'range_expression_binary',
  'range_expression_postfix',
  'range_expression_prefix',
  'range_pattern',
  'range_pattern_left',
  'range_pattern_prefix',
  'raw_string_literal',
  'ref_pattern',
  'reference_expression',
  'reference_pattern',
  'reference_type',
  'removed_trait_bound',
  'return_expression',
  'scoped_identifier',
  'scoped_type_identifier',
  'scoped_type_identifier_in_expression_position',
  'scoped_use_list',
  'self_parameter',
  'shorthand_field_initializer',
  'slice_pattern',
  'source_file',
  'static_item',
  'string_literal',
  'struct_expression',
  'struct_item',
  'struct_item_brace',
  'struct_item_tuple',
  'struct_pattern',
  'token_binding_pattern',
  'token_repetition',
  'token_repetition_pattern',
  'token_tree',
  'token_tree_pattern',
  'trait_bounds',
  'trait_item',
  'try_block',
  'try_expression',
  'tuple_expression',
  'tuple_pattern',
  'tuple_struct_pattern',
  'tuple_type',
  'type_arguments',
  'type_binding',
  'type_cast_expression',
  'type_item',
  'type_parameter',
  'type_parameters',
  'unary_expression',
  'union_item',
  'unsafe_block',
  'use_as_clause',
  'use_bounds',
  'use_declaration',
  'use_list',
  'use_wildcard',
  'variadic_parameter',
  'visibility_modifier',
  'where_clause',
  'where_predicate',
  'while_expression',
  'yield_expression',
] as const;

/** All leaf/terminal node kind strings. */
export const LEAF_KINDS = [
  '_',
  '_array_expression_list',
  '_array_expression_semi',
  '_error_sentinel',
  '_inner_block_doc_comment_marker',
  '_kw_async',
  '_kw_attribute',
  '_kw_attributes',
  '_kw_block',
  '_kw_const',
  '_kw_crate',
  '_kw_default',
  '_kw_elements',
  '_kw_expression',
  '_kw_extern_modifier',
  '_kw_for',
  '_kw_for_lifetimes',
  '_kw_function_modifiers',
  '_kw_identifier',
  '_kw_index',
  '_kw_label',
  '_kw_left',
  '_kw_lifetime',
  '_kw_move',
  '_kw_mutable_specifier',
  '_kw_object',
  '_kw_operand',
  '_kw_path',
  '_kw_pattern',
  '_kw_raw_string_literal_end',
  '_kw_raw_string_literal_start',
  '_kw_right',
  '_kw_self',
  '_kw_shebang',
  '_kw_statements',
  '_kw_static',
  '_kw_string_content',
  '_kw_string_literal',
  '_kw_token_tree',
  '_kw_trailing_where_clause',
  '_kw_type_parameters',
  '_kw_unsafe',
  '_kw_value',
  '_kw_visibility_modifier',
  '_kw_where_clause',
  '_line_doc_content',
  '_outer_block_doc_comment_marker',
  '_wildcard_pattern',
  'as',
  'async',
  'await',
  'boolean_literal',
  'break',
  'char_literal',
  'const',
  'continue',
  'crate',
  'default',
  'dyn',
  'else',
  'enum',
  'escape_sequence',
  'extern',
  'field_identifier',
  'float_literal',
  'fn',
  'for',
  'fragment_specifier',
  'gen',
  'identifier',
  'if',
  'impl',
  'in',
  'inner_doc_comment_marker',
  'integer_literal',
  'let',
  'loop',
  'match',
  'metavariable',
  'mod',
  'move',
  'mut',
  'mutable_specifier',
  'outer_doc_comment_marker',
  'primitive_type',
  'pub',
  'raw',
  'raw_string_literal_content',
  'ref',
  'return',
  'self',
  'shebang',
  'shorthand_field_identifier',
  'static',
  'string_content',
  'struct',
  'super',
  'trait',
  'try',
  'type',
  'type_identifier',
  'union',
  'unit_expression',
  'unit_type',
  'unsafe',
  'use',
  'where',
  'while',
  'wildcard_pattern',
  'yield',
] as const;

/** All node kind strings (branch + leaf). */
export const ALL_KINDS = [...NODE_KINDS, ...LEAF_KINDS] as const;

/** Language keywords (alphabetic anonymous tokens). */
export const KEYWORDS = [
  '_',
  '_impl_item_semi',
  '_inner_line_doc_comment_marker',
  '_kw_async',
  '_kw_const',
  '_kw_default',
  '_kw_for',
  '_kw_move',
  '_kw_operator',
  '_kw_unsafe',
  '_mod_item_external',
  '_outer_line_doc_comment_marker',
  '_struct_item_unit',
  '_wildcard_pattern',
  'as',
  'async',
  'await',
  'break',
  'const',
  'continue',
  'crate',
  'default',
  'dyn',
  'else',
  'empty_statement',
  'enum',
  'extern',
  'fn',
  'for',
  'gen',
  'if',
  'impl',
  'impl_item_semi',
  'in',
  'let',
  'loop',
  'match',
  'mod',
  'mod_item_external',
  'move',
  'mut',
  'mutable_specifier',
  'never_type',
  'pub',
  'raw',
  'ref',
  'remaining_field_pattern',
  'return',
  'self',
  'static',
  'struct',
  'struct_item_unit',
  'super',
  'trait',
  'try',
  'type',
  'union',
  'unsafe',
  'use',
  'where',
  'while',
  'wildcard_pattern',
  'yield',
] as const;

/** Operator/punctuation tokens. */
export const OPERATORS = [
  "!",
  "\"",
  "#",
  "$",
  "&",
  "&&",
  "'",
  "(",
  ")",
  "*",
  "*/",
  "+",
  ",",
  "-",
  "->",
  ".",
  "..",
  "...",
  "/",
  "/*",
  "//",
  ":",
  "::",
  ";",
  "<",
  "=",
  "=>",
  ">",
  "?",
  "@",
  "[",
  "]",
  "^",
  "{",
  "|",
  "||",
  "}",
] as const;

export type NodeKind = (typeof NODE_KINDS)[number];
export type LeafKind = (typeof LEAF_KINDS)[number];
export type AnyKind = (typeof ALL_KINDS)[number];
export type Keyword = (typeof KEYWORDS)[number];
export type Operator = (typeof OPERATORS)[number];

/** Per-node-kind field metadata. */
export const FIELD_MAP: Record<NodeKind, ReadonlyArray<{
  name: string;
  required: boolean;
  multiple: boolean;
}>> = {
  '_closure_expression_expr': [
    { name: 'body', required: true, multiple: false },
  ],
  '_field_identifier': [
  ],
  '_field_pattern_shorthand': [
    { name: 'name', required: true, multiple: false },
  ],
  '_function_type_fn_form': [
  ],
  '_function_type_trait_form': [
    { name: 'trait', required: true, multiple: false },
  ],
  '_impl_item_body': [
    { name: 'body', required: true, multiple: false },
  ],
  '_let_chain': [
  ],
  '_macro_definition_brace': [
  ],
  '_macro_definition_bracket': [
  ],
  '_macro_definition_paren': [
  ],
  '_mod_item_inline': [
    { name: 'body', required: true, multiple: false },
  ],
  '_non_special_token': [
  ],
  '_range_expression_bare': [
    { name: 'operator', required: true, multiple: false },
  ],
  '_reserved_identifier': [
  ],
  '_type_identifier': [
  ],
  'abstract_type': [
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'trait', required: true, multiple: false },
  ],
  'arguments': [
  ],
  'array_expression': [
  ],
  'array_expression_list': [
    { name: 'attributes', required: true, multiple: true },
    { name: 'elements', required: true, multiple: true },
  ],
  'array_expression_semi': [
    { name: 'attributes', required: true, multiple: true },
    { name: 'elements', required: true, multiple: false },
    { name: 'length', required: true, multiple: false },
  ],
  'array_type': [
    { name: 'element', required: true, multiple: false },
    { name: 'length', required: false, multiple: false },
  ],
  'assignment_expression': [
    { name: 'left', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'associated_type': [
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'bounds', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'async_block': [
    { name: 'move', required: false, multiple: false },
    { name: 'block', required: true, multiple: false },
  ],
  'attribute': [
    { name: 'value', required: false, multiple: false },
    { name: 'arguments', required: false, multiple: false },
  ],
  'attribute_item': [
    { name: 'attribute', required: true, multiple: false },
  ],
  'await_expression': [
  ],
  'base_field_initializer': [
  ],
  'binary_expression': [
    { name: 'left', required: true, multiple: false },
    { name: 'operator', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'block': [
    { name: 'label', required: false, multiple: false },
  ],
  'block_comment': [
    { name: 'outer', required: false, multiple: false },
    { name: 'inner', required: false, multiple: false },
    { name: 'doc', required: false, multiple: false },
  ],
  'bounded_type': [
    { name: 'left', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'bracketed_type': [
  ],
  'break_expression': [
    { name: 'label', required: false, multiple: false },
    { name: 'expression', required: false, multiple: false },
  ],
  'call_expression': [
    { name: 'function', required: true, multiple: false },
    { name: 'arguments', required: true, multiple: false },
  ],
  'captured_pattern': [
    { name: 'identifier', required: true, multiple: false },
    { name: 'pattern', required: true, multiple: false },
  ],
  'closure_expression': [
    { name: 'static', required: false, multiple: false },
    { name: 'async', required: false, multiple: false },
    { name: 'move', required: false, multiple: false },
    { name: 'parameters', required: true, multiple: false },
  ],
  'closure_expression_block': [
    { name: 'returnType', required: false, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'closure_expression_expr': [
    { name: 'body', required: true, multiple: false },
  ],
  'closure_parameters': [
  ],
  'comment': [
  ],
  'compound_assignment_expr': [
    { name: 'left', required: true, multiple: false },
    { name: 'operator', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'const_block': [
    { name: 'const', required: true, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'const_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'const_parameter': [
    { name: 'const', required: true, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'continue_expression': [
    { name: 'label', required: false, multiple: false },
  ],
  'declaration_list': [
  ],
  'delim_token_tree': [
  ],
  'dynamic_type': [
    { name: 'dyn', required: true, multiple: false },
    { name: 'trait', required: true, multiple: false },
  ],
  'else_clause': [
    { name: 'else', required: true, multiple: false },
  ],
  'enum_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'enum_variant': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'body', required: false, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'enum_variant_list': [
  ],
  'expression_statement': [
  ],
  'extern_crate_declaration': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'crate', required: true, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'alias', required: false, multiple: false },
  ],
  'extern_modifier': [
    { name: 'stringLiteral', required: false, multiple: false },
  ],
  'field_declaration': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'field_declaration_list': [
  ],
  'field_expression': [
    { name: 'value', required: true, multiple: false },
    { name: 'field', required: true, multiple: false },
  ],
  'field_initializer': [
    { name: 'field', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'field_initializer_list': [
  ],
  'field_pattern': [
    { name: 'mutableSpecifier', required: false, multiple: false },
  ],
  'field_pattern_named': [
    { name: 'name', required: true, multiple: false },
    { name: 'pattern', required: true, multiple: false },
  ],
  'field_pattern_shorthand': [
    { name: 'name', required: true, multiple: false },
  ],
  'for_expression': [
    { name: 'label', required: false, multiple: false },
    { name: 'pattern', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'for_lifetimes': [
    { name: 'for', required: true, multiple: false },
  ],
  'foreign_mod_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'externModifier', required: true, multiple: false },
    { name: 'body', required: false, multiple: false },
  ],
  'function_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'functionModifiers', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'parameters', required: true, multiple: false },
    { name: 'returnType', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'function_modifiers': [
    { name: 'async', required: false, multiple: true },
    { name: 'default', required: false, multiple: true },
    { name: 'const', required: false, multiple: true },
    { name: 'unsafe', required: false, multiple: true },
  ],
  'function_signature_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'functionModifiers', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'parameters', required: true, multiple: false },
    { name: 'returnType', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'function_type': [
    { name: 'forLifetimes', required: false, multiple: false },
    { name: 'parameters', required: true, multiple: false },
    { name: 'returnType', required: false, multiple: false },
  ],
  'function_type_fn_form': [
  ],
  'function_type_trait_form': [
    { name: 'trait', required: true, multiple: false },
  ],
  'gen_block': [
    { name: 'move', required: false, multiple: false },
    { name: 'block', required: true, multiple: false },
  ],
  'generic_function': [
    { name: 'function', required: true, multiple: false },
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'generic_pattern': [
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'generic_type': [
    { name: 'type', required: true, multiple: false },
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'generic_type_with_turbofish': [
    { name: 'type', required: true, multiple: false },
    { name: 'turbofish', required: true, multiple: false },
    { name: 'typeArguments', required: true, multiple: false },
  ],
  'higher_ranked_trait_bound': [
    { name: 'for', required: true, multiple: false },
    { name: 'typeParameters', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'if_expression': [
    { name: 'if', required: true, multiple: false },
    { name: 'condition', required: true, multiple: false },
    { name: 'consequence', required: true, multiple: false },
    { name: 'alternative', required: false, multiple: false },
  ],
  'impl_item': [
    { name: 'unsafe', required: false, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'trait', required: false, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
  ],
  'impl_item_body': [
    { name: 'body', required: true, multiple: false },
  ],
  'index_expression': [
    { name: 'object', required: true, multiple: false },
    { name: 'index', required: true, multiple: false },
  ],
  'inner_attribute_item': [
    { name: 'attribute', required: true, multiple: false },
  ],
  'label': [
    { name: 'identifier', required: true, multiple: false },
  ],
  'last_match_arm': [
    { name: 'pattern', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'let_chain': [
  ],
  'let_condition': [
    { name: 'let', required: true, multiple: false },
    { name: 'pattern', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'let_declaration': [
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'pattern', required: true, multiple: false },
    { name: 'type', required: false, multiple: false },
    { name: 'value', required: false, multiple: false },
    { name: 'alternative', required: false, multiple: false },
  ],
  'lifetime': [
    { name: 'identifier', required: true, multiple: false },
  ],
  'lifetime_parameter': [
    { name: 'name', required: true, multiple: false },
    { name: 'bounds', required: false, multiple: false },
  ],
  'line_comment': [
    { name: 'outer', required: false, multiple: false },
    { name: 'inner', required: false, multiple: false },
    { name: 'doc', required: false, multiple: false },
  ],
  'loop_expression': [
    { name: 'label', required: false, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'macro_definition': [
    { name: 'name', required: true, multiple: false },
  ],
  'macro_definition_brace': [
  ],
  'macro_definition_bracket': [
  ],
  'macro_definition_paren': [
  ],
  'macro_invocation': [
    { name: 'macro', required: true, multiple: false },
    { name: 'tokenTree', required: true, multiple: false },
  ],
  'macro_rule': [
    { name: 'left', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'match_arm': [
    { name: 'pattern', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'match_block': [
  ],
  'match_expression': [
    { name: 'match', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'match_pattern': [
    { name: 'condition', required: false, multiple: false },
  ],
  'mod_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'mod_item_inline': [
    { name: 'body', required: true, multiple: false },
  ],
  'mut_pattern': [
    { name: 'mutableSpecifier', required: true, multiple: false },
    { name: 'pattern', required: true, multiple: false },
  ],
  'negative_literal': [
    { name: 'value', required: true, multiple: false },
  ],
  'or_pattern': [
  ],
  'or_pattern_binary': [
    { name: 'left', required: true, multiple: false },
    { name: 'right', required: true, multiple: false },
  ],
  'or_pattern_prefix': [
    { name: 'right', required: true, multiple: false },
  ],
  'ordered_field_declaration_list': [
    { name: 'type', required: true, multiple: true },
  ],
  'parameter': [
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'pattern', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'parameters': [
  ],
  'parenthesized_expression': [
  ],
  'pointer_type': [
    { name: 'mutableSpecifier', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'qualified_type': [
    { name: 'type', required: true, multiple: false },
    { name: 'alias', required: true, multiple: false },
  ],
  'range_expression': [
  ],
  'range_expression_bare': [
    { name: 'operator', required: true, multiple: false },
  ],
  'range_expression_binary': [
    { name: 'start', required: true, multiple: false },
    { name: 'operator', required: true, multiple: false },
    { name: 'end', required: true, multiple: false },
  ],
  'range_expression_postfix': [
    { name: 'start', required: true, multiple: false },
    { name: 'operator', required: true, multiple: false },
  ],
  'range_expression_prefix': [
    { name: 'operator', required: true, multiple: false },
    { name: 'end', required: true, multiple: false },
  ],
  'range_pattern': [
  ],
  'range_pattern_left': [
    { name: 'left', required: true, multiple: false },
    { name: 'right', required: false, multiple: false },
  ],
  'range_pattern_prefix': [
    { name: 'right', required: true, multiple: false },
  ],
  'raw_string_literal': [
    { name: 'rawStringLiteralStart', required: true, multiple: false },
    { name: 'stringContent', required: true, multiple: false },
    { name: 'rawStringLiteralEnd', required: true, multiple: false },
  ],
  'ref_pattern': [
    { name: 'ref', required: true, multiple: false },
  ],
  'reference_expression': [
    { name: 'mutableSpecifier', required: true, multiple: false },
    { name: 'value', required: true, multiple: false },
  ],
  'reference_pattern': [
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'pattern', required: true, multiple: false },
  ],
  'reference_type': [
    { name: 'lifetime', required: false, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'removed_trait_bound': [
  ],
  'return_expression': [
  ],
  'scoped_identifier': [
    { name: 'path', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'scoped_type_identifier': [
    { name: 'path', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'scoped_type_identifier_in_expression_position': [
    { name: 'path', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
  ],
  'scoped_use_list': [
    { name: 'path', required: false, multiple: false },
    { name: 'list', required: true, multiple: false },
  ],
  'self_parameter': [
    { name: 'lifetime', required: true, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'self', required: false, multiple: false },
  ],
  'shorthand_field_initializer': [
    { name: 'attributes', required: true, multiple: true },
    { name: 'identifier', required: true, multiple: false },
  ],
  'slice_pattern': [
  ],
  'source_file': [
    { name: 'shebang', required: false, multiple: false },
    { name: 'statements', required: true, multiple: true },
  ],
  'static_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'value', required: false, multiple: false },
  ],
  'string_literal': [
  ],
  'struct_expression': [
    { name: 'name', required: true, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'struct_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
  ],
  'struct_item_brace': [
    { name: 'body', required: true, multiple: false },
  ],
  'struct_item_tuple': [
    { name: 'body', required: true, multiple: false },
  ],
  'struct_pattern': [
    { name: 'type', required: true, multiple: false },
  ],
  'token_binding_pattern': [
    { name: 'name', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'token_repetition': [
  ],
  'token_repetition_pattern': [
  ],
  'token_tree': [
  ],
  'token_tree_pattern': [
  ],
  'trait_bounds': [
  ],
  'trait_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'unsafe', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'bounds', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'try_block': [
    { name: 'block', required: true, multiple: false },
  ],
  'try_expression': [
    { name: 'value', required: true, multiple: false },
  ],
  'tuple_expression': [
    { name: 'attributes', required: true, multiple: true },
    { name: 'elements', required: false, multiple: true },
  ],
  'tuple_pattern': [
  ],
  'tuple_struct_pattern': [
    { name: 'type', required: true, multiple: false },
  ],
  'tuple_type': [
  ],
  'type_arguments': [
  ],
  'type_binding': [
    { name: 'name', required: true, multiple: false },
    { name: 'typeArguments', required: false, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'type_cast_expression': [
    { name: 'value', required: true, multiple: false },
    { name: 'type', required: true, multiple: false },
  ],
  'type_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
    { name: 'type', required: true, multiple: false },
    { name: 'trailingWhereClause', required: false, multiple: false },
  ],
  'type_parameter': [
    { name: 'name', required: true, multiple: false },
    { name: 'bounds', required: false, multiple: false },
    { name: 'defaultType', required: false, multiple: false },
  ],
  'type_parameters': [
  ],
  'unary_expression': [
    { name: 'operator', required: true, multiple: false },
    { name: 'operand', required: true, multiple: false },
  ],
  'union_item': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'name', required: true, multiple: false },
    { name: 'typeParameters', required: false, multiple: false },
    { name: 'whereClause', required: false, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'unsafe_block': [
    { name: 'block', required: true, multiple: false },
  ],
  'use_as_clause': [
    { name: 'path', required: true, multiple: false },
    { name: 'alias', required: true, multiple: false },
  ],
  'use_bounds': [
    { name: 'use', required: true, multiple: false },
  ],
  'use_declaration': [
    { name: 'visibilityModifier', required: false, multiple: false },
    { name: 'argument', required: true, multiple: false },
  ],
  'use_list': [
  ],
  'use_wildcard': [
    { name: 'path', required: false, multiple: false },
  ],
  'variadic_parameter': [
    { name: 'mutableSpecifier', required: false, multiple: false },
    { name: 'pattern', required: false, multiple: false },
  ],
  'visibility_modifier': [
    { name: 'pub', required: true, multiple: false },
    { name: 'in', required: false, multiple: false },
  ],
  'where_clause': [
    { name: 'where', required: true, multiple: false },
  ],
  'where_predicate': [
    { name: 'left', required: true, multiple: false },
    { name: 'bounds', required: true, multiple: false },
  ],
  'while_expression': [
    { name: 'label', required: false, multiple: false },
    { name: 'condition', required: true, multiple: false },
    { name: 'body', required: true, multiple: false },
  ],
  'yield_expression': [
  ],
};

/** Valid values for `_array_expression_list` nodes. */
export const _ARRAY_EXPRESSION_LISTS = [
] as const;
export type ArrayExpressionListValue = (typeof _ARRAY_EXPRESSION_LISTS)[number];

/** Valid values for `_array_expression_semi` nodes. */
export const _ARRAY_EXPRESSION_SEMIS = [
] as const;
export type ArrayExpressionSemiValue = (typeof _ARRAY_EXPRESSION_SEMIS)[number];

/** Valid values for `_kw_attribute` nodes. */
export const _KW_ATTRIBUTES = [
] as const;
export type KwAttributeValue = (typeof _KW_ATTRIBUTES)[number];

/** Valid values for `_kw_attributes` nodes. */
export const _KW_ATTRIBUTESS = [
] as const;
export type KwAttributesValue = (typeof _KW_ATTRIBUTESS)[number];

/** Valid values for `_kw_block` nodes. */
export const _KW_BLOCKS = [
] as const;
export type KwBlockValue = (typeof _KW_BLOCKS)[number];

/** Valid values for `_kw_crate` nodes. */
export const _KW_CRATES = [
] as const;
export type KwCrateValue = (typeof _KW_CRATES)[number];

/** Valid values for `_kw_elements` nodes. */
export const _KW_ELEMENTSS = [
] as const;
export type KwElementsValue = (typeof _KW_ELEMENTSS)[number];

/** Valid values for `_kw_expression` nodes. */
export const _KW_EXPRESSIONS = [
] as const;
export type KwExpressionValue = (typeof _KW_EXPRESSIONS)[number];

/** Valid values for `_kw_extern_modifier` nodes. */
export const _KW_EXTERN_MODIFIERS = [
] as const;
export type KwExternModifierValue = (typeof _KW_EXTERN_MODIFIERS)[number];

/** Valid values for `_kw_for_lifetimes` nodes. */
export const _KW_FOR_LIFETIMESS = [
] as const;
export type KwForLifetimesValue = (typeof _KW_FOR_LIFETIMESS)[number];

/** Valid values for `_kw_function_modifiers` nodes. */
export const _KW_FUNCTION_MODIFIERSS = [
] as const;
export type KwFunctionModifiersValue = (typeof _KW_FUNCTION_MODIFIERSS)[number];

/** Valid values for `_kw_identifier` nodes. */
export const _KW_IDENTIFIERS = [
] as const;
export type KwIdentifierValue = (typeof _KW_IDENTIFIERS)[number];

/** Valid values for `_kw_index` nodes. */
export const _KW_INDEXS = [
] as const;
export type KwIndexValue = (typeof _KW_INDEXS)[number];

/** Valid values for `_kw_label` nodes. */
export const _KW_LABELS = [
] as const;
export type KwLabelValue = (typeof _KW_LABELS)[number];

/** Valid values for `_kw_left` nodes. */
export const _KW_LEFTS = [
] as const;
export type KwLeftValue = (typeof _KW_LEFTS)[number];

/** Valid values for `_kw_lifetime` nodes. */
export const _KW_LIFETIMES = [
] as const;
export type KwLifetimeValue = (typeof _KW_LIFETIMES)[number];

/** Valid values for `_kw_mutable_specifier` nodes. */
export const _KW_MUTABLE_SPECIFIERS = [
] as const;
export type KwMutableSpecifierValue = (typeof _KW_MUTABLE_SPECIFIERS)[number];

/** Valid values for `_kw_object` nodes. */
export const _KW_OBJECTS = [
] as const;
export type KwObjectValue = (typeof _KW_OBJECTS)[number];

/** Valid values for `_kw_operand` nodes. */
export const _KW_OPERANDS = [
] as const;
export type KwOperandValue = (typeof _KW_OPERANDS)[number];

/** Valid values for `_kw_path` nodes. */
export const _KW_PATHS = [
] as const;
export type KwPathValue = (typeof _KW_PATHS)[number];

/** Valid values for `_kw_pattern` nodes. */
export const _KW_PATTERNS = [
] as const;
export type KwPatternValue = (typeof _KW_PATTERNS)[number];

/** Valid values for `_kw_raw_string_literal_end` nodes. */
export const _KW_RAW_STRING_LITERAL_ENDS = [
] as const;
export type KwRawStringLiteralEndValue = (typeof _KW_RAW_STRING_LITERAL_ENDS)[number];

/** Valid values for `_kw_raw_string_literal_start` nodes. */
export const _KW_RAW_STRING_LITERAL_STARTS = [
] as const;
export type KwRawStringLiteralStartValue = (typeof _KW_RAW_STRING_LITERAL_STARTS)[number];

/** Valid values for `_kw_right` nodes. */
export const _KW_RIGHTS = [
] as const;
export type KwRightValue = (typeof _KW_RIGHTS)[number];

/** Valid values for `_kw_self` nodes. */
export const _KW_SELFS = [
] as const;
export type KwSelfValue = (typeof _KW_SELFS)[number];

/** Valid values for `_kw_shebang` nodes. */
export const _KW_SHEBANGS = [
] as const;
export type KwShebangValue = (typeof _KW_SHEBANGS)[number];

/** Valid values for `_kw_statements` nodes. */
export const _KW_STATEMENTSS = [
] as const;
export type KwStatementsValue = (typeof _KW_STATEMENTSS)[number];

/** Valid values for `_kw_static` nodes. */
export const _KW_STATICS = [
] as const;
export type KwStaticValue = (typeof _KW_STATICS)[number];

/** Valid values for `_kw_string_content` nodes. */
export const _KW_STRING_CONTENTS = [
] as const;
export type KwStringContentValue = (typeof _KW_STRING_CONTENTS)[number];

/** Valid values for `_kw_string_literal` nodes. */
export const _KW_STRING_LITERALS = [
] as const;
export type KwStringLiteralValue = (typeof _KW_STRING_LITERALS)[number];

/** Valid values for `_kw_token_tree` nodes. */
export const _KW_TOKEN_TREES = [
] as const;
export type KwTokenTreeValue = (typeof _KW_TOKEN_TREES)[number];

/** Valid values for `_kw_trailing_where_clause` nodes. */
export const _KW_TRAILING_WHERE_CLAUSES = [
] as const;
export type KwTrailingWhereClauseValue = (typeof _KW_TRAILING_WHERE_CLAUSES)[number];

/** Valid values for `_kw_type_parameters` nodes. */
export const _KW_TYPE_PARAMETERSS = [
] as const;
export type KwTypeParametersValue = (typeof _KW_TYPE_PARAMETERSS)[number];

/** Valid values for `_kw_value` nodes. */
export const _KW_VALUES = [
] as const;
export type KwValueValue = (typeof _KW_VALUES)[number];

/** Valid values for `_kw_visibility_modifier` nodes. */
export const _KW_VISIBILITY_MODIFIERS = [
] as const;
export type KwVisibilityModifierValue = (typeof _KW_VISIBILITY_MODIFIERS)[number];

/** Valid values for `_kw_where_clause` nodes. */
export const _KW_WHERE_CLAUSES = [
] as const;
export type KwWhereClauseValue = (typeof _KW_WHERE_CLAUSES)[number];

/** Valid values for `boolean_literal` nodes. */
export const BOOLEAN_LITERALS = [
  'true',
  'false',
] as const;
export type BooleanLiteralValue = (typeof BOOLEAN_LITERALS)[number];

/** Valid values for `fragment_specifier` nodes. */
export const FRAGMENT_SPECIFIERS = [
  'block',
  'expr',
  'expr_2021',
  'ident',
  'item',
  'lifetime',
  'literal',
  'meta',
  'pat',
  'pat_param',
  'path',
  'stmt',
  'tt',
  'ty',
  'vis',
] as const;
export type FragmentSpecifierValue = (typeof FRAGMENT_SPECIFIERS)[number];

/** Valid values for `primitive_type` nodes. */
export const PRIMITIVE_TYPES = [
  'u8',
  'i8',
  'u16',
  'i16',
  'u32',
  'i32',
  'u64',
  'i64',
  'u128',
  'i128',
  'isize',
  'usize',
  'f32',
  'f64',
  'bool',
  'str',
  'char',
] as const;
export type PrimitiveTypeValue = (typeof PRIMITIVE_TYPES)[number];
