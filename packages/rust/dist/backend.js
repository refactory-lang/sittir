...
function computeBackend() {
    ...
    if (!hashMatch) {
        if (forced === 'native') {
            throw new Error(`SITTIR_BACKEND=native but template-bundle hash mismatch (native=${nativeHash}, js=${TEMPLATE_BUNDLE_HASH})`);
        }
        return createJsStatus('template-bundle hash mismatch', false);
    }
    if (nativeRenderTransportAbi !== NATIVE_RENDER_TRANSPORT_ABI) {
        if (forced === 'native') {
            throw new Error(`SITTIR_BACKEND=native but native render transport ABI mismatch (native=${String(nativeRenderTransportAbi)}, js=${NATIVE_RENDER_TRANSPORT_ABI})`);
        }
