 ...
 * and factory-render-parse already use (`deriveRuleKinds(templatesPath)` —
 * the grammar's `.jinja` template file names), not a separately-derived
...
export async function computeCorpusCoverageCensus(grammar: string, templatesPath: string): Promise<CorpusCoverageCensus> {
	...
	const declaredKinds = deriveRuleKinds(templatesPath);
...
export async function run(opts: CorpusCoverageCensusOptions): Promise<number> {
	...
	const { defaultTemplatesPath } = await import('../run.ts');
	const grammars = opts.allGrammars ? GRAMMARS : [opts.grammar as (typeof GRAMMARS)[number]];
	const results: CorpusCoverageCensus[] = [];
	for (const grammar of grammars) {
		results.push(await computeCorpusCoverageCensus(grammar, defaultTemplatesPath(grammar)));
	}
