...
// ---------------------------------------------------------------------------
// Schema types — mirror packages/common/src/metrics.ts
// ---------------------------------------------------------------------------

/** Subset of MetricsFile we read. Keep in sync with packages/common/src/metrics.ts. */
...
export interface CheckPerfOptions {
	baseline?: string;
	metrics?: string;
}
...
export async function run(opts: CheckPerfOptions): Promise<number> {
	...
	const result = checkPerfBaseline(baselinePath, metricsPath, warnOnly);
