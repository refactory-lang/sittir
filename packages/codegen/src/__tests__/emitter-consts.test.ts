...
describe('emitConsts', () => {
    ...
    it('disambiguates bitflag const names when two kinds share a field name', () => {
        const mkField = (values: string[]) => ({
            ...
            source: 'grammar',
            projection: { typeName: '', kinds: [] },
            values: values.map(v => ({ kind: 'terminal', value: v, multiplicity: 'nonEmptyArray' })),
        })
        const nodeMap = makeNodeMap([
            ['class_declaration', {
                kind: 'class_declaration', typeName: 'ClassDeclaration', factoryName: 'classDeclaration',
                modelType: 'branch',
                fields: [mkField(['public', 'abstract'])],
            } as unknown as AssembledBranch],
            ['method_definition', {
                kind: 'method_definition', typeName: 'MethodDefinition', factoryName: 'methodDefinition',
                modelType: 'branch',
    ...
    it('PascalCases keyword values with non-identifier characters', () => {
        ...
        const nodeMap = makeNodeMap([
            ['visibility_modifier', {
                kind: 'visibility_modifier', typeName: 'VisibilityModifier', factoryName: 'visibilityModifier',
                modelType: 'branch',
                fields: [field],
            } as unknown as AssembledBranch],
        ])
