...
function ruleA(): SeqRule {
	return {
		type: "seq",
		members: [{ type: "field", name: "name", content: { type: "symbol", name: "_ident" } }],
	};
}
...
function ruleB(): SeqRule {
	return {
		type: "seq",
		members: [{ type: "field", name: "value", content: { type: "symbol", name: "_ident" } }],
	};
}
...
describe("T060: single-file diff on one-rule metadata change", () => {
	...
	it("T061a: synthetic new grammar emits .jinja files, not YAML", () => {
		...
		try {
			...
			writeJinjaTemplates(
				emitJinjaTemplates({ grammar: "synthetic", nodeMap: makeNodeMap(nodes) }),
				dir,
			);
