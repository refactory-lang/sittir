...
function diffSummary(
	baseline: string,
	generated: string,
): { same: boolean; baselineLines: number; generatedLines: number; diffLines: number } {
	...
	describe(`Baseline diff — ${grammar}`, () => {
		...
		for (const [key, filename] of Object.entries(FILE_MAP)) {
			it(`${filename} — diff summary`, () => {
				...
				console.log(
					`  ${filename}: baseline=${diff.baselineLines} lines, generated=${diff.generatedLines} lines, diff=${diff.diffLines} lines (${diff.same ? "IDENTICAL" : "DIFFERS"})`,
				);
