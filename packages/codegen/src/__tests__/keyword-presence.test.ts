...
import { AssembledKeyword, AssembledLeaf, AssembledEnum } from '../compiler/node-map.ts'
...
const terminal = (value: string, multiplicity: NodeOrTerminal['multiplicity']): NodeOrTerminal =>
    ({ kind: 'terminal', value, multiplicity })
const ref = (name: string, multiplicity: NodeOrTerminal['multiplicity']): NodeOrTerminal =>
    ({ kind: 'node-ref', node: { kind: 'unresolved-ref', name }, multiplicity })
