...
// ---------------------------------------------------------------------------
// Co-optional lift (task #14) — generalized extractClauseBranch
//
// `optional(seq(...))` is a co-optional block: all members (fields AND
// flanking literals) appear together or are absent together. The walker
// lifts each branch into a single Jinja conditional anchored on the
// first field, with the entire body (literals + every field placeholder)
// inside the `{% if X | isPresent %}...{% endif %}` wrapper.
// ---------------------------------------------------------------------------

describe('renderRuleTemplate — co-optional lift (extractClauseBranch generalization)', () => {
	it('optional(field) emits a single-field conditional', () => {
		// Baseline single-field shape — must remain identical to the
		// pre-generalization behaviour.
		const r = optional(field('name', sym('identifier')));
		const { template } = renderRuleTemplate(r);
		expect(template).toBe('{% if name | isPresent %}$NAME{% endif %}');
	});

	it('optional(seq(literal, field)) wraps the leading literal inside the conditional', () => {
		// e.g. javascript variable_declarator: `seq('=', field('value', ...))`
		// inlined under an optional. Literal moves INSIDE the conditional so
		// the `=` only renders when `value` is populated.
		const r = optional(seq(str('='), field('value', sym('expression'))));
		const { template } = renderRuleTemplate(r);
		expect(template).toBe('{% if value | isPresent %}=$VALUE{% endif %}');
	});

	it('optional(seq(literal, field, literal)) wraps both flanking literals inside the conditional', () => {
		// Generalized shape — one field with literals on both sides.
		// Before generalization the walker handled only leading or one-
		// sided trailing literals via the dedicated `trailing` accumulator;
		// the new body-string approach preserves order naturally.
		const r = optional(seq(str('('), field('value', sym('expression')), str(')')));
		const { template } = renderRuleTemplate(r);
		expect(template).toBe('{% if value | isPresent %}($VALUE){% endif %}');
	});

	it('optional(seq(field, field)) anchors on first field and emits both placeholders', () => {
		// Two co-optional fields — anchor field gates the conditional;
		// both placeholders appear inside the body since the seq makes
		// them co-present at the parse level.
		const r = optional(seq(field('a', sym('expression')), field('b', sym('expression'))));
		const { template } = renderRuleTemplate(r);
		expect(template).toBe('{% if a | isPresent %}$A$B{% endif %}');
	});

	it('optional(seq(literal, field, literal, field, literal)) handles the full general shape', () => {
		// Full interspersed shape: literals flanking multiple fields.
		// Body preserves original order of fields and literals.
		const r = optional(
			seq(
				str('['),
				field('first', sym('expression')),
				str(','),
				field('second', sym('expression')),
				str(']')
			)
		);
		const { template } = renderRuleTemplate(r);
		expect(template).toBe('{% if first | isPresent %}[$FIRST,$SECOND]{% endif %}');
	});

	it('optional(seq) with a non-literal non-field member bails out', () => {
		// Conservative bail-out: an inner symbol/supertype that is not a
		// `field` and not a `literalTextOf`-recognised literal must NOT
		// be lifted. Walker falls through to the default content walk —
		// which in this case surfaces `$$$CHILDREN` from the bare symbol.
		const r = optional(seq(str('='), field('value', sym('expression')), sym('extra')));
		const { template } = renderRuleTemplate(r);
		// No `{% if value | isPresent %}` wrapper — lift bailed and the
		// default walker emits the seq members directly.
		expect(template).not.toContain('isPresent');
