...
describe('nested-alias polymorph — python assignment e2e', () => {
	it.skipIf(!existsSync(wasmPath))(
		'parse tree shows variant child type',
		async () => {
			...
			for (let i = 0; i < assign.namedChildCount; i++) {
				...
				if (
					c.type === 'assignment_eq' ||
					c.type === 'assignment_type' ||
					c.type === 'assignment_typed'
				) {
					variantChild = c;
					break;
				}
	...
	it.skipIf(!existsSync(wasmPath))(
		'type annotation → assignment_type variant',
		async () => {
			const { Parser, lang } = await loadLanguageForGrammar('python');
			const parser = new Parser();
			parser.setLanguage(lang as any);
	...
	it.skipIf(!existsSync(wasmPath))(
		'annotated assignment → assignment_typed variant',
		async () => {
			const { Parser, lang } = await loadLanguageForGrammar('python');
			const parser = new Parser();
			parser.setLanguage(lang as any);
	...
	it.skipIf(!existsSync(wasmPath))(
		'parent kind "assignment" still present for ast-grep compat',
		async () => {
			const { Parser, lang } = await loadLanguageForGrammar('python');
			const parser = new Parser();
			parser.setLanguage(lang as any);
