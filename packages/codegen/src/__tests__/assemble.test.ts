		...
		const rule: Rule = {
			...
			forms: [
				...
				},
				{
					...
					content: {
						type: "seq",
						members: [{ type: "field", name: "body", content: { type: "symbol", name: "block" } }],
					},
				},
			],
		};
	...
	it("classifies hidden seq with fields as group", () => {
		const rule: Rule = {
			...
			content: {
				type: "seq",
				members: [
					{ type: "field", name: "params", content: { type: "symbol", name: "parameters" } },
				],
			},
		};
...
describe("Assemble — assemble()", () => {
	...
	it("assigns typeName and factoryName to nodes", () => {
		const optimized = makeOptimized({
			function_item: {
				type: "seq",
				members: [{ type: "field", name: "name", content: { type: "symbol", name: "id" } }],
			},
