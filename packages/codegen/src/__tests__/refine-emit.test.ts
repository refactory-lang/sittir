...
import { validateRefineForms, resolveRefinePath, narrowedFieldLiteralsForForm } from '../compiler/link-refine.ts'
...
describe('link-refine — resolveRefinePath + narrowedFieldLiteralsForForm', () => {
    ...
    it('extracts narrowed field literals per form', () => {
        const form: RefineForm = { name: 'curly', selections: { 'opening:': '{', 'closing:': '}' } }
    ...
    it('resolves numeric-index selections to the string literal', () => {
        const form: RefineForm = { name: 'flow', selections: { 'opening:': 1, 'closing:': 1 } }
        const raw = makeRefineRaw([form])
        const rule = raw.rules.iface_body!
        const narrowed = narrowedFieldLiteralsForForm(rule, form)
        expect(narrowed).toEqual([
...
describe('factories emitter — per-form factory emission', () => {
    ...
    it('stamps the selected literal into $fields', () => {
        ...
        expect(factoriesSrc).toMatch(/ifaceBodyCurly[\s\S]*opening: "\{" as const[\s\S]*closing: "\}" as const/)
        // Flow factory stamps '{|' / '|}'.
        expect(factoriesSrc).toMatch(/ifaceBodyFlow[\s\S]*opening: "\{\|" as const[\s\S]*closing: "\|\}" as const/)
    })
...
describe('ir emitter — per-form key attachment', () => {
    it('attaches per-form factories under the parent ir key', () => {
        ...
        expect(irSrc).toMatch(/ifaceBody: _attach\(F\.ifaceBody, \{ from: FR\.ifaceBodyFrom, curly: F\.ifaceBodyCurly, flow: F\.ifaceBodyFlow \}\)/)
    })
})
