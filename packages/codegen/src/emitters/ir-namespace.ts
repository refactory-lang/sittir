...
export function emitIrNamespace(config: EmitIrNamespaceConfig): string {
	...
	const operatorContexts = deriveOperatorContexts(allNodes);
...
function deriveOperatorContexts(nodes: NodeModel[]): { field: string; tokens: string[] }[] {
	const results: { field: string; tokens: string[] }[] = [];
	for (const node of nodes) {
		if (node.modelType !== 'branch' && node.modelType !== 'leafWithChildren') continue;
		for (const field of fieldsOf(node)) {
			if (field.types.anonTokens.length > 0) {
				results.push({ field: field.name, tokens: field.types.anonTokens });
			}
		}
	}
	return results;
