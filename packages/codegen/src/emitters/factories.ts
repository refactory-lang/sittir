...
export function emitFactories(config: EmitFactoriesConfig): string {
	...
	lines.push('');
	lines.push("import type { NodeData, Edit, ReplaceTarget } from '@sittir/core';");
	...
	lines.push(' */');
	lines.push('export function edit<K extends string>(target: ReplaceTarget<K>): any {');
	lines.push('  const fields: any = {};');
	lines.push("  const node: any = { type: target.type, fields };");
	// Add ALL fields as fluent setters for every known branch kind
	lines.push('  const fieldSetters: Record<string, string[]> = {');
	for (const node of nodes) {
		const setters = node.fields.map(f => {
			const camel = toFieldName(f.name);
			return camel === 'type' ? 'typeField' : camel;
		});
		if (node.hasChildren) setters.push('children');
		lines.push(`    '${node.kind}': [${setters.map(s => `'${s}'`).join(', ')}],`);
	}
	lines.push('  };');
	lines.push('  const names = fieldSetters[target.type] ?? [];');
	lines.push("  // Map setter names back to grammar field names");
	lines.push('  const fieldNameMap: Record<string, string> = { typeField: "type" };');
	lines.push('  for (const name of names) {');
	lines.push('    const grammarName = fieldNameMap[name] ?? name;');
	lines.push("    node[name] = (...v: any[]) => { fields[grammarName] = v.length === 1 ? resolveInput(v[0]) : v; return node; };");
	lines.push('  }');
	lines.push('  node.render = () => render(node, rules, joinBy);');
	lines.push('  return bindRange(target, node);');
	lines.push('}');
	lines.push('');
