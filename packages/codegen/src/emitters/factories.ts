...
function collectUsesNonEmptyArray(nodeMap: NodeMap): boolean {
	return [...nodeMap.nodes.values()].some((n) => {
		const fs =
			n.modelType === "polymorph" ? n.allFormFields : n.modelType === "branch" ? n.fields : [];
...
function buildLeafReConsts(nodeMap: NodeMap, lines: string[]): Map<string, string> {
	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		const literal =
			flag === "u" ? `/${`^(?:${escapedForLiteral})`}/u` : `/${`^(?:${escapedForLiteral})`}/`;
...
function emitPerNodeFactories(
	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		const isHiddenGroup =
			kind.startsWith("_") && node.modelType !== "token" && node.modelType !== "multi";
 ...
 */
function buildFactoryMapEntries(nodeMap: NodeMap, aliasSourceKinds: Set<string>): MapEntry[] {
...
function renderFactoryForNode(
	...
	if (isTextTemplate) {
		return emitTextFactory(
			node as unknown as { kind: string; treeTypeName: string; rawFactoryName?: string },
			"(text: string)",
			"text",
		);
	}
...
function resolveRefineFormConfigOptional(
	...
	const hasRequired =
		fields.some(
			(f) =>
				isRequired(f) && autoStampExpression(f, nodeMap) === undefined && !narrowed.has(f.name),
...
function emitContainerFactory(node: ContainerNode, nodeMap: NodeMap): string {
	...
	if (anyMultiple) {
		...
		lines.push(
			required ? `  const children = [child];` : `  const children = child != null ? [child] : [];`,
		);
	}
	lines.push("  return {");
	lines.push(`    $type: '${node.kind}' as const,`);
...
function emitHoistedPolymorphFormFactory(
	...
	if (innerIsContainer && hoist.innerFactoryName !== undefined) {
		...
		const innerArg =
			opt === "?" ? `config as Parameters<typeof ${hoist.innerFactoryName}>[0]` : `config`;
 ...
 */
function emitHoistedSetter(f: AssembledField, ctx: HoistedSetterContext): string[] {
...
function resolvePolymorphConfigOptional(forms: AssembledGroup[]): string {
	const anyFormHasRequired = forms.some(
		(f) => f.fields.some((fd) => isRequired(fd)) || f.children.some((c) => isRequired(c)),
	);
