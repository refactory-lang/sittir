...
export function emitTypeTests(config: EmitTypeTestsConfig): string {
	...
	for (const s of structuralKinds) {
		...
		body.push(
			`export type _Type_${s.typeName} = _TypeAssert<_TypeExtends<${s.typeName}['$type'], '${s.kind}'>>;`,
		);
	}
	for (const l of leafKinds) {
		...
		body.push(
			`export type _Type_${l.typeName} = _TypeAssert<_TypeExtends<${l.typeName}['$type'], '${l.kind}'>>;`,
		);
	}
	body.push("");
