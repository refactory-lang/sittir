...
function shapeOf(node: AssembledNode, nodeMap: NodeMap): 'config' | 'children' | 'text' | null {
    // Text-template detection applies to any structural node whose rule
    // is verbatim-token-stream or references hidden externals — the
    // factory takes a `text: string` arg rather than fields/children.
