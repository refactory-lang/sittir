 ...
 */
import { AssembledKeyword, AssembledToken, AssembledEnum, AssembledSupertype, AssembledBranch, AssembledGroup, isNodeRef, isTerminalValue, isUnresolvedRef, isRequired, isMultiple, isNonEmpty, deriveSlotCardinality, deriveChildrenCardinality, allSlotsOf } from '../compiler/node-map.js';
import { hasCatalogEntry } from './kind-discriminant.js';
 ...
 * hidden.
 */
export function collectAliasSourceKinds(nodeMap) {
    const out = new Set();
 ...
 */
export function collectAliasTargetToSourceMap(nodeMap) {
    const out = new Map();
    for (const [kind, node] of nodeMap.nodes) {
        ...
        if (visible.length === 0)
            continue;
        if (nodeMap.nodes.has(visible))
            continue;
        out.set(visible, kind);
    }
    return out;
}
export function referencedKinds(nodeMap) {
        ...
        switch (node.modelType) {
            ...
            case 'group':
                for (const s of Object.values(node.slots))
                    for (const t of slotKindNames(s))
                        referenced.add(t);
                break;
            case 'polymorph':
...
export function isValidIdent(s) {
 ...
 */
export function resolveEffectiveLiteral(field, nodeMap) {
    ...
    if (isNodeRef(v)) {
        ...
        if (kindName.startsWith('_')) {
            ...
            if (ref instanceof AssembledKeyword)
                return ref.text;
            if (ref instanceof AssembledToken)
                return ref.text;
        }
    }
    return undefined;
 ...
 */
export function isAutoStampField(field, nodeMap) {
 ...
 */
export function resolveHiddenKeywordLiteral(kindName, nodeMap) {
    if (!kindName.startsWith('_'))
        return undefined;
 ...
 * factory parameter.
 */
export function isHiddenInfraSlot(slot, nodeMap) {
    const kinds = slotKindNames(slot);
    if (kinds.length === 0)
        return false;
    return kinds.every((kind) => isHiddenInfraKind(kind, nodeMap));
}
function isHiddenInfraKind(kindName, nodeMap) {
    if (!kindName.startsWith('_'))
        return false;
 ...
 */
export function isAutoStampSlot(slot, nodeMap) {
    if (!isRequired(slot))
        return true; // optional → does not block
    if (isMultiple(slot))
        return false; // required repeated → user must supply
    ...
    if (isNodeRef(v)) {
        ...
        if (kindName.startsWith('_') && (ref instanceof AssembledKeyword || ref instanceof AssembledToken))
            return true;
    }
    return false;
}
 ...
 * children slots too.
 */
export function stampExpressionFor(slot, nodeMap, context = 'field') {
    if (!isRequired(slot))
        return undefined; // optional — no stamp
    if (isMultiple(slot))
        return undefined; // repeated — no stamp
...
function resolveAliasedSlotKind(slot, kind, nodeMap) {
 ...
 */
export function fieldTypeComponents(field, nodeMap) {
    const out = [];
    for (const v of field.values) {
        ...
        if (!node) {
            const fallback = t.replace(/(?:^|_)([a-z])/g, (_, c) => c.toUpperCase());
 ...
 */
export function childTypeComponents(child, nodeMap) {
    const out = [];
    for (const rawKind of slotKindNames(child)) {
        ...
        if (!node) {
            const fallback = rawKind.replace(/(?:^|_)([a-z])/g, (_, c) => c.toUpperCase());
 ...
 * shape.
 */
export function resolveHoistedForm(form, nodeMap) {
    if (form.overridePassthrough)
        return undefined;
    // Exactly one child slot.
    ...
    if (form.fields.length > 0) {
        ...
        for (const f of innerFields) {
            if (formNames.has(f.configKey)) {
                console.warn(`[resolveHoistedForm] name collision on form '${form.kind}': inner field '${f.configKey}' shadows form-level field; falling back to non-hoisted Config shape.`);
                return undefined;
            }
        }
    }
...
function resolvePolymorphLiteralKind(kindName, nodeMap, seen = new Set()) {
    ...
    if (direct !== undefined)
        return direct;
    const node = nodeMap.nodes.get(kindName);
    if (!(node instanceof AssembledGroup) || node.fields.length > 0 || node.children.length !== 1)
        return undefined;
    const child = node.children[0];
    const literals = new Set();
    for (const lit of slotLiteralValues(child))
        literals.add(lit);
...
export function collectPolymorphLiteralDispatchCases(forms, nodeMap) {
    const formByLiteral = new Map();
    const ambiguous = new Set();
    for (const form of forms) {
        if (!form.fromFunctionName)
            continue;
        const configurableFields = form.fields.filter((field) => !isAutoStampField(field, nodeMap));
        if (configurableFields.some((field) => isRequired(field)))
            continue;
 ...
 */
function resolveEntryLiteral(entry, nodeMap) {
    if (isTerminalValue(entry))
        return entry.value;
    if (!isNodeRef(entry))
        return undefined;
 ...
 */
export function keywordPresenceKind(field, nodeMap) {
    ...
    if (distinct.size === 1)
        return 'boolean'; // degenerate repeat(single-literal)
    if (distinct.size >= 2)
        return 'bitflag';
    return null;
}
 ...
 */
export function keywordPresenceValue(field, nodeMap) {
    if (keywordPresenceKind(field, nodeMap) !== 'boolean')
        return undefined;
 ...
 */
export function keywordPresenceValues(field, nodeMap) {
    if (keywordPresenceKind(field, nodeMap) !== 'bitflag')
        return [];
    const seen = new Set();
    const out = [];
    for (const v of field.values) {
 ...
 */
export function keywordPresenceIsNonEmptyRepeat(field) {
    if (field.values.length === 0)
        return false;
    return field.values.every((v) => v.multiplicity === 'nonEmptyArray');
}
function classifyFieldStorageInfo(field, nodeMap) {
...
export function computeFieldStorageInfo(nodeMap) {
 ...
 * generator.
 */
export function resolveFieldStorageInfo(field, nodeMap, _kindEntries) {
 ...
 */
export function classifyBranchSlots(node, nodeMap) {
    ...
    for (const f of node.fields) {
        if (stampExpressionFor(f, nodeMap) !== undefined)
            continue;
        if (isHiddenInfraSlot(f, nodeMap))
            continue;
        if (keywordPresenceKind(f, nodeMap) !== null)
            continue;
        userSlots.push(f);
    }
    for (const c of node.children) {
        if (isAutoStampSlot(c, nodeMap))
            continue;
        userSlots.push(c);
    }
 ...
 */
export function computeSlotClasses(nodeMap) {
    for (const [, node] of nodeMap.nodes) {
        if (node.modelType === 'branch' ||
            node.modelType === 'group' ||
            node.modelType === 'polymorph') {
 ...
 */
export function resolveSingleFieldFactorySlot(node, nodeMap) {
    if (node.modelType !== 'branch' && node.modelType !== 'group')
        return undefined;
    if (node.kind.startsWith('_'))
        return undefined;
    ...
    if (slot.source === 'inferred')
        return undefined;
    return slot;
}
function configurableFactoryFields(fields, nodeMap) {
    return fields.filter((field) => stampExpressionFor(field, nodeMap) === undefined &&
        !isHiddenInfraSlot(field, nodeMap) &&
        keywordPresenceKind(field, nodeMap) === null);
}
function hasUserFacingFactoryChildren(children, nodeMap) {
 ...
 */
function hasOptionalUserContentChildren(children, nodeMap) {
    return children.some((child) => {
        if (isRequired(child))
            return false; // only optional slots are missed by isAutoStampSlot
 ...
 */
export function resolveFactoryFieldNames(node, nodeMap) {
    switch (node.modelType) {
        case 'branch':
        case 'group':
        case 'polymorph': {
            ...
            if (fields.length === 0)
                return undefined;
            if (hasUserFacingFactoryChildren(node.children, nodeMap))
                return undefined;
 ...
 */
export function classifyChildFactorySurface(node, nodeMap) {
    if (node.modelType !== 'branch')
        return null;
 ...
 */
export function classifyFactoryShape(node, nodeMap, options) {
    switch (node.modelType) {
        case 'pattern':
        case 'enum':
        case 'keyword':
        ...
        case 'branch': {
            ...
            if (slotClass.tag === 'singleSlot') {
                ...
                if (!node.kind.startsWith('_') &&
                    ...
                    return 'direct';
                if (slotClass.slot.source === 'inferred')
                    return 'spread';
                return 'config';
            }
            const fields = configurableFactoryFields(node.fields, nodeMap);
...
export function classifyParserSymbolEmission(kind, context) {
    ...
    if (inlineKinds?.includes(kind))
        return 'skip-inline-kind';
    if (synthesizedKinds?.has(kind))
        return 'skip-synthesized-kind';
    return 'skip-missing-parser-symbol';
}
export function warnSkippedParserSymbol(kind, emitter, emission) {
    switch (emission) {
        case 'skip-inline-kind':
            console.warn(`[codegen] '${kind}' is in inline: array — no parser symbol expected. ` +
        ...
        case 'skip-missing-parser-symbol':
            console.warn(`[codegen] VAPORIZED: '${kind}' has no parser symbol and is ` +
                `NOT in the grammar's inline: array. Skipping ${emitter} ` +
                `emission. Investigate why tree-sitter dropped this rule.`);
            return;
    }
}
function isHiddenStructuralFactoryKind(kind, node) {
    return kind.startsWith('_') && node.modelType !== 'token' && node.modelType !== 'multi';
}
export function classifyFactoryEmission(kind, node, context) {
    if (!node.userFacing && !isHiddenStructuralFactoryKind(kind, node))
        return 'skip-non-surface-kind';
    if (context.nodeMap.polymorphFormKinds.has(kind))
        return 'skip-polymorph-form';
    if (resolveHiddenKeywordLiteral(kind, context.nodeMap) !== undefined)
        return 'skip-hidden-keyword-literal';
...
export function classifyFromEmission(kind, node, context) {
    if (kind.startsWith('_'))
        return 'skip-hidden-kind';
    if (context.nodeMap.polymorphFormKinds.has(kind))
        return 'skip-polymorph-form';
...
export function classifyTemplateEmission(node) {
    if (!node.userFacing)
        return 'skip-non-user-facing';
    if (node.modelType === 'group' && node.parentKind)
        return 'skip-polymorph-form-group';
    return 'emit';
}
