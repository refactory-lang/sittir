...
export class TemplateEmitter implements CodegenEmitter<EmittedTemplates> {
	...
	#emitNode(node: AssembledNode): void {
		...
		if (body === undefined) {
			...
			this.#jinja.set(node.kind, `${GENERATED_HEADER}\n`);
			return;
		}
...
function emitChoice(rule: Extract<RenderRule, { type: 'CHOICE' }>, ctx: EmitCtx): Body {
	...
	}
	{
		...
		if (process.env.SITTIR_DEBUG_FLATCHOICE) {
			process.stderr.write(
				`[flatchoice] id=${String((rule as { id?: string }).id)} ungateable=${ungateableArm} litFallback=${JSON.stringify(literalFallback === undefined ? undefined : printJinja(literalFallback))} arms=${armInfos.length} bodies=${JSON.stringify(armInfos.map((i) => printJinja(i.body)))}\n`
			);
		}
	...
	if (missing.length > 0) {
		...
		throw new Error(
			`TemplateEmitter slot-preservation violation on kind '${node.kind}' (${node.modelType}): ` +
				`missing slot(s) [${slotDetails.join(', ')}] in body: ${JSON.stringify(printJinja(body))}`
		);
	}
}
...
export function writeJinjaTemplates(emitted: EmittedTemplates, outputDir: string): void {
	...
	for (const [kind, body] of emitted.jinja) {
