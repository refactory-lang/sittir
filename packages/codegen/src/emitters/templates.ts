...
// ---------------------------------------------------------------------------
// Jinja per-rule emission — feature 011 / ADR-0013 follow-up.
//
// Replaces the YAML-serializer path for rendering templates with a
// per-rule `.jinja` file emission. Each rule whose `modelType` has a
// template (branch / container / polymorph / polymorph-form group)
// produces one file keyed by `<kind>.jinja`.
//
// `emitJinjaTemplates` — pure function returning a Map of kind → body.
// `writeJinjaTemplates` — writes the Map to disk, deleting stale
//                          `.jinja` files whose kind is not in the
//                          current emission (prevents drift across
//                          regenerations).
// ---------------------------------------------------------------------------
 ...
 */
export function emitJinjaTemplates(config: EmitTemplatesConfig): Map<string, string> {
    const { nodeMap } = config
    const wordMatcher = compileWordMatcher(nodeMap.word, nodeMap.rules ?? {})
    const result = new Map<string, string>()
    for (const [kind, node] of nodeMap.nodes) {
        // Hidden kinds (`_`-prefixed) follow the same skip-unless-polymorph
        // rule as the YAML emitter. Hidden polymorphs emit because
        // factories stamp their source kind (ADR-0006 drillAs).
        if (kind.startsWith('_') && node.modelType !== 'polymorph') continue
        // Polymorph-form groups already get their own file via the
        // group branch; avoid double emission when the form kind is
        // also registered at the top level.
        if (node instanceof AssembledGroup && node.parentKind) continue
        ...
        result.set(kind, `${GENERATED_HEADER}\n${body}`)
    }
    return result
}
 ...
 */
export function writeJinjaTemplates(emitted: Map<string, string>, outputDir: string): void {
    fs.mkdirSync(outputDir, { recursive: true })
    for (const [kind, body] of emitted) {
        fs.writeFileSync(join(outputDir, `${kind}.jinja`), body, 'utf-8')
    }
    // Stale-file cleanup — only touches `.jinja` files. Anything else
    // (`.gitkeep`, README, etc.) is left alone.
