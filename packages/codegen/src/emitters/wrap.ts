...
export function emitWrap(config: EmitWrapConfig): string {
    ...
    lines.push(' * `source`. Initial root NodeData is cached per-TreeHandle on a')
    lines.push(' * WeakMap so repeated reads share engine state. Drill-ins by')
    lines.push(' * `$nodeId` fall back to the JS reader because the native')
    lines.push(' * engine\'s sequential counter is incompatible with tree-sitter\'s')
    lines.push(' * pointer ids; that\'s a follow-up.')
    lines.push(' */')
    ...
    lines.push(' * `drillAs` for ADR-0006 alias-target → alias-source rewrites at')
