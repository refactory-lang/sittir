...
export class WrapEmitter implements CodegenEmitter<string> {
	...
	finalize(): string {
		...
		lines.push(' * `readTreeNode`, `drillIn` and `drillAs` all read through THIS');
		lines.push(' * function. tree-sitter `Node::id()` is documented as "unique');
		lines.push(' * within a given syntax tree" and is a raw pointer cast —');
		lines.push(' * different parses yield different ids — so the engine that');
		lines.push(' * parsed the tree is the only thing that can dereference its');
		lines.push(' * ids. Native handles set `tree.read` to a closure that routes');
		lines.push(' * through napi.');
		lines.push(' */');
		lines.push('function readNode(tree: TreeHandle, handle?: number, childIndex?: number): AnyNodeData {');
		lines.push('  // Native-engine handles carry a `read` closure that routes');
		lines.push('  // through napi (engine owns the tree; navigation via handle +');
		lines.push('  // childIndex replaces nodeId).');
		lines.push('  return tree.read!(handle, childIndex);');
		lines.push('}');
		lines.push('');
		lines.push('/**');
