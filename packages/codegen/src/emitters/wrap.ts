...
export function emitWrap(config: EmitWrapConfig): string {
	...
	// generatedIdTables is present for numeric $type stamping. Undefined
	...
	const lines: string[] = [
		...
		// (wrap normalizes string $type from core to numeric when kindEntries is present).
	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		if (node.modelType === 'branch' || node.modelType === 'polymorph') {
			...
			// but stamp numeric $type when kindEntries is present (readNode returns
			// string $type from the JS wasm path; wrap normalizes it to numeric).
			if (kindEntries) {
