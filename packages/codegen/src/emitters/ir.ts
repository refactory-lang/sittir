...
export function emitIr(config: EmitIrConfig): string {
	...
	const namespaceOf = (node: AssembledNode) => namespacedConstructors(node, nodeMap, namespaceOptions).entries;
	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		for (const subKind of sup.subtypeNames) {
			...
			if (sub.modelType === 'branch' || sub.modelType === 'separatedList') {
				...
				memberEntries.push(`  ${memberKey}: ${bundleExpr(sub, undefined, namespaceOf(sub))},`);
	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		for (const subKind of sup.subtypeNames) {
			...
			if (sub.modelType === 'branch' || sub.modelType === 'separatedList') {
				if (!sub.fromFunctionName) continue;
				bundle = bundleExpr(sub, refineByKind.get(subKind), namespaceOf(sub));
			} else if (sub.modelType === 'keyword' || sub.modelType === 'pattern' || sub.modelType === 'enum') {
	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		// never appear at runtime; no factory was emitted for them.
		if (kindEntries && !hasCatalogEntry(kindEntries, kind)) continue;
		lines.push(`  ${node.irKey}: ${bundleExpr(node, refineByKind.get(kind), namespaceOf(node))},`);
	}
	lines.push('');
...
function bundleExpr(
	...
	if (node.modelType === 'branch' || node.modelType === 'separatedList') {
		if (!node.rawFactoryName) {
			return `attachProps(FR.${node.fromFunctionName}, { from: FR.${node.fromFunctionName} })`;
		}
		...
		const taken = new Set(['from', 'strict', ...refineEntries.map((e) => e.slice(0, e.indexOf(':')))]);
		const namespaceEntries: string[] = [];
		for (const entry of namespace) {
			const key = JSON.stringify(entry.name);
			if (taken.has(key) || taken.has(entry.name)) {
				console.warn(
			...
			namespaceEntries.push(`${key}: F.${baseFactoryName}.${entry.name}`);
		}
		const extras = [...refineEntries, ...namespaceEntries].map((e) => `, ${e}`).join('');
		return `attachProps(FR.${node.fromFunctionName}, { from: FR.${node.fromFunctionName}, strict: F.${baseFactoryName}${extras} })`;
	}
	return `attachProps(F.${node.rawFactoryName}, { from: FR.${node.fromFunctionName} })`;
}
