...
function emitFromMapDeclaration(lines: string[], nodeMap: NodeMap): void {
	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		if (node.modelType === "token" || node.modelType === "supertype" || node.modelType === "group")
			continue;
...
function emitVariantFrom(
	...
	const parentFields =
		"fields" in node ? (node as { fields: readonly AssembledField[] }).fields : [];
	...
	for (const pv of polymorphVariants) {
		...
		if (!vNode) continue;
		const vFields =
			"fields" in vNode ? (vNode as { fields: readonly AssembledField[] }).fields : [];
