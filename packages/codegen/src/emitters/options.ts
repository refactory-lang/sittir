...
export function renderOptionsModule(inputs: OptionsModuleInputs = {}): string {
	...
	const alias = (type: string): string =>
		type === spacingType ? 'SpacingArm' : type === whitespaceType ? 'WhitespaceArm' : type;
	...
	L.push(`export type SpacingArm = ${spacingType};`, '');
	L.push(`export type WhitespaceArm = ${whitespaceType};`, '');
