...
export function rootKeyOf(kind: string): string {
	return snakeToCamel(publicKindName(kind));
}
...
export interface HintEmitter {
	readonly hintOf: (root: string) => string | undefined;
	readonly labelRoots: readonly string[];
}
...
export function hintEmitterOf(addresses: AddressTables, kindEntries: readonly KindEntryLike[], arms: ArmAliases | undefined, kinds: ReadonlySet<string>): HintEmitter {
	...
	const rootEntry = new Map(addresses.branches.filter((b) => b.segments.length === 1).map((b) => [b.path, b]));
	for (const leaf of addresses.leaves) {
		if (leaf.segments.length === 1) throw new Error(`options: address '${leaf.path}' is a site at the root, which has no hint home`);
	}
	return {
		hintOf: (root) => {
			const entry = rootEntry.get(root);
			return entry === undefined ? undefined : bodyOf(entry.segments);
		},
		labelRoots: addresses.roots.filter((root) => !kinds.has(root))
	};
}
...
export function renderOptionsModule(inputs: OptionsModuleInputs = {}): string {
	const labels = (inputs.hints?.labelRoots ?? []).map((root) => ({ key: snakeToCamel(root), hint: inputs.hints!.hintOf(root) }));
