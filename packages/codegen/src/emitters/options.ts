...
export interface OptionsModuleInputs {
	...
	readonly edgeBeforeType?: string;
	readonly edgeAfterType?: string;
...
export function renderOptionsModule(shape: OptionsShape, inputs: OptionsModuleInputs = {}): string {
	...
	const edgeBeforeType = inputs.edgeBeforeType ?? spacingType;
	const edgeAfterType = inputs.edgeAfterType ?? spacingType;
	const isSpacing = (e: OptionEntry): boolean => e.type === spacingType;
	const edgeKinds = shape.kinds
		.filter((g) =>
			g.entries.some((e) => e.key === `${g.key}_before` && e.type === edgeBeforeType) &&
			g.entries.some((e) => e.key === `${g.key}_after` && e.type === edgeAfterType)
		)
		.map((g) => g.key);
	...
	const otherLabels = shape.topLevel.filter((e) => !isSpacing(e) && !edgeKeys.has(e.key));
	...
	const kindOther = shape.kinds
		.map((g) => ({ key: g.key, entries: g.entries.filter((e) => !isSpacing(e) && !edgeKeys.has(e.key)) }))
	...
	const allTypes = [spacingType, edgeBeforeType, edgeAfterType, ...otherLabels.map((e) => e.type), ...kindOther.flatMap((g) => g.entries.map((e) => e.type))].join(' ');
	...
	L.push(`export type EdgeBefore = ${edgeBeforeType};`, '');
	L.push(`export type EdgeAfter = ${edgeAfterType};`, '');
	...
	L.push(
		...
		'} & { readonly [P in K extends EdgeKind ? `${K}_after` : never]?: EdgeAfter } & Merge<K extends keyof KindOther ? KindOther[K] : never>;',
		'',
...
export function emitOptions(config: EmitOptionsConfig): string {
	...
	const edge = (side: 'before' | 'after'): string | undefined =>
		sites.find((s) => s.side === 'seam' && s.address === `${publicKindName(s.kind)}_${side}`)?.arms.map(armType).join(' | ');
	return renderOptionsModule(shape, { spacingType, edgeBeforeType: edge('before'), edgeAfterType: edge('after'), supertypeMembers });
}
