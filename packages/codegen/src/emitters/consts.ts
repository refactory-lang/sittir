...
export function emitConsts(config: EmitConstsConfig): string {
  ...
  const nodes = structuralNodes(allNodes);
  const nodeKinds = nodes.map(n => n.kind);
  const leafKinds = leafKindsOf(allNodes);
  const keywords = keywordTokensOf(allNodes);
  const operators = operatorTokensOf(allNodes);
  const enumKinds = enumKindsOf(allNodes);
