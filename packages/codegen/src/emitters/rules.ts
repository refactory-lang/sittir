...
import type { KindMeta, FieldMeta, GrammarRule } from '../grammar-reader.ts';
...
export function emitRule(config: EmitRulesConfig): EmittedRule {
	...
	const fieldMap = new Map(node.fields.map(f => [f.name, f]));
	...
	if (!rule) {
		const fieldParts = node.fields.map(f => {
			const q = !f.required ? '?' : f.multiple ? '*' : '';
			return `${f.name}: (_)${q}`;
		});
		const childPart = node.hasChildren ? ' (_)*' : '';
		return {
	...
	if (node.hasChildren && !deduped.some(e => e === '(_)*')) {
		deduped.push('(_)*');
	}
...
export function emitRules(config: {
	...
	lines.push('');
	lines.push("import type { RulesRegistry } from '@sittir/core';");
	lines.push('');
