...
export function emitTypes(config: EmitTypesConfig): string {
	...
	lines.push("import type { NodeData as BaseNodeData, NodeConfig as BaseNodeConfig, TreeNode as BaseTreeNode, ConfigOf, FromInputOf, TreeNodeOf, NodeKind } from '@sittir/types';");
	lines.push('');
	...
	lines.push('// Tree types — derived from concrete interfaces');
	for (const kind of nodeKinds) {
		lines.push(`export type ${toTypeName(kind)}Tree = TreeNodeOf<${toTypeName(kind)}>;`);
	}
	lines.push('');

	lines.push('// Leaf tree types');
	for (const kind of leafKinds) {
		lines.push(`export type ${toTypeName(kind)}Tree = TreeNodeOf<{ readonly type: '${kind}'; readonly text: string }>;`);
	}
	lines.push('');
