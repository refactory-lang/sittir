 ...
 */
function emitDedupedSyntaxKindEnum(lines: string[], allKinds: string[], nodeMap: NodeMap): void {
...
function emitNamespaceSugarBlock(
	...
	if (refineInfo && refineInfo.forms.length > 0) {
		...
		const defaultShortName = refineFormTypeName(node.typeName, defaultForm.name).slice(
			node.typeName.length,
		);
