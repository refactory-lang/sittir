...
export function emitTypes(config: EmitTypesConfig): string {
	...
	lines.push("import type { NodeData as BaseNodeData, NodeConfig as BaseNodeConfig, NodeFromInput as BaseNodeFromInput, TreeNode as BaseTreeNode, ConfigOf, NodeKind } from '@sittir/types';");
	lines.push('');
	...
	lines.push('// Tree types — grammar-projected (matches assign/from emitters)');
	for (const kind of nodeKinds) {
	...
	lines.push('// FromInput types — grammar-projected');
	for (const kind of nodeKinds) {
		lines.push(`export interface ${toTypeName(kind)}FromInput extends NodeFromInput<'${kind}'> {}`);
	}
	lines.push('');
