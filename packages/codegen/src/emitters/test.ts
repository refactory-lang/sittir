	...
	for (const [kind, node] of nodeMap.nodes) {
		...
		switch (node.modelType) {
			...
			case 'envelope':
				...
				emitSubFactoryTests(target, node, kind, key, nodeMap, kindEntries, bundledKinds, config.expectTestFailures);
				break;
			case 'polymorph':
				emitBranchTest(target, node, kind, key, nodeMap, kindEntries);
				emitSubFactoryTests(target, node, kind, key, nodeMap, kindEntries, bundledKinds, config.expectTestFailures);
...
function emitSubFactoryTests(
	...
	const isEmitted = (candidateKind: string): boolean => {
		const candidate = nodeMap.nodes.get(candidateKind);
		return candidate !== undefined && classifyFactoryEmission(candidateKind, candidate, { nodeMap, kindEntries }) === 'emit';
	};
	const { entries } = subFactoriesOf(node, nodeMap, { isEmitted });
