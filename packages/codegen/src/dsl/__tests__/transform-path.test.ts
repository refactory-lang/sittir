...
describe("applyPath()", () => {
	...
	describe("wildcards (_)", () => {
		it("applies to every member of a choice", () => {
			...
			const result = applyPath(rule, [{ kind: "wildcard" }, { kind: "index", value: 0 }], (m) =>
				fld("first", m),
			);
		...
		it("throws on zero-match wildcard", () => {
			...
			expect(() =>
				applyPath(rule, [{ kind: "wildcard" }, { kind: "index", value: 0 }], fld("x", sym("y"))),
	...
	describe("field traversal (fieldName segment)", () => {
		...
		it("throws when the field name at that position does not match", () => {
			...
			expect(() =>
				applyPath(rule, [{ kind: "fieldName", name: "body" }], sym("new_identifier")),
