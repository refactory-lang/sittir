 ...
 */
import { isPrecWrapper as isPrecWrapperShape, isContainerType, isWrapperType, isSeqType, isChoiceType, isFieldType } from '../runtime-shapes.js';
function dsl() {
    return globalThis;
}
...
export function applyPath(rule, segments, patch, precStack) {
...
function descendThroughSingleWrapper(rule, head, rest, patch, precStack) {
    switch (head.kind) {
        ...
        case 'index': {
            ...
            throw new ApplyPathSkip(`applyPath: index ${head.value} out of bounds — '${rule.type}' wraps a single content rule (only index 0 / -1 is valid)`);
        }
        case 'kind-match':
        case 'fieldName': {
...
function descendThroughNamedField(rule, fieldName, rest, patch, precStack) {
    if (!isFieldType(rule.type)) {
        throw new Error(`applyPath: path segment '${fieldName}:' at this level expects a field('${fieldName}', ...) wrapper; got type '${rule.type}'`);
    }
    const actualName = rule.name;
    if (actualName !== fieldName) {
        throw new Error(`applyPath: path segment '${fieldName}:' doesn't match field name '${actualName}' at this position`);
    }
    const newContent = applyPath(contentOf(rule), rest, patch, precStack);
    return reconstructWrapper(rule, newContent);
}
 ...
 */
export function reconstructContainer(rule, members) {
    const t = rule.type;
    if (isSeqType(t))
        return nativeRequired('seq')(...members);
    if (isChoiceType(t))
        return nativeRequired('choice')(...members);
 ...
 */
export function reconstructWrapper(rule, newContent) {
    const t = rule.type;
    if (t === 'optional')
        return nativeRequired('optional')(newContent);
    ...
    throw new Error(`reconstructWrapper: no native dsl reconstruction for wrapper type '${rule.type}' — this is a bug in the path-descent logic.`);
}
/**
...
function reconstructRepeatWithMetadata(rule, newContent) {
    ...
    if (r.separator !== undefined)
        baseNode.separator = r.separator;
    if (r.leading !== undefined)
        baseNode.leading = r.leading;
    if (r.trailing !== undefined)
        baseNode.trailing = r.trailing;
    return baseNode;
}
/**
...
export function reconstructPrec(rule, newContent) {
 ...
 */
export function wrapInPrecStack(content, precStack, reconstructPrec) {
    if (!precStack?.length)
        return content;
    let result = content;
...
//# sourceMappingURL=transform-path.js.map