...
export interface LinkedGrammar {
	...
	/**
	 * Link-time-pinned word-shape matcher — see the glossary's Phase 2: Link
	 * "Word-matcher pin-at-link invariant" for why this is compiled once here
	 * and carried (never recompiled) by every later phase.
	 */
...
/**
 * Normalize-phase view of the grammar (`Grammar<'normalize'>`) — see the
 * glossary's Phase 3: Normalize "`normalizedRules` vs `rules` — the opacity
 * invariant" for what `rules` (wrapper-deleted, this phase's product) vs
 * `linkRules` (carried, wrapper-intact) each mean and when to reach for each.
 */
...
export interface SimplifiedGrammar {
	readonly name: string;
	/**
	 * Carried mid-normalize link-phase view (wrappers intact). Consumers today
	 * are narrowly the by-design authoring-shape diagnostics
	 * (`emitters/suggested.ts`, `emitters/refine-emit.ts`) — see the enumerated
	 * exception list on {@link NodeMap.linkRules}'s doc comment (`compiler/types.ts`
	 * on the assemble side) before adding a new one.
	 */
	readonly linkRules: Record<string, Rule<'link'>>;
	readonly aliasedHiddenKinds?: Map<string, string>;
	readonly topLevelAliasBodies?: Map<string, Rule<'link'>>;
	...
	readonly visibleAliasTargets?: ReadonlyMap<string, readonly string[]>;
	/**
	 * `SimplifiedGrammar`'s phase product — see the glossary's "opacity
	 * invariant" note (Phase 3: Normalize) for how this relates to `linkRules`
	 * / `normalizedRules`. Default to this field; the other two are for
	 * enumerated exceptions only.
	 */
	readonly rules: Record<string, SimplifiedRule>;
	/** Wrapper-deleted view (`applyWrapperDeletion` output) — see the glossary's opacity-invariant note. */
...
export interface NodeMap {
	 ...
	 *
	 * This is the ENUMERATED-EXCEPTION list the glossary's "opacity
	 * invariant" note (Phase 3: Normalize) refers callers to. Every other
	 * consumer that once read this view has migrated to `normalizedRules`
	 * (below) or `rules`; do not add a new consumer here without checking
	 * whether it genuinely needs wrapper shapes. Confirmed-unsafe
	 * alternative: migrating to `rules` (SimplifiedGrammar's own phase
	 * product) loses python's `_simple_pattern_negative` subtype — simplify's
	 * SEQ-collapse unmasks an intentionally opaque SEQ into a dispatchable
	 * CHOICE, discarding the variant-adopted kind's name (see `AssembleCtx`'s
	 * class doc comment for the root-cause). Current consumers, both
	 * BY-DESIGN authoring-shape diagnostics (not render/derivation paths):
	 ...
	 *
	 * `topLevelAliasBodies` stays a distinct field (its presence test — "is
	 * this hidden kind an alias-mint target" — has no rule-attribute
	 * equivalent).
	 */
	readonly linkRules?: Record<string, Rule<'link'>>;
	/**
