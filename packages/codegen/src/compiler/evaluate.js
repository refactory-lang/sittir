...
import { normalizeEnumMembers } from './rule.js';
import { attachReferenceRuleIds, buildRuleCatalog } from './rule-catalog.js';
import { withRoleScope } from '../dsl/primitives/role.js';
// ---------------------------------------------------------------------------
...
export function seq(...members) {
    ...
    if (absorbed)
        return { type: 'seq', members: absorbed };
 ...
 */
function absorbTrailingSeparator(members) {
    let changed = false;
    const out = [];
    for (let i = 0; i < members.length; i++) {
        ...
        const isOptionalSepLit = (r, sep) => !!r && r.type === 'optional' && r.content.type === 'string' && r.content.value === sep;
        if (isSepRepeat && isOptionalSepLit(next, cur.separator)) {
 ...
 */
function liftCommaSep(members) {
    if (members.length < 2 || members.length > 3)
        return null;
    const repeatIdx = findRepeatWithSeparator(members);
    if (repeatIdx === -1)
        return null;
    const repeatNode = members[repeatIdx];
    const sep = repeatNode.separator;
    const elem = repeatNode.content;
    const matchesElem = (r) => rulesEqual(r, elem);
    const matchesOptionalSep = (r) => r.type === 'optional' && r.content.type === 'string' && r.content.value === sep;
 ...
 */
function findRepeatWithSeparator(members) {
...
function rulesEqual(a, b) {
    ...
    switch (a.type) {
        ...
        case 'repeat':
            return a.separator === b.separator && rulesEqual(a.content, b.content);
        case 'repeat1':
            return a.separator === b.separator && rulesEqual(a.content, b.content);
        case 'field':
 ...
 * handling.
 */
export function choice(...members) {
    const normalized = members.map(normalize);
    if (normalized.length === 1)
        return normalized[0];
    const isBlank = (r) => (r.type === 'seq' && r.members.length === 0) || (r.type === 'choice' && r.members.length === 0);
    ...
    if (normalized.length > 0 && normalized.every((m) => m.type === 'string')) {
        return normalizeEnumMembers(normalized, 'grammar');
    }
    if (normalized.length >= 2 && normalized.every((m) => m.type === 'field')) {
        return collapseAllFieldChoiceMembers(normalized);
    }
 ...
 */
function collapseAllFieldChoiceMembers(fieldMembers) {
    ...
    const retyped = fieldMembers.map((f) => ({
        type: 'variant',
        name: f.name,
        content: f.content
    }));
 ...
 */
export function optional(content) {
 ...
 */
function extractRepeatSeparator(resolved) {
 ...
 */
function extractFirstStringFromChoice(r) {
    if (r.type !== 'choice')
        return null;
    const lit = r.members.find((m) => m.type === 'string');
 ...
 */
export function repeat(content) {
    const resolved = normalize(content);
    walkRefs(resolved, (ref) => {
        ref.repeated = true;
    });
    if (resolved.type === 'repeat' && !resolved.separator)
        return resolved;
    if (resolved.type === 'optional') {
 ...
 */
export function repeat1(content) {
    const resolved = normalize(content);
    walkRefs(resolved, (ref) => {
        ref.repeated = true;
    });
    if (resolved.type === 'repeat1' && !resolved.separator)
        return resolved;
    const sep = extractRepeatSeparator(resolved);
    if (sep) {
        return {
...
// ---------------------------------------------------------------------------
export function createProxy(currentRule, refs) {
    return new Proxy({}, {
        get(_target, name) {
            const ref = { refType: 'symbol', from: currentRule, to: name };
            refs.push(ref);
            return {
                type: 'symbol',
                name,
 ...
 */
export function isHiddenKind(name, inlineList) {
    if (name.startsWith('_'))
        return true;
    if (inlineList && inlineList.includes(name))
        return true;
    return false;
}
// ---------------------------------------------------------------------------
...
function walkRefs(rule, visit) {
    ...
    switch (rule.type) {
        ...
        case 'repeat':
        case 'repeat1':
        case 'prec': // prec wrappers are stripped by normalize but defensive
            walkRefs(rule.content, visit);
            return;
        case 'field':
        case 'alias':
 ...
 * write.
 */
function collapseOptionalRepeatInField(resolved) {
    if (resolved.type !== 'optional')
        return resolved;
    const inner = resolved.content;
...
export const token = Object.assign(function token(content) {
...
export const prec = Object.assign(function prec(precedenceOrContent, content) {
    if (content === undefined)
        return normalize(precedenceOrContent);
...
// ---------------------------------------------------------------------------
export function alias(rule, value) {
 ...
 */
function grammarFn(optionsOrBase, options) {
    let baseRules = {};
    ...
    if (options === undefined) {
        ...
        baseGrammar = optionsOrBase.grammar;
        baseRules = { ...baseGrammar.rules };
        opts = options;
    }
    mergeEnrichOverridesIntoOptions(optionsOrBase, opts);
    const refs = seedRefsFromBaseGrammar(baseGrammar);
    ...
    const { roles: collectedRoles } = withRoleScope(() => {
        ...
        evaluateMetadataCallbacksInScope(opts, baseGrammar, refs, { extras, externals, supertypes, inline, conflicts }, (w) => {
            word = w;
        });
    });
 ...
 */
function synthesizeInlineAliasSources(rules, provenanceByKind, externals) {
...
function rewriteInlineAliases(rule, rules, provenanceByKind, externals) {
    const recurse = (r) => rewriteInlineAliases(r, rules, provenanceByKind, externals);
    switch (rule.type) {
        case 'alias':
            if (rule.named && rule.value) {
                ...
                const isBareSymbolToKnownSource = inner.type === 'symbol' && (rules[inner.name] !== undefined || externals.has(inner.name));
                ...
                if (!targetAlreadyExists && !isBareSymbolToKnownSource) {
                    ...
                    return {
                        ...rule,
                        content: { type: 'symbol', name: syntheticHiddenName }
                    };
                }
            }
        ...
        case 'seq':
        case 'choice':
            return { ...rule, members: rule.members.map(recurse) };
        case 'optional':
        case 'repeat':
        case 'repeat1':
        case 'field':
 ...
 */
function synthesizeFieldEnumRules(rules, provenanceByKind) {
    ...
    for (const [parentKind, rule] of Object.entries(rules)) {
        const rewritten = rewriteFieldEnums(rule, parentKind, rules, newRules, memberKeyToCanonicalName, conflictingSites);
        if (rewritten !== rule)
            rewrites.set(parentKind, rewritten);
    }
    // Apply rule rewrites.
 ...
 */
function purgeSupersededEnumRules(rules, provenanceByKind, memberKeyToCanonicalName) {
 ...
 */
function walkFieldEnums(rule, parentKind, rules, out) {
    switch (rule.type) {
        case 'field': {
        ...
        case 'choice':
            for (const m of rule.members)
                walkFieldEnums(m, parentKind, rules, out);
            return;
        case 'optional':
        case 'repeat':
        case 'repeat1':
...
function buildCanonicalEnumNames(occurrences, rules) {
    ...
    for (const group of groups) {
        const chosenName = claimUniqueEnumName(group.name, group.memberKey, claimedNames, rules);
...
function collectConflictingFieldEnumSites(occurrences) {
    const memberKeysBySite = new Map();
    for (const occ of occurrences) {
 ...
 */
function claimUniqueEnumName(baseName, memberKey, claimedNames, rules) {
 ...
 */
function canReuseExistingEnumName(name, memberKey, rules) {
 ...
 */
function deriveCandidateName(group, first, rules) {
 ...
 */
function fieldNameMatchesGrammarRule(fieldName, members, rules) {
 ...
 */
function rewriteFieldEnums(rule, parentKind, rules, newRules, memberKeyToCanonicalName, conflictingSites) {
    const recurse = (r) => rewriteFieldEnums(r, parentKind, rules, newRules, memberKeyToCanonicalName, conflictingSites);
    switch (rule.type) {
        case 'field': {
        ...
        case 'choice': {
            ...
            if (newMembers.every((m, i) => m === rule.members[i]))
                return rule;
    ...
    switch (target.type) {
        ...
        case 'choice': {
            ...
            if (target.members.length === 0)
                return null;
            const allStrings = target.members.every((m) => m.type === 'string');
 ...
 */
function drainPolymorphMetadata(opts) {
    const wireCtx = opts.__wireContext__;
 ...
 */
function drainRefineMetadata(opts) {
    const wireCtx = opts.__wireContext__;
    if (!wireCtx || wireCtx.refineForms.size === 0)
        return undefined;
 ...
 * deferred.
 */
function mergeEnrichOverridesIntoOptions(optionsOrBase, opts) {
    const enrichOverrides = optionsOrBase.__enrichOverrides__;
 ...
 */
function seedRefsFromBaseGrammar(baseGrammar) {
 ...
 */
function evaluateRulesAndInjectSynthetics(opts, baseRules, refs, rules, provenanceByKind, isExtension) {
 ...
 */
function prunePlaceholderOrphans(ctx, rules) {
    for (const name of Object.keys(rules)) {
        if (!name.startsWith('_'))
            continue;
        if (ctx.deposits.has(name))
            continue;
 ...
 */
function evaluateMetadataCallbacksInScope(opts, baseGrammar, refs, sinks, setWord) {
 ...
 */
function evaluateRuleFunctions(opts, baseRules, refs, rules, provenanceByKind, isExtension) {
 ...
 */
function injectSyntheticRules(syntheticRules, rules, provenanceByKind) {
 ...
 */
function inheritBaseGrammarMetadata(opts, baseGrammar, sinks, setWord) {
    ...
    if (inherited) {
        ...
        if (!opts.word && inherited.word)
            setWord(inherited.word);
    }
}
/**
 ...
 */
function appendDedup(sink, value) {
    if (!sink.includes(value))
        sink.push(value);
}
/**
 ...
 */
function evaluateMetadataCallbacks(opts, baseGrammar, refs, sinks, setWord) {
    if (opts.extras) {
        ...
        if (Array.isArray(result)) {
            for (const e of result) {
                const n = normalize(e);
                if (n.type === 'symbol')
                    appendDedup(sinks.extras, n.name);
                else if (n.type === 'pattern')
                    appendDedup(sinks.extras, n.value);
            }
        }
    }
    if (opts.externals) {
        ...
        if (Array.isArray(result)) {
            for (const e of result) {
                const n = normalize(e);
                if (n.type === 'symbol')
                    appendDedup(sinks.externals, n.name);
                else if (n.type === 'string')
                    appendDedup(sinks.externals, n.value);
            }
        }
    }
    if (opts.supertypes) {
        ...
        if (Array.isArray(result)) {
            for (const s of result) {
                ...
                const n = normalize(s);
                if (n.type === 'symbol')
                    appendDedup(sinks.supertypes, n.name);
            }
        }
    }
    if (opts.inline) {
        ...
        if (Array.isArray(result)) {
            for (const i of result) {
                const n = normalize(i);
                if (n.type === 'symbol')
                    appendDedup(sinks.inline, n.name);
            }
        }
    }
    if (opts.conflicts) {
 ...
 */
export async function evaluate(entryPath) {
    const g = globalThis;
 ...
 */
function saveAndInjectDslGlobals(g) {
    const dslFunctions = {
        grammar: grammarFn,
        seq,
        choice,
 ...
 */
async function importAndExtractGrammar(entryPath) {
 ...
 */
function restoreSavedGlobals(g, savedGlobals) {
