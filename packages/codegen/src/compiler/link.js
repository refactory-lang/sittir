 ...
 */
import { isField, isSeq, isChoice, isString, normalizeEnumMembers } from './rule.js';
import { collectGeneratedKindEntries, findGeneratedKindEntry } from './generated-metadata.js';
import { hasAnyField, hasAnyChild } from './node-map.js';
import { collectFieldNames } from './rule.js';
import { isHiddenKind } from './evaluate.js';
import { validateRefineForms } from './link-refine.js';
// ---------------------------------------------------------------------------
// link() — main entry point
// ---------------------------------------------------------------------------
export function link(raw, include, generatedIdTables) {
    ...
    const includeRules = new Set(include?.rules ?? ['promoted']);
    const _includeFields = new Set(include?.fields ?? ['enriched', 'inferred', 'inlined']);
    ...
    const topLevelAliasBodies = collectTopLevelAliasBodies(raw.rules, rules, supertypes, externalRoles);
    ...
    if (raw.refineForms && raw.refineForms.size > 0) {
        for (const [kind, forms] of raw.refineForms) {
            ...
            if (!rule) {
                throw new Error(`refine(${kind}): no rule named '${kind}' found at link time — refine() target must be a top-level rule`);
            }
 ...
 */
function buildExternalRolesMap(rawExternalRoles) {
    return rawExternalRoles ? new Map(rawExternalRoles) : new Map();
}
/**
 ...
 */
function stripResolvedRoleRules(rules) {
 ...
 */
function createSyntheticExternalRules(rules, externals) {
    for (const ext of externals) {
        if (!rules[ext]) {
            rules[ext] = { type: 'pattern', value: '' };
        }
    }
}
function canonicalizeCatalogLiteralRefs(rules, kindEntries) {
    for (const [name, rule] of Object.entries(rules)) {
        rules[name] = canonicalizeRuleLiterals(rule, kindEntries, false);
    }
}
function canonicalizeCatalogLiteralRefsInMap(rules, kindEntries) {
    for (const [name, rule] of rules.entries()) {
        rules.set(name, canonicalizeRuleLiterals(rule, kindEntries, false));
    }
}
function canonicalizeRuleLiterals(rule, kindEntries, allowLiteralRewrite) {
    switch (rule.type) {
        case 'seq':
            return {
                ...rule,
 ...
 */
function classifyAndLogHiddenRules(rules, inline, supertypes, references, derivations, applyPromotedRules) {
    for (const [name, rule] of Object.entries(rules)) {
 ...
 */
function promoteAndLogTerminalRules(rules, derivations, applyPromotedRules) {
    for (const [name, rule] of Object.entries(rules)) {
 ...
 */
function runFieldNameInferencePass(rules, references, derivations) {
 ...
 *   override.
 */
function detectAndLogPolymorphs(rules, derivations) {
    for (const [name, rule] of Object.entries(rules)) {
 ...
 */
function collectAliasedHiddenKinds(rawRules) {
    const out = new Map();
...
function extractTopLevelAliasTarget(rule) {
    ...
    if (rule.type === 'group' ||
        ...
        rule.type === 'token' ||
        rule.type === 'terminal') {
        return extractTopLevelAliasTarget(rule.content);
    }
    return undefined;
}
function collectTopLevelAliasBodies(rawRules, resolvedRules, supertypes, externalRoles) {
    const out = new Map();
    for (const [name, rule] of Object.entries(rawRules)) {
        if (!name.startsWith('_'))
            continue;
        ...
        out.set(name, dereferenceTopLevelAliasBody(resolvedContent, resolvedRules, supertypes, new Set()));
    }
    return out;
}
function extractTopLevelNamedAliasContent(rule) {
    if (rule.type === 'alias' && rule.named)
        return rule.content;
    if (rule.type === 'group' ||
        rule.type === 'variant' ||
        ...
        rule.type === 'token' ||
        rule.type === 'terminal') {
        return extractTopLevelNamedAliasContent(rule.content);
    }
    return undefined;
}
function dereferenceTopLevelAliasBody(rule, resolvedRules, supertypes, seen) {
    if (rule.type !== 'symbol')
        return rule;
    const refName = rule.aliasedFrom ?? rule.name;
    if (supertypes.has(refName))
        return rule;
    if (seen.has(refName))
        return rule;
 ...
 */
function extractAliasedFromName(content, supertypes) {
    if (content.type === 'symbol') {
        if (supertypes.has(content.name))
            return undefined;
        return content.name;
    }
    if (content.type === 'variant' ||
        ...
        return extractAliasedFromName(content.content, supertypes);
    }
    return undefined;
}
 ...
 * shape.
 */
function _wouldInlineAtAssemble(kindName, rules) {
    ...
    const unwrap = (r) => (r.type === 'optional' || r.type === 'variant' ? unwrap(r.content) : r);
...
export function wrapVariants(choice) {
...
function findDetectSymbol(rule) {
    if (rule.type === 'symbol')
        return rule.name;
    if (rule.type === 'field')
        return rule.name;
...
function rulesEqualForVariant(a, b) {
    if (a.type !== b.type)
        return false;
    ...
    switch (a.type) {
        ...
        case 'choice': {
            const bn = b;
            return (a.members.length === bn.members.length && a.members.every((m, i) => rulesEqualForVariant(m, bn.members[i])));
        }
        case 'optional':
        case 'repeat':
        case 'repeat1': {
...
export function promotePolymorph(rule) {
    ...
    if (!allVariantContentsAreDistinguishable(contents))
        return rule;
    if (variantFieldSetsAreHomogeneous(contents))
        return rule;
 ...
 */
function allVariantContentsAreDistinguishable(contents) {
    const isDistinguishable = (c) => hasAnyField(c) || hasAnyChild(c) || (c.type === 'symbol' && !c.hidden) || c.type === 'supertype';
 ...
 */
function variantFieldSetsAreHomogeneous(contents) {
 ...
 */
function buildPolymorphFromFusedForms(choice, prefix, suffix) {
    const fuse = (inner) => {
    ...
    const forms = choice.members.map((m, i) => ({
...
export function applyOverridePolymorphs(rules, variants, derivations) {
...
function buildOverridePolymorphForms(parentKind, children, found, rules) {
    const variantChildSymbolNames = new Set(children.map((child) => `${parentKind}_${child}`));
    const explicitVariantForms = new Map();
    for (const child of children) {
        ...
        const content = directMember?.type === 'variant'
    ...
    for (const member of found.choice.members) {
        ...
        if (!discriminatorKind)
            continue;
        orderedForms.push({
            name: visibleOverrideFormName(discriminatorKind),
            content: fuseOverridePolymorphFormContent(found.prefix, resolveVisibleSymbolRule(discriminatorKind, rules), found.suffix),
...
function resolveVisibleSymbolRule(name, rules) {
    const hiddenAlias = `_${name}`;
    return rules[hiddenAlias]
        ? { type: 'symbol', name, aliasedFrom: hiddenAlias }
        : { type: 'symbol', name };
}
function fuseOverridePolymorphFormContent(prefix, content, suffix) {
    if (prefix.length === 0 && suffix.length === 0)
        return content;
    return {
        type: 'seq',
...
function matchesOverrideChoiceMember(member, targetSymbolName) {
    const core = member.type === 'variant' ? member.content : member;
    if (core.type === 'symbol')
        return core.name === targetSymbolName;
    return false;
}
function findMatchingOverrideVariantKind(member, variantChildSymbolNames) {
    const core = member.type === 'variant' ? member.content : member;
    if (core.type === 'symbol') {
    ...
    for (const inner of core.members) {
        ...
        if (nested.type === 'symbol' && variantChildSymbolNames.has(nested.name))
            return nested.name;
    }
    return undefined;
}
function extractOverridePassthroughKind(member, variantChildSymbolNames) {
    const core = member.type === 'variant' ? member.content : member;
    if (core.type !== 'symbol')
        return undefined;
    return variantChildSymbolNames.has(core.name) ? undefined : core.name;
}
function visibleOverrideFormName(kind) {
 ...
 */
function emitVariantChildDerivations(parentKind, children, derivations) {
    for (const child of children) {
 ...
 */
function pushAmbientScaffoldIntoVariantChildren(rules, parentKind, children) {
    ...
    if (rewritten !== parentRule)
        rules[parentKind] = rewritten;
}
/**
 ...
 * unchanged.
 */
function rewriteSeqWithVariantAliasChoice(rule, rules, variantChildVisibleNames) {
    switch (rule.type) {
        case 'seq': {
        ...
        case 'token':
        case 'terminal': {
            const content = rewriteSeqWithVariantAliasChoice(rule.content, rules, variantChildVisibleNames);
            return { ...rule, content };
        }
        default:
            return rule;
 ...
 * need to count.
 */
function isAllAliasChoice(rule, variantChildVisibleNames) {
    if (rule.type !== 'choice' || rule.members.length === 0)
        return false;
    return rule.members.every((m) => {
        const core = m.type === 'variant' ? m.content : m;
        if (core.type === 'alias')
            return variantChildVisibleNames.has(core.value);
        if (core.type === 'symbol')
            return variantChildVisibleNames.has(core.name);
        return false;
    });
}
/**
 ...
 */
function applyVariantScaffoldPushDown(seq, choiceIdx, rules) {
    const prefix = seq.members.slice(0, choiceIdx).filter((m) => m.type === 'string');
    const suffix = seq.members.slice(choiceIdx + 1).filter((m) => m.type === 'string');
    if (prefix.length === 0 && suffix.length === 0)
        return seq; // nothing to push
    const choice = seq.members[choiceIdx];
    for (const member of choice.members) {
        ...
        if (hiddenName in rules)
            rules[hiddenName] = wrapped;
        if (visibleName in rules)
            rules[visibleName] = wrapped;
    }
...
export function findVariantChoice(rule) {
 ...
 */
function findVariantSymbolInSeqArm(choice, targetSymbolName, rules) {
    for (const arm of choice.members) {
        ...
        const foundIdx = arm.members.findIndex((m) => {
            if (m.type === 'symbol')
                return m.name === targetSymbolName;
            if (m.type === 'alias')
                return m.value === targetSymbolName;
            if (m.type === 'choice') {
                return m.members.some((mm) => {
                    ...
                    if (core.type === 'symbol')
                        return core.name === targetSymbolName;
                    if (core.type === 'alias')
                        return core.value === targetSymbolName;
                    return false;
                });
            }
            return false;
        ...
        const resolvedSymbol = rules[hiddenAlias]
        ...
        const newMembers = arm.members.map((m, i) => (i === foundIdx ? resolvedSymbol : m));
 ...
 */
function looksLikePolymorphCandidate(choice) {
    if (choice.members.length < 2)
        return false;
    const contents = choice.members.map((m) => (m.type === 'variant' ? m.content : m));
    ...
    const isDistinguishable = (c) => {
        if (hasAnyField(c) || hasAnyChild(c))
            return true;
        if (c.type === 'symbol' || c.type === 'supertype')
            return true;
        ...
        if (c.type === 'string' || c.type === 'pattern')
            return true;
        if (c.type === 'seq')
            return c.members.some(isDistinguishable);
    ...
    if (contents.some((c) => !isDistinguishable(c)))
        return false;
    ...
    const signature = (c) => {
        ...
        if (c.type === 'symbol' || c.type === 'supertype')
            return `sym:${c.name}`;
        if (c.type === 'seq')
            return `seq:${c.members.map(signature).join('|')}`;
        return `shape:${c.type}`;
    };
 ...
 */
function choiceNeedsVariantWrapping(choice) {
    const armNeedsVariant = (c) => {
 ...
 */
export function findAllPolymorphCandidates(rule) {
    const qualifies = (c) => looksLikePolymorphCandidate(c) && choiceNeedsVariantWrapping(c);
    const out = [];
    const pushCandidate = (choice, prefix, suffix, path, fieldWrapperName) => {
    ...
    const walk = (r, path, prefix, suffix) => {
 ...
 */
function isTerminalShape_allowBareTerm(rule) {
    switch (rule.type) {
        case 'string':
        case 'pattern':
        case 'indent':
...
// ---------------------------------------------------------------------------
function resolveRule(rule, currentName, allRules, supertypes, externalRoles) {
    switch (rule.type) {
        case 'seq':
            return {
                type: 'seq',
 ...
 *   signal.
 */
function resolveRepeat1PreservingNonEmpty(rule, currentName, allRules, supertypes, externalRoles) {
    return {
        ...rule,
        content: resolveRule(rule.content, currentName, allRules, supertypes, externalRoles)
    };
 ...
 */
function resolveNamedAliasWithProvenance(content, targetName, supertypes) {
    const aliasedFrom = extractAliasedFromName(content, supertypes);
    const sym = aliasedFrom
 ...
 */
function resolveSymbolRoleOrPass(rule, allRules, externalRoles) {
    const preBound = externalRoles.get(rule.name);
    if (preBound) {
        return { type: preBound.role };
    }
...
// ---------------------------------------------------------------------------
function classifyHiddenRule(name, rule, supertypes, _references) {
 ...
 */
function classifyHiddenChoiceRule(name, rule, supertypes) {
    if (rule.members.every((m) => m.type === 'string')) {
        return normalizeEnumMembers(rule.members, 'promoted');
    }
    const supertypeCompatible = (m) => m.type === 'symbol' || (m.type === 'alias' && m.named) || m.type === 'enum' || m.type === 'string';
 ...
 */
function classifyHiddenSeqRule(name, rule) {
    if (hasAnyField(rule)) {
        return {
            type: 'group',
            name,
 ...
 * symbol in a seq.
 */
function collectSubtypeNames(rule) {
    const names = [];
    const visit = (current) => {
        switch (current.type) {
            case 'symbol':
            ...
            case 'alias':
                ...
                return;
            case 'string':
                if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(current.value))
...
// ---------------------------------------------------------------------------
export function enrichPositions(rules, refs) {
    for (const ref of refs) {
        const rule = rules[ref.from];
        if (!rule || rule.type !== 'seq')
            continue;
...
// ---------------------------------------------------------------------------
export function computeParentSets(refs) {
...
function hoistIndentIntoRepeat(rules) {
    for (const [, rule] of Object.entries(rules)) {
        walkForIndentHoist(rule, rules);
    }
}
function walkForIndentHoist(rule, rules) {
    switch (rule.type) {
        case 'seq': {
            ...
            for (let i = 0; i < rule.members.length; i++) {
                if (rule.members[i].type !== 'indent')
                    continue;
                for (let j = i + 1; j < rule.members.length; j++) {
                    if (assignRepeatSeparator(rule.members[j], rules, new Set()))
                        break;
                    if (rule.members[j].type === 'dedent')
                        break;
                }
            }
            for (const m of rule.members)
                walkForIndentHoist(m, rules);
            return;
        }
        case 'choice':
            for (const m of rule.members)
 ...
 */
function assignRepeatSeparator(rule, rules, visited) {
    if (rule.type === 'repeat' || rule.type === 'repeat1') {
        if (!rule.separator)
            rule.separator = BLOCK_SEPARATOR;
        return true;
    }
    if (rule.type === 'symbol') {
    ...
    if (rule.type === 'seq') {
        for (const m of rule.members) {
            if (assignRepeatSeparator(m, rules, visited))
                return true;
        }
        return false;
    }
 ...
 */
function computeHiddenBearerSet(rules) {
    const bearers = new Set();
    for (const [name, rule] of Object.entries(rules)) {
        if (name.startsWith('_') && containsIndent(rule))
            bearers.add(name);
    }
    let changed = true;
    while (changed) {
...
function annotateBlockBearerFields(rules) {
...
function referencesBearer(rule, bearers) {
    switch (rule.type) {
        case 'symbol':
...
function markBlockBearerFields(rule, bearers) {
    switch (rule.type) {
        case 'field':
            if (referencesBearer(rule.content, bearers)) {
                rule.blockBearer = true;
            }
        ...
        case 'choice':
            for (const m of rule.members)
                markBlockBearerFields(m, bearers);
            return;
        case 'optional':
        case 'repeat':
        case 'repeat1':
...
// ---------------------------------------------------------------------------
function detectClause(content, currentName) {
    if (isSeq(content)) {
...
function inferFieldNames(references) {
    // Group references by `to` (the symbol being referenced).
    const refsByTo = new Map();
    ...
    for (const [to, refs] of refsByTo) {
        ...
        if (agreement < 0.8)
            continue;
        const confidence = agreement >= 0.95 ? 'high' : agreement >= 0.85 ? 'medium' : 'low';
        inferred.set(to, {
            name: bestName,
            confidence,
            agreement,
 ...
 */
function applyInferredFields(rule, ruleName, inferred, apply, log, _insideField = false) {
    switch (rule.type) {
        case 'symbol': {
            if (_insideField)
        ...
        case 'group': {
            ...
            return {
                rule: r.applied ? { ...rule, content: r.rule } : rule,
                applied: r.applied
            };
        }
        default:
 ...
 */
function collectSeqSiblingDuplicates(members) {
    const symCounts = new Map();
    for (const m of members) {
        const sn = unwrapSymRef(m);
        if (sn)
            symCounts.set(sn, (symCounts.get(sn) ?? 0) + 1);
    }
    const dups = new Set();
    for (const [sn, cnt] of symCounts)
        if (cnt > 1)
            dups.add(sn);
    return dups;
}
 ...
 *     `group` otherwise.
 */
function collectRepeatedShapes(rules, out) {
    ...
    const existingSupertypeKeys = new Set();
    ...
    for (const [key, parents] of parentsByKey) {
        if (parents.size < 2)
            continue;
        if (existingSupertypeKeys.has(key))
            continue;
        ...
        const shape = kinds.every((k) => /^[\w]+$/.test(k)) ? 'supertype' : 'group';
        out.push({
 ...
 * resolver kind lists.
 */
function collectFieldKindSets(rule, yield_) {
    switch (rule.type) {
        case 'field': {
 ...
 */
function directContentKinds(rule) {
    switch (rule.type) {
        case 'symbol':
            return [rule.name];
        case 'supertype':
...
function suggestSharedName(kinds) {
