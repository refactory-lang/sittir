 ...
 */
function deriveFieldsRaw(rule: Rule, outerMultiplicity: Multiplicity): AssembledField[] {
	switch (rule.type) {
		case "field": {
			...
			const kindNames = values
				.filter(isNodeRef)
				.map((v) =>
					isUnresolvedRef(v.node) ? (v.node as UnresolvedRef).name : (v.node as AssembledNode).kind,
				);
...
export function deriveChildren(rule: Rule): AssembledChild[] {
	...
	for (const m of members) {
		const t = memberTarget(m);
		const mult: Multiplicity = t !== null && (targetCounts.get(t) ?? 0) > 1 ? "array" : "single";
		...
		case "symbol":
			...
			// (only source kinds exist in rules post-synthesis-removal).
			{
				...
				out.push({
					...
					values: [
						{ kind: "node-ref", node: { kind: "unresolved-ref", name: refName }, multiplicity },
					],
				});
			}
			break;
 ...
 */
function deriveValuesForRule(rule: Rule, multiplicity: Multiplicity): NodeOrTerminal[] {
	switch (rule.type) {
		case "symbol":
...
export abstract class AssembledNodeBase<R extends Rule = Rule> {
	...
	constructor(
		kind: string,
		rule: R,
		opts?: { factoryName?: string; irKey?: string; source?: RuleSource; hidden?: boolean },
	) {
		this.kind = kind;
		this.rule = rule;
		...
		this.factoryName =
			opts?.hidden === true ? undefined : (opts?.factoryName ?? derived.factoryName);
...
function wrapOptionalFieldPlaceholders(
	...
	return tmpl.replace(
		placeholder,
		(full: string, lead: string, dollars: string, name: string, offset: number) => {
			const key = name.toLowerCase();
...
export class AssembledBranch extends AssembledNodeBase<SeqRule | ChoiceRule> {
	...
	constructor(
		...
		simplifiedRule: Rule,
		opts?: { factoryName?: string; irKey?: string; variantChildKinds?: readonly string[] },
	) {
...
export class AssembledPolymorph extends AssembledNodeBase<PolymorphRule> {
	...
	renderTemplate(rules?: Record<string, Rule>, wordMatcher?: RegExp): { template: string } {
		...
		for (const form of this.#forms) {
			...
			const wrapped =
				localOptional.size > 0 ? wrapOptionalFieldPlaceholders(template, localOptional) : template;
...
export class AssembledGroup extends AssembledNodeBase<Rule> {
	...
	constructor(
		...
		const factoryName =
			opts?.factoryName ?? (kind.startsWith("_") ? `_${nameNode(kind).factoryName}` : undefined);
