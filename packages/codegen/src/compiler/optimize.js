 ...
 * re-collapse.
 */
import { isChoice } from './rule.js';
import { simplifyRules } from './simplify.js';
import { compileWordMatcher } from './common.js';
/**
 ...
 */
function applyNormalizationPasses(rawRules) {
    let rules = {};
...
export function optimize(linked) {
 ...
 */
export function fanOutSeqChoices(rule) {
    switch (rule.type) {
        case 'seq': {
 ...
 */
function isAtomForFactoring(rule) {
    switch (rule.type) {
        case 'symbol':
        case 'string':
        case 'pattern':
 ...
 */
function extractFactoredChoiceBody(members, seqs, prefixLen, suffixLen) {
    ...
    for (let i = 0; i < members.length; i++) {
        ...
        const bodyRule = body.length === 1 ? body[0] : { type: 'seq', members: body };
 ...
 */
export function factorChoiceBranches(rule) {
    switch (rule.type) {
        case 'choice': {
            ...
            if (!canFactor)
                return { ...rule, members };
            const seqs = unwrapped.map((b) => (b.type === 'seq' ? b.members : [b]));
            ...
            const core = nonEmpty.length === 1 ? nonEmpty[0] : { type: 'choice', members: nonEmpty };
            const inner = hasEmpty ? { type: 'optional', content: core } : core;
            const outerMembers = [...prefix, inner, ...suffix];
 ...
 */
export function dedupeSeqMembers(rule) {
    switch (rule.type) {
        case 'seq': {
 ...
 */
function inlineSingleUseHidden(rules) {
 ...
 */
function iterateInliningToFixedPoint(work) {
    for (let pass = 0; pass < 4; pass++) {
        ...
        for (const [name, rule] of Object.entries(work)) {
            ...
            if (!name.startsWith('_'))
                continue;
            if (isStructurallyMeaningfulHiddenRule(rule))
                continue;
 ...
 */
function isStructurallyMeaningfulHiddenRule(rule) {
    return (rule.type === 'supertype' ||
        rule.type === 'polymorph' ||
        rule.type === 'enum' ||
        rule.type === 'terminal' ||
        rule.type === 'group');
 ...
 */
function spliceHiddenRuleIntoSingleParent(work, name, rule) {
 ...
 */
function countReferences(rules) {
...
function walkSymbols(rule, visit) {
    switch (rule.type) {
        case 'symbol':
            visit(rule.name);
            return;
        ...
        case 'polymorph':
            for (const form of rule.forms)
                walkSymbols(form.content, visit);
            return;
        case 'supertype':
 ...
 */
function replaceSymbolRef(rule, targetName, targetRule) {
    switch (rule.type) {
        case 'symbol':
 ...
 */
export function collapseWrappers(rule) {
    switch (rule.type) {
        case 'optional': {
        ...
        case 'repeat': {
            ...
            if (inner.type === 'optional')
                return { ...rule, content: inner.content };
            return { ...rule, content: inner };
        }
        case 'seq': {
...
function outerFromParts(prefix, suffix) {
...
function mapPolymorphForms(rule, rewrite) {
    return {
        ...rule,
...
export function rulesEqual(a, b) {
    ...
    switch (a.type) {
        ...
        case 'symbol':
            ...
            return a.name === b.name && a.aliasedFrom === b.aliasedFrom;
        case 'seq':
            return (a.members.length === b.members.length &&
                a.members.every((m, i) => rulesEqual(m, b.members[i])));
        case 'choice':
            return (a.members.length === b.members.length &&
                a.members.every((m, i) => rulesEqual(m, b.members[i])));
        case 'optional':
        ...
        case 'repeat':
            return rulesEqual(a.content, b.content) && a.separator === b.separator;
        case 'field':
...
// ---------------------------------------------------------------------------
export function factorSeqChoice(branches) {
...
function findCommonPrefix(seqs) {
    if (seqs.length === 0)
        return 0;
    const first = seqs[0];
    let len = 0;
...
function findCommonSuffix(seqs, prefixLen) {
    if (seqs.length === 0)
        return 0;
    const first = seqs[0];
    let len = 0;
...
export { wrapVariants, deduplicateVariants, nameVariant, tokenToName } from './link.js';
// ---------------------------------------------------------------------------
// Spacing
// ---------------------------------------------------------------------------
const WORD_CHAR = /\w/;
export function needsSpace(prev, next) {
