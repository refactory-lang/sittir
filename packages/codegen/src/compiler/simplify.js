 ...
 */
function isEmptyMatchMember(rule) {
...
function isKeywordShape(value, wordMatcher) {
    if (wordMatcher)
        return wordMatcher.test(value);
    return /^\w+$/.test(value);
}
export function simplifyRule(rule, wordMatcher, inField = false) {
    switch (rule.type) {
        case 'seq': {
            ...
            const members = rule.members
                ...
                if (m.type === 'string' && !isKeywordShape(m.value, wordMatcher))
                    return false;
                if (m.type === 'seq' && m.members.length === 0)
                    return false;
                return true;
            })
        ...
        case 'choice': {
            ...
            if (empty >= 0 && members.length > 1) {
                ...
                const inner = nonEmpty.length === 1 ? nonEmpty[0] : { type: 'choice', members: nonEmpty };
 ...
 *      duplicate-named field occurrences across choice branches.
 */
export function simplifyRules(rules, wordMatcher) {
    const out = {};
 ...
 * converges in 2-3.
 */
function normalizeToFixpoint(rule, wordMatcher, rules) {
    const MAX_ITERS = 16;
    let current = rule;
    for (let i = 0; i < MAX_ITERS; i++) {
        const next = simplifyRule(inlineGroupRefs(current, rules), wordMatcher);
        if (rulesStructurallyEqual(current, next))
            return next;
        current = next;
    }
 ...
 */
function rulesStructurallyEqual(a, b) {
 ...
 */
function hoistFieldOutOfSingleContentWrapper(rule) {
    if (rule.type !== 'optional' && rule.type !== 'repeat' && rule.type !== 'repeat1')
        return rule;
    const inner = rule.content;
    if (inner.type !== 'field')
        return rule;
    const wrapper = { ...rule, content: inner.content };
 ...
 */
export function hoistInnerFieldOutOfFieldWrapper(rule) {
    if (rule.type !== 'field')
        return rule;
    const content = rule.content;
    if (content.type === 'field')
        return rule; // direct nesting handled elsewhere
    if (!hasInnerFieldAtExposableDepth(content))
        return rule;
    // Bail if a named-symbol sibling would lose its outer-field label.
    if (hasNamedSiblingOfInnerField(content))
        return rule;
    return content;
}
/**
 ...
 */
function hasNamedSiblingOfInnerField(rule) {
    switch (rule.type) {
        case 'seq': {
...
function isNamedReference(rule) {
    switch (rule.type) {
        case 'symbol':
        case 'supertype':
            return true;
...
function hasInnerFieldAtExposableDepth(rule) {
    switch (rule.type) {
        case 'field':
            return true;
        case 'optional':
 ...
 */
function hoistSharedFieldAcrossChoiceBranches(rule) {
    if (rule.members.length < 2)
        return rule;
    if (rule.members.some((m) => m.type === 'variant'))
        return rule;
 ...
 * occurrences.
 */
function normalizeBranchToMembers(branch) {
    if (branch.type === 'seq')
 ...
 */
function countFieldNames(members) {
 ...
 */
function firstFieldNameSharedExactlyOncePerBranch(perBranchCounts) {
    if (perBranchCounts.length === 0)
        return null;
    ...
    outer: for (const [name, count] of first) {
        ...
        for (let i = 1; i < perBranchCounts.length; i++) {
            if (perBranchCounts[i].get(name) !== 1)
                continue outer;
        }
        return name;
    }
 ...
 */
function extractFieldAcrossBranches(perBranch, name) {
    const hoistedContents = [];
    const residuals = [];
    let hoistedFieldTemplate = null;
    for (const members of perBranch) {
        ...
        residuals.push(rest.length === 0 ? { type: 'seq', members: [] } : rest.length === 1 ? rest[0] : { type: 'seq', members: rest });
    }
    const unionedContent = hoistedContents.length === 1 ? hoistedContents[0] : { type: 'choice', members: hoistedContents };
    const hoistedField = {
    ...
    if (nonEmptyResiduals.length === 0)
        return hoistedField;
    const residualCore = nonEmptyResiduals.length === 1 ? nonEmptyResiduals[0] : { type: 'choice', members: nonEmptyResiduals };
    const residualPart = hasEmptyResidual ? { type: 'optional', content: residualCore } : residualCore;
 ...
 */
function mergeChoiceBranches(rule) {
    ...
    if (unwrapped.every((b) => b.type === 'field')) {
 ...
 */
function positionsAreMergeable(position) {
...
function mergePosition(position) {
    const first = position[0];
    if (first.type === 'field') {
        const fields = position.filter((p) => p.type === 'field');
        const contents = dedupeByJson(fields.map((f) => f.content));
        const mergedContent = contents.length === 1 ? contents[0] : { type: 'choice', members: contents };
        return { ...first, content: mergedContent };
    }
    return first;
}
/** Deduplicate rules by JSON equality, preserving first-seen order. */
function dedupeByJson(rules) {
    const seen = new Set();
    const out = [];
 ...
 */
export function hoistInnerFieldsForTemplate(rule) {
    switch (rule.type) {
        case 'seq':
            return {
                ...rule,
 ...
 */
export function inlineGroupRefs(rule, rules, visited = new Set()) {
    const recurse = (r, v) => inlineGroupRefs(r, rules, v);
    switch (rule.type) {
        case 'symbol': {
            if (!rule.hidden)
                return rule;
        ...
        case 'token':
            return {
                ...rule,
                content: recurse(rule.content, visited)
            };
        default:
            return rule;
    }
 ...
 */
function resolveGroupOrMultiInlineTarget(target) {
    const isGroup = target.type === 'group';
    const isMulti = extractRepeatShape(target) !== null;
    if (!isGroup && !isMulti)
        return null;
    return isGroup ? target.content : target;
}
/**
 ...
 */
export function extractRepeatShape(rule) {
    switch (rule.type) {
        case 'repeat':
        ...
        case 'group':
        case 'token':
            return extractRepeatShape(rule.content);
        default:
            return null;
    }
}
