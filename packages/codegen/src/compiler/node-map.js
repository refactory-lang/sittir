 ...
 */
import { isSeq, isField } from './rule.js';
import { renderRuleTemplate, deriveWalkSlots, findRepeatSeparator, findRepeatFlag } from './template-walker.js';
...
export function isTerminalValue(v) {
...
export function mergeSlotValues(slots) {
    if (slots.length === 0)
        return undefined;
    return {
...
export function pluralize(name) {
    if (name.endsWith('s') || name.endsWith('List') || name.endsWith('children') || name.endsWith('Children'))
        return name;
    if (/[Cc]hild$/.test(name))
        return name.slice(0, -5) + (name.endsWith('Child') ? 'Children' : 'children');
 ...
 * deriveFields.
 */
export function hasAnyField(rule) {
    switch (rule.type) {
        case 'field':
            return true;
        case 'seq':
 ...
 */
export function hasAnyChild(rule) {
    switch (rule.type) {
        case 'symbol':
        case 'supertype':
            return true;
...
const auditKindsByShape = new Map();
...
let currentAuditKind;
export function setAuditKindContext(kind) {
    currentAuditKind = kind;
}
function auditDerivationShape(rule, context) {
    ...
    if (mode === 'strict') {
        ...
        throw new Error(`derive: non-canonical shape '${shape}' reached ${context} derivation for '${kindLabel}'. ` +
    ...
    if (currentAuditKind !== undefined) {
        ...
        if (!kinds.includes(currentAuditKind))
            kinds.push(currentAuditKind);
...
function classifyTopLevelShape(rule) {
    ...
    switch (rule.type) {
        ...
        case 'choice': {
            ...
            // with fields.
            if (rule.members.every((m) => m.type === 'variant'))
                return 'canonical';
 ...
 */
function isTokenLikeChoiceMember(m) {
    const peel = (r) => r.type === 'alias'
        ? peel(r.content)
        : r.type === 'token'
            ? peel(r.content)
            : r.type === 'variant'
    ...
    if (core.type === 'string' || core.type === 'pattern')
        return true;
    ...
    if (core.type === 'optional')
        return isTokenLikeChoiceMember(core.content);
    ...
    if (core.type === 'repeat1' || core.type === 'repeat') {
        ...
        if (inner.type === 'enum')
            return true;
        if (inner.type === 'string' || inner.type === 'pattern')
            return true;
        if (inner.type === 'symbol' || inner.type === 'supertype')
            return true;
 ...
 */
function isFlatSymbolSeqOrTokenLike(m) {
    if (m.type === 'seq') {
...
export function dumpDerivationAudit(label = 'derivation-audit') {
    if (!DERIVE_AUDIT || auditCounts.size === 0)
        return;
 ...
 */
function _deriveFieldsInternal(rule, kindEntries) {
 ...
 */
function mergeFieldsByName(fields) {
    if (fields.length <= 1)
        return fields;
 ...
 */
function deriveFieldsRaw(rule, outerMultiplicity, kindEntries) {
    switch (rule.type) {
        case 'field': {
        ...
        case 'choice': {
            ...
            if (values.length === 0)
                return [];
            const firstRef = values.find((v) => v.kind === 'node-ref');
            const isMultiSlot = values.some((v) => v.multiplicity === 'array' || v.multiplicity === 'nonEmptyArray');
            const baseName = firstRef
                ? (firstRef.node.name ?? '').replace(/^_+/, '') || (isMultiSlot ? 'children' : 'child')
                : isMultiSlot
                    ? 'children'
                    : 'child';
        ...
        case 'polymorph':
            ...
            throw new Error(`deriveFieldsRaw: unexpected '${rule.type}' in canonical input — ` +
 ...
 */
function fieldContentMultiplicity(content, outerMultiplicity) {
    switch (content.type) {
        case 'repeat':
            return 'array';
        case 'repeat1':
 ...
 */
export function deriveSlots(rule, kindEntries) {
 ...
 */
function deriveAliasSources(rule) {
    const out = {};
    const walk = (r) => {
        switch (r.type) {
            case 'symbol':
                if (r.aliasedFrom) {
                    out[r.name] = r.aliasedFrom;
                }
                return;
            case 'choice':
            case 'seq':
 ...
 */
export function isSyntheticFieldWrapper(content) {
 ...
 */
function deriveValuesForRule(rule, multiplicity, kindEntries) {
    switch (rule.type) {
        case 'symbol':
...
function dedupeValues(values) {
    ...
    for (const v of values) {
        const key = v.kind === 'node-ref'
            ? `node-ref:${v.node.name ?? '?'}:${v.multiplicity}`
 ...
 * flattening.
 */
function prepareKindForPascalCase(normalized) {
...
export class AssembledNodeBase {
     ...
     */
    renderTemplate(_rules, _wordMatcher, _externals) {
        return undefined;
    }
}
/**
 ...
 */
function inlineJinjaClauses(template, clauses) {
    ...
    const isClauseName = (lowerName) => clauses[lowerName] !== undefined;
    ...
    for (let pass = 0; pass < 16; pass++) {
        ...
        const next = current.replace(/\$([A-Z][A-Z0-9_]*_CLAUSE)/g, (full, marker) => {
            ...
            if (body === undefined)
                return full; // not a clause we emitted — leave as-is
            ...
            const convertedBody = body.replace(/(?<!\$)\$([A-Z][A-Z0-9_]*)/g, (m, name) => {
 ...
 */
function childrenMayBeEmpty(rule) {
    switch (rule.type) {
        case 'repeat':
        case 'optional':
            return true;
 ...
 */
function absorbFlankingChildrenSpaces(tmpl) {
    ...
    let result = tmpl.replace(/ (\{\{ children \| \w+\([^)]*\) \}\}) /g, '{%- if children | isPresent %} $1 {% endif -%}');
    ...
    result = result.replace(/\{(\{\{ children \| \w+\([^)]*\) \}\})\}/g, '{ {%- if children | isPresent %} $1 {% endif -%} }');
    return result;
}
/**
 ...
 */
function hasSingularChildrenSlot(children) {
    return children.length === 1 && !deriveSlotCardinality(children[0]).multiple;
}
function normalizeChildrenPlaceholderArity(tmpl, children) {
    if (!hasSingularChildrenSlot(children) || !tmpl.includes('$$$CHILDREN')) {
        return tmpl;
    }
    return tmpl.replaceAll('$$$CHILDREN', '$CHILDREN');
}
function normalizeChildrenClausePlaceholderArity(clauses, children) {
    if (!hasSingularChildrenSlot(children))
        return clauses;
    let changed = false;
    const normalized = {};
...
export function translateToJinja(tmpl, meta) {
    ...
    const translated = guarded.replace(varPattern, (_full, pfx, name) => {
 ...
 */
function absorbHeadConditionalTrailingSpace(tmpl) {
    let work = tmpl;
    let runStart = 0;
 ...
 */
function wrapOptionalFieldPlaceholders(tmpl, optionalFields) {
    ...
    return tmpl.replace(placeholder, (full, lead, dollars, name, offset) => {
        const key = name.toLowerCase();
        ...
        //
        if (SPECIAL_PLACEHOLDERS.has(key))
            return full;
        ...
        if (dollarStart > 0 && tmpl[dollarStart - 1] === '$')
            return full;
        if (offset >= tmpl.length)
            return full;
        // Skip placeholders enclosed in a walker-emitted Jinja
        // conditional — the placeholder is already guarded.
        if (isWithinGuardedRange(dollarStart, guardedRanges))
            return full;
 ...
 * `endif` markers.
 */
function computeGuardedRanges(tmpl) {
    const ranges = [];
...
function isWithinGuardedRange(offset, ranges) {
...
/** @internal — exported for direct unit testing. */
export function filterForFlanks(key, meta) {
    ...
    if (key === 'children') {
        if (meta.joinByLeading && meta.joinByTrailing)
            return 'joinWithFlanks';
        if (meta.joinByTrailing)
            return 'joinWithTrailing';
        if (meta.joinByLeading)
            return 'joinWithLeading';
        return 'join';
    }
 ...
 * structural wrapper.
 */
function deriveOptionalSlotNames(slots) {
    const out = new Set();
    for (const slot of slots) {
        ...
        if (isMultiple(slot) && !isNonEmpty(slot))
            out.add(slot.name);
    }
    return out;
}
...
function buildRenderSurface(opts) {
    ...
    const slots = (opts?.slots ?? []).map((slot) => {
        ...
        const view = field != null && slot.view === 'list' ? 'field' : slot.view;
 ...
 */
function deriveCrossFormOptionalFields(forms) {
    if (forms.length <= 1)
        return new Set();
 ...
 */
function tryCrossFormOptionalCollapse(forms, rawTemplates, formNames) {
    const crossFormOptional = deriveCrossFormOptionalFields(forms);
    if (crossFormOptional.size === 0)
        return null;
...
function inlineSingleParameterlessChildTemplate(form, rawTemplate, rules, wordMatcher, externals) {
    if (!rawTemplate.includes('$$$CHILDREN'))
        return rawTemplate;
    ...
    if (!isNodeRef(value) || isUnresolvedRef(value.node) || !value.node.isParameterless)
        return rawTemplate;
    const stampedLiteral = value.node.modelType === 'keyword' || value.node.modelType === 'token'
        ? resolveStampedLiteral(value.node.stampExpression)
        : undefined;
    if (stampedLiteral !== undefined)
        return rawTemplate.replace('$$$CHILDREN', stampedLiteral);
...
function inlineFixedParameterlessSlotPlaceholders(form, rawTemplate, rules, wordMatcher, externals) {
...
function resolveStampedLiteral(stampExpression) {
    if (!stampExpression?.endsWith(' as const'))
        return undefined;
    try {
        return JSON.parse(stampExpression.slice(0, -' as const'.length));
    }
    catch {
        return undefined;
    }
...
function longestCommonPrefix(values) {
    ...
    for (let i = 1; i < values.length && prefix.length > 0; i++) {
        ...
        let common = 0;
        while (common < prefix.length && common < value.length && prefix[common] === value[common])
            common++;
        prefix = prefix.slice(0, common);
    }
    return prefix;
}
function tryChildrenPresenceCollapse(rawTemplates, formNames) {
    if (formNames.length < 2)
        return null;
    const templates = formNames.map((name) => rawTemplates[name]).filter((value) => value != null);
    if (templates.length !== formNames.length)
        return null;
    ...
    if (childSuffixes.length !== 1 || literalSuffixes.length !== 1)
        return null;
 ...
 */
function escapeJinjaBraceCollisions(s) {
    let prev = s;
    for (let i = 0; i < 4; i++) {
        const next = prev
            .replace(/\{(\{\{[^}]+\}\})/g, '{ $1')
 ...
 */
function buildSlotsRecord(kind, rule, kindEntries) {
    const out = {};
        ...
        for (const [storageName, slots] of byStorageName) {
            if (slots.length > 1) {
                const details = slots.map((s) => {
                    const kinds = s.values.map((v) => v.kind === 'terminal'
                        ? `"${v.value}"`
...
function freezeSlotRecord(slots) {
    const out = {};
    for (const slot of slots) {
        out[slot.name] = slot;
    }
...
export class AssembledBranch extends AssembledNodeBase {
     ...
     */
    _slots;
    constructor(kind, rule, simplifiedRule, opts) {
    ...
    renderTemplate(rules, wordMatcher, externals) {
        ...
        if (!template) {
            throw new Error(`AssembledBranch.renderTemplate: '${this.kind}' produced an empty template. ` +
 ...
 */
function isVerbatimTokenStream(rule) {
    if (rule.type !== 'choice')
        return false;
    if (rule.members.length === 0)
        return false;
    return rule.members.every((m) => {
        ...
        if (core.type !== 'seq' || core.members.length !== 3)
            return false;
        const [start, mid, end] = core.members;
        if (!start || !mid || !end)
            return false;
        if (start.type !== 'string' || end.type !== 'string')
            return false;
        if (mid.type !== 'repeat' && mid.type !== 'repeat1')
            return false;
 ...
 */
function hasOptionalPunctPrefix(rule) {
    if (rule.type !== 'seq' || rule.members.length < 2)
        return false;
    const first = rule.members[0];
    if (first.type !== 'optional')
        return false;
...
function isAllPunct(rule) {
    switch (rule.type) {
        ...
        case 'seq':
        case 'choice':
            return rule.members.every(isAllPunct);
        case 'optional':
        case 'repeat':
        case 'repeat1':
        case 'variant':
        case 'clause':
        case 'group':
            return isAllPunct(rule.content);
        default:
            return false;
    }
}
 ...
 */
export function unwrapStructuralPassthroughs(rule) {
    let r = rule;
    for (;;) {
        switch (r.type) {
            case 'optional':
 ...
 * fallback.
 */
function isExternalTerminalMember(rule, externals) {
...
function hasHiddenExternalRef(rule, externals) {
    ...
    // "all external" tally.
    const isIgnorableBoundaryExternal = (r) => {
        if (r.type !== 'optional')
            return false;
    ...
    for (const m of core.members) {
        if (isIgnorableBoundaryExternal(m))
            continue;
        hasContent = true;
 ...
 */
function hasExternalBoundaries(seqRule, externals) {
    if (seqRule.type !== 'seq')
        return false;
    if (seqRule.members.length < 2)
        return false;
...
export class AssembledPolymorph extends AssembledNodeBase {
    ...
    slotClass;
    slots;
    constructor(kind, rule, forms, opts) {
    ...
    renderTemplate(rules, wordMatcher, externals) {
        if (this.#forms.length === 0) {
            throw new Error(`AssembledPolymorph.renderTemplate: '${this.kind}' has zero synthesised forms. ` +
        ...
        const detect = {};
        const mergedClauses = {};
        const mergedJoinByField = {};
        const mergedOptionalFields = new Set();
        ...
        for (const form of this.#forms) {
            ...
            for (const name of localOptional)
                mergedOptionalFields.add(name);
            ...
            if (form.detectToken)
                detect[form.name] = form.detectToken;
        ...
        const mergedTrailingFields = new Set();
        const mergedLeadingFields = new Set();
        for (const form of this.#forms) {
        ...
        if (mergedLeadingFields.size > 0)
            meta.leadingFields = mergedLeadingFields;
        if (Object.keys(mergedJoinByField).length > 0)
            meta.joinByField = mergedJoinByField;
        if (mergedOptionalFields.size > 0)
            meta.optionalFields = mergedOptionalFields;
        return {
            template: translateToJinja(withClauses, meta),
            surface: buildRenderSurface({
...
export class AssembledToken extends AssembledLeaf {
    modelType = 'token';
    resolvedKind;
    constructor(kind, rule, opts) {
...
export class AssembledEnum extends AssembledLeaf {
    modelType = 'enum';
    resolvedKinds;
    constructor(kind, rule, opts) {
        super(kind, rule, opts);
        ...
        if (this.values.length < 2) {
            throw new Error(`AssembledEnum '${kind}' must have at least two members; normalize single-literal sets upstream to StringRule`);
        }
    }
...
export class AssembledSupertype extends AssembledNodeBase {
    ...
    #subtypes;
    constructor(kind, rule, subtypes) {
...
export class AssembledGroup extends AssembledNodeBase {
    ...
    renderTemplate(rules, wordMatcher, externals) {
        ...
        const clauses = normalizeChildrenClausePlaceholderArity(rawClauses, this.children);
        if (!template) {
            throw new Error(`AssembledGroup.renderTemplate: '${this.kind}' produced an empty template. ` +
                `Rule has no visible content — should have been classified as leaf/token.`);
        }
        const withClauses = inlineJinjaClauses(template, clauses);
        const meta = {};
 ...
 */
export function allFormFieldsOf(node) {
 ...
 */
export function allFormChildrenOf(node) {
    if (node.modelType === 'polymorph')
 ...
 */
export function allSlotsOf(node) {
    if (node.modelType === 'polymorph')
