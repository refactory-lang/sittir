 ...
 */
import { isChoice, isEnum, isField, isString, isSeq, isOptional, isRepeat, isRepeat1, isSymbol } from './rule.js';
import { parsePath } from '../dsl/transform/transform-path.js';
/**
 ...
 */
export function validateRefineForms(kind, rule, forms, rules) {
 ...
 */
export function resolveRefinePath(kind, formName, pathStr, rule, rules) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        cur = res.next;
        if (seg.kind === 'fieldName')
            fieldName = seg.name;
    }
    const final = unwrapToChoice(cur, rules);
    if (!final) {
        throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' does not resolve to a choice (got '${cur.type}')`);
    }
 ...
 * `field(name, ...)` wrapper.
 */
function stepPath(rule, seg, kind, formName, pathStr) {
    switch (seg.kind) {
        case 'fieldName': {
            const target = findFieldByName(rule, seg.name);
            if (!target) {
                throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' segment '${seg.name}:' does not match any field in rule (type '${rule.type}')`);
            }
        ...
        case 'index': {
            ...
            if (!members) {
                throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' segment '${seg.value}' cannot descend into '${rule.type}'`);
            }
            const idx = seg.value < 0 ? members.length + seg.value : seg.value;
            if (idx < 0 || idx >= members.length) {
                throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' segment '${seg.value}' out of bounds for ${rule.type} (length ${members.length})`);
            }
        ...
        case 'wildcard': {
            ...
            if (content)
                return { next: content };
            throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' wildcard cannot descend into '${rule.type}'`);
        }
        case 'kind-match':
            throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' uses kind-match '(${seg.name})' — refine paths only support positional indices and 'name:' field traversal`);
    }
}
/**
 ...
 */
function unwrapToChoice(rule, rules) {
    let cur = rule;
    ...
    for (;;) {
        ...
        if (isSymbol(cur) && rules !== undefined) {
            if (visitedSymbols.has(cur.name))
                return undefined;
 ...
 */
function findFieldByName(rule, fieldName) {
    if (isField(rule))
        return rule.name === fieldName ? rule : undefined;
    if (isSeq(rule)) {
 ...
 */
function validateSelection(kind, formName, pathStr, choice, selection) {
    const arms = choice.members;
    if (typeof selection === 'number') {
        if (selection < 0 || selection >= arms.length) {
            throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' selection index ${selection} out of range (choice has ${arms.length} branches)`);
        }
        return;
    }
    const stringValues = arms.map(unwrapToStringValue).filter((v) => v !== undefined);
    if (!stringValues.includes(selection)) {
        throw new Error(`refine(${kind}) form '${formName}': path '${pathStr}' selection '${selection}' does not match any string branch of the choice (available: ${stringValues.map((v) => `'${v}'`).join(', ') || '<none>'})`);
    }
}
/**
 ...
 * return `undefined`.
 */
function unwrapToStringValue(rule) {
    if (isString(rule))
        return rule.value;
 ...
 */
export function narrowedFieldLiteralsForForm(rule, form, rules) {
    const out = [];
    for (const [pathStr, selection] of Object.entries(form.selections)) {
        const resolution = resolveRefinePath('<emit>', form.name, pathStr, rule, rules);
        if (!resolution.fieldName)
            continue;
 ...
 * non-string branch.
 */
export function resolveSelectionLiteral(choice, selection) {
...
//# sourceMappingURL=link-refine.js.map