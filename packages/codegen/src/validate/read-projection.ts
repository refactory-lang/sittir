...
export async function validateReadProjection(grammar: string): Promise<ReadProjectionResult> {
...
export function formatReadProjectionReport(result: ReadProjectionResult): string {
	const lines: string[] = [];
	const icon = result.issues.length === 0 ? 'v' : 'x';
	lines.push(
		`  ${icon} ${result.pass}/${result.total} read projection` +
