...
import { loadRulesFromPath as loadRulesFromTemplatesPath } from './templates-path.js';
/**
 ...
 */
function jinjaBodyToLegacyRule(body) {
    const clauses = {};
    ...
    const withClauseRefs = body.replace(/\{%-?\s*if\s+([a-z_][a-z0-9_]*)(?:\s*\|\s*(?:isPresent|is_present))?\s*-?%\}([\s\S]*?)\{%-?\s*endif\s*-?%\}/g, (_m, stem, clauseBody) => {
 ...
 */
function jinjaInterpolationsToLegacy(body) {
    return body.replace(/\{\{\s*([a-z_][a-z0-9_]*)(?:\s*\|\s*(join|joinWithTrailing|joinWithLeading|joinWithFlanks|join_with_trailing|join_with_leading|join_with_flanks)\([^)]*\)|\s*\|\s*value)?\s*\}\}/g, (_m, name, joinFilter) => {
...
function loadRulesFromPath(templatesPath) {
    return loadRulesFromTemplatesPath(templatesPath, (_kind, body) => jinjaBodyToLegacyRule(body));
}
// ---------------------------------------------------------------------------
// Main
// ---------------------------------------------------------------------------
export function validateTemplateCoverage(grammar, templatesPath) {
    ...
    const hoistedOuterByKind = grammarJson ? computeHoistedOuterFields(grammarJson) : new Map();
    const issues = [];
    let total = 0;
    let pass = 0;
    for (const entry of entries) {
        ...
        if (rule === undefined)
            continue; // validate-renderable catches this.
        total++;
 ...
 */
function computeUnionPlaceholders(variants, clauseTemplates) {
    const unionPlaceholders = new Set();
    for (const { template } of variants) {
        for (const p of extractPlaceholders(template))
            unionPlaceholders.add(p);
    }
 ...
 */
function checkVariantsForLiteralLeaks(entry, variants) {
    const issues = [];
...
function checkRule(entry, rule, hoistedOuterFields) {
    const fields = entry.fields ?? {};
    const fieldNames = Object.keys(fields);
    const issues = [];
    const variants = collectTemplates(rule);
    if (variants.length === 0)
        return issues; // no inspectable template (shouldn't happen)
    ...
    const clauseKeys = ruleObj ? collectClauseKeys(ruleObj) : new Set();
    ...
    for (const fname of fieldNames) {
        if (isFieldReferenced(fname, unionPlaceholders, clauseKeys, clauseTemplates))
            continue;
        ...
        if (hoistedOuterFields.has(fname))
            continue;
 ...
 */
function collectTemplates(rule) {
    if (typeof rule === 'string')
        return [{ label: '', template: rule }];
    const out = [];
    const obj = rule;
    ...
    if (obj.variants && typeof obj.variants === 'object') {
        for (const [name, tmpl] of Object.entries(obj.variants)) {
            if (typeof tmpl === 'string')
                out.push({ label: name, template: tmpl });
        }
    }
    return out;
}
function collectClauseKeys(obj) {
    const keys = new Set();
    for (const k of Object.keys(obj)) {
        if (k === 'template' || k === 'variants' || k === 'joinBy' || k === 'joinByField' || k === 'detect')
            continue;
        const v = obj[k];
        if (typeof v === 'string')
            keys.add(k);
...
function collectClauseTemplates(obj) {
    const out = {};
    for (const k of Object.keys(obj)) {
        if (k === 'template' || k === 'variants' || k === 'joinBy' || k === 'joinByField' || k === 'detect')
            continue;
 ...
 */
function isReferencedViaClausePlaceholder(fieldName, placeholders, clauseKeys) {
 ...
 */
function isReferencedInClauseBody(fieldName, clauseTemplates) {
    for (const body of Object.values(clauseTemplates)) {
        if (extractPlaceholders(body).has(fieldName))
            return true;
    }
    return false;
}
function isFieldReferenced(fieldName, placeholders, clauseKeys, clauseTemplates) {
    ...
    if (isReferencedInClauseBody(fieldName, clauseTemplates))
        return true;
    return false;
}
// ---------------------------------------------------------------------------
 ...
 */
function computeHoistedOuterFields(grammar) {
 ...
 */
function collectHoistedOuterFields(node, rules, out, visited) {
    if (!node || typeof node !== 'object')
        return;
    if (isField(node)) {
        if (fieldHoistsViaSimplify(node.content, rules))
            out.add(node.name);
 ...
 */
function fieldHoistsViaSimplify(content, rules) {
    if (!content || typeof content !== 'object')
        return false;
    if (isField(content))
        return false; // direct field-of-field, different hoist path
    if (!hasInnerFieldAtExposableDepthGrammar(content, rules, new Set()))
        return false;
    if (hasNamedSiblingOfInnerFieldGrammar(content))
        return false;
    return true;
}
/**
 ...
 */
function hasInnerFieldAtExposableDepthGrammar(node, rules, visited) {
    if (!node || typeof node !== 'object')
        return false;
    if (isField(node))
        return true;
    ...
    if (Array.isArray(n.members)) {
        for (const m of n.members) {
            if (hasInnerFieldAtExposableDepthGrammar(m, rules, visited))
                return true;
        }
    }
    return false;
 ...
 */
function hasNamedSiblingOfInnerFieldGrammar(node) {
    if (!node || typeof node !== 'object')
        return false;
    const n = node;
 ...
 */
function isNamedReferenceGrammar(node) {
    if (!node || typeof node !== 'object')
        return false;
    if (isSymbol(node))
        return !node.name.startsWith('_');
    if (isAlias(node))
        return node.named === true;
    const n = node;
    if (n.type === 'OPTIONAL' ||
        n.type === 'REPEAT' ||
        n.type === 'REPEAT1' ||
...
function isField(n) {
    return !!n && typeof n === 'object' && n.type === 'FIELD';
}
function isSymbol(n) {
    return !!n && typeof n === 'object' && n.type === 'SYMBOL';
}
function isAlias(n) {
    return !!n && typeof n === 'object' && n.type === 'ALIAS';
}
/**
 ...
 */
function loadGrammarJson(grammar) {
