 ...
 */
export function isNodeError(err, code) {
 ...
 */
export function deriveRuleKinds(templatesPath) {
 ...
 */
export function loadRulesFromPath(templatesPath, bodyReader) {
    const dirEntries = tryReadDirEntries(templatesPath);
    if (dirEntries === null)
        return {};
    const rules = {};
 ...
 */
function tryReadDirEntries(path) {
    let stat;
    try {
        stat = statSync(path);
    }
    catch (err) {
        if (isNodeError(err, 'ENOENT') || isNodeError(err, 'ENOTDIR'))
            return null;
        throw err;
    }
    if (!stat.isDirectory())
...
//# sourceMappingURL=templates-path.js.map