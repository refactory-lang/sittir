...
## Method
0. A function's rationale is not in the source: it is its `### \`<qualified name>\`` entry in `docs/glossary/<dir>.md` (`docs/glossary/README.md`; infigraph `search` with the name, `scope: docs`). Read it before concluding a function is dead, redundant, or mis-designed.
