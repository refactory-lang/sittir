...
- **Edit ONLY** `packages/codegen/src/**`, `packages/<lang>/grammar.sittir.ts`, and — when the dispatcher's brief names them — the hand-written runtime the codegen targets: `rust/crates/sittir-core/src/**`, `packages/common/src/**`, `packages/tools/src/**`, `packages/<lang>/tests/**`, and the three hand-written crate roots `rust/crates/sittir-<lang>/src/lib.rs`. Never `packages/<lang>/overrides.ts` unless the dispatcher says so.
- **NEVER hand-edit generated artifacts** — `packages/{rust,python,typescript}/{src,.sittir}`, `packages/*/factory-map.json5`, `packages/*/overrides.suggested.ts`, `rust/crates/sittir-{rust,typescript,python}/src/render/**`, `rust/crates/sittir-*/test-fixtures.json`. A PreToolUse hook blocks them. Fix the codegen and regenerate.
- **The render path is generated Rust, not templates.** There is no askama, no `.jinja`, no template engine: `packages/codegen/src/emitters/render-body.ts` builds a body IR from the spaced render rules and `render-module.ts` prints one render function per kind into `rust/crates/sittir-<lang>/src/render/transport.rs`; `options.rs` beside it carries the site tables. The validators read `packages/<lang>/.sittir/render-bodies.json`. `createEngine()` is native-only.
...
- **Comments live in `docs/glossary/<dir>.md`, not in source** (`docs/glossary/README.md`): a declaration's rationale is its `### \`<file>::<qualified name>\`` entry there — read it before editing the declaration (infigraph `search` with the qualified name, `scope: docs`), and document what you add as a glossary entry, never as a source comment. Only directive comments (`@ts-*`, `eslint*`, `@rule-type-consts`, `/// <reference>`) belong in source.
...
1. **Type-check**: `cd packages/codegen && pnpm exec tsc --noEmit -p tsconfig.json 2>&1 | grep -c 'error TS'`. The baseline is the number the dispatcher gives you (currently **0**). Any error is yours. For a task that also edits Rust, `cd rust && cargo build --workspace && cargo test -p sittir-core` is a fast signal too; the generated crates only compile against core once they are regenerated.
...
4. **Finish with the comment gates, in this order, before reporting:**
   ...
   - Comments belong in `docs/glossary/<dir>.md`, not in source — a declaration's doc as its `###` entry, a body comment as a `####` sub-section under it. Run `pnpm exec tsx scripts/relocate-comments-to-glossary.mts <files you touched>` (dry run — lists what would move), then with `--apply`. Only directive comments (`@ts-*`, `eslint`, `@rule-type-consts`, `/// <reference>`) stay in source. The script is idempotent and sees untracked files; so does `comment-slop-check.sh --working`.
5. **Do NOT regenerate grammars or run the validator** unless the dispatcher asks — those are the dispatcher's gates (regen ×3 separately, `git diff --stat packages/*/src` empty, `validate history` exact, full `pnpm test`, tsc baseline). If asked to regenerate: `SITTIR_NATIVE_DEBUG=0 pnpm exec tsx packages/cli/src/cli.ts gen --grammar <g> --all --output packages/<g>/src`, one grammar at a time, never concurrently with a test run (regen rewrites `.sittir/` id tables mid-run and the tests then fail spuriously), and regenerate AFTER your last source edit (the pre-commit manifest check hashes the source). `pnpm exec vitest run` is always its own shell call, never chained after a regen or `validate:native`: a suite run started while `.sittir/` is being rewritten fails on exactly the tests you just added, or with a wasm language-version error, and both re-run green. Regenerate python again after any vitest run before validating (the roundtrip tests rewrite `packages/python/.sittir/grammar.js`). `validate:native` rebuilds the three `.node` binaries itself; never measure with a raw `validate counts`.
...
- The governing spec + plan for your task: the dispatcher names them — do not assume a default. The newest file under `docs/superpowers/handoffs/` is the current state of the branch; read it when the brief's context is not enough.
