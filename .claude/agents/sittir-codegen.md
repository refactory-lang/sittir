...
1. **Type-check with tsgo** (`tsc` IS tsgo here): `cd packages/codegen && pnpm exec tsc --noEmit 2>&1 | grep -c 'error TS'`. The baseline is the number the dispatcher gives you (currently **4**, all pre-existing and none in files you touch). Any other error is yours.
