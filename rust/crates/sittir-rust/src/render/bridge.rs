...
pub(crate) fn missing_required_field(node: &NodeData, name: &str) -> ::askama::Error {
    ::askama::Error::Custom(
        format!(
            "render_nodedata_into: missing required field '{}' on '{}'",
            name, node.type_
        )
        .into(),
    )
        ...
        for candidate in values {
            ...
            if let Some(span) = candidate.span {
                if span.start >= boundary
                    && candidate.text.as_deref().map_or(false, is_join_flank_token)
                {
                    return true;
                }
            }
    ...
    if let Some(children) = &node.children {
        for child in children {
            ...
            if let Some(span) = child.span {
                if span.start >= boundary
                    && child.text.as_deref().map_or(false, is_join_flank_token)
                {
                    return true;
                }
            }
...
pub(crate) fn resolve_children(
    ...
    if let Some(items) = &node.children {
        for (index, child) in items.iter().enumerate() {
            ...
            child_nodes.push((
                child.span.map_or(u32::MAX, |span| span.start),
                child_ordinal,
                child,
            ));
            child_ordinal += 1;
    ...
    if let Some(fields) = &node.fields {
        for (name, value) in fields {
            if consumed_fields
                .iter()
                .any(|consumed| consumed == &name.as_str())
            {
                continue;
            }
            match value {
    ...
    if let Some(items) = &node.children {
        if let Some(first) = first_named_idx {
            if first > 0 {
                if let Some(before) = items.get(first - 1) {
                    leading_sep =
                        !before.named && before.text.as_deref().map_or(false, is_join_flank_token);
                }
            }
        }
        if let Some(last) = last_named_idx {
            if let Some(after) = items.get(last + 1) {
                trailing_sep =
                    !after.named && after.text.as_deref().map_or(false, is_join_flank_token);
            }
        }
    }
    Ok(ResolvedField::from_items(
...
pub(crate) fn token_shaped_fallback_into(
    node: &NodeData,
    ...
    let children_all_anon = node
        .children
        .as_ref()
        .map_or(true, |children| children.iter().all(|child| !child.named));
    ...
    Err(::askama::Error::Custom(
        format!(
            "render_nodedata_into: no template for kind '{}'",
            node.type_
        )
        .into(),
    ))
...
pub fn render_nodedata_into(
    ...
    match node.type_.0 {
        322 => {
            // "_array_expression_list" | "array_expression_list"
        ...
        321 => {
            // "_array_expression_semi" | "array_expression_semi"
            ...
            let template = ArrayExpressionSemiTemplate {
                ...
                elements: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
                length: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_2.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        323 => {
            // "_closure_expression_block" | "closure_expression_block"
            ...
            let template = ClosureExpressionBlockTemplate {
                ...
                return_type: match field_1.kind {
                    ...
                    ResolvedFieldKind::Scalar | ResolvedFieldKind::List => {
        ...
        324 => {
            // "_closure_expression_expr" | "closure_expression_expr"
        ...
        381 => {
            // "_delim_token_tree_brace" | "delim_token_tree_brace"
        ...
        380 => {
            // "_delim_token_tree_bracket" | "delim_token_tree_bracket"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        379 => {
            // "_delim_token_tree_paren" | "delim_token_tree_paren"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        366 => {
            // "_expression_statement_block_ending" | "expression_statement_block_ending"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        365 => {
            // "_expression_statement_with_semi" | "expression_statement_with_semi"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        326 => {
            // "_field_pattern_named" | "field_pattern_named"
            ...
            let template = FieldPatternNamedTemplate {
                name: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                pattern: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        325 => {
            // "_field_pattern_shorthand" | "field_pattern_shorthand"
        ...
        368 => {
            // "_foreign_mod_item_body" | "foreign_mod_item_body"
            let children = resolve_children(node, &["body"])?;
            let field_0 = resolve_field(node, "body", true)?;
        ...
        328 => {
            // "_function_type_fn_form" | "function_type_fn_form"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        327 => {
            // "_function_type_trait_form" | "function_type_trait_form"
            ...
            let template = FunctionTypeTraitFormTemplate {
                trait_: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        329 => {
            // "_impl_item_body" | "impl_item_body"
            let children = resolve_children(node, &["body"])?;
            let field_0 = resolve_field(node, "body", true)?;
        ...
        372 => {
            // "_line_comment_doc" | "line_comment_doc"
            ...
            let template = LineCommentDocTemplate {
                doc: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
        ...
        333 => {
            // "_macro_definition_brace" | "macro_definition_brace"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        332 => {
            // "_macro_definition_bracket" | "macro_definition_bracket"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        331 => {
            // "_macro_definition_paren" | "macro_definition_paren"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        370 => {
            // "_match_arm_block_ending" | "match_arm_block_ending"
            ...
            let template = _MatchArmBlockEndingTemplate {
                value: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        369 => {
            // "_match_arm_with_comma" | "match_arm_with_comma"
            let children = resolve_children(node, &["value"])?;
            let field_0 = resolve_field(node, "value", true)?;
        ...
        335 => {
            // "_mod_item_inline" | "mod_item_inline"
            let children = resolve_children(node, &["body"])?;
            let field_0 = resolve_field(node, "body", true)?;
        ...
        336 => {
            // "_or_pattern_binary" | "or_pattern_binary"
            ...
            let template = OrPatternBinaryTemplate {
                ...
                right: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        337 => {
            // "_or_pattern_prefix" | "or_pattern_prefix"
        ...
        359 => {
            // "_pointer_type_mut" | "pointer_type_mut"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        341 => {
            // "_range_expression_bare" | "range_expression_bare"
            ...
            let template = _RangeExpressionBareTemplate {
                operator: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        338 => {
            // "_range_expression_binary" | "range_expression_binary"
            ...
            let template = RangeExpressionBinaryTemplate {
                end: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                operator: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
                start: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_2.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        339 => {
            // "_range_expression_postfix" | "range_expression_postfix"
        ...
        340 => {
            // "_range_expression_prefix" | "range_expression_prefix"
        ...
        343 => {
            // "_range_pattern_left_with_right" | "range_pattern_left_with_right"
            let children = resolve_children(node, &["right"])?;
            let field_0 = resolve_field(node, "right", true)?;
        ...
        342 => {
            // "_range_pattern_prefix" | "range_pattern_prefix"
            let children = resolve_children(node, &["right"])?;
            let field_0 = resolve_field(node, "right", true)?;
        ...
        362 => {
            // "_reference_expression_raw_mut" | "reference_expression_raw_mut"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        345 => {
            // "_struct_item_brace" | "struct_item_brace"
            let children = resolve_children(node, &["body"])?;
            let field_0 = resolve_field(node, "body", true)?;
            let children_renderables = children.renderable_items();
        ...
        346 => {
            // "_struct_item_tuple" | "struct_item_tuple"
            let children = resolve_children(node, &["body"])?;
            let field_0 = resolve_field(node, "body", true)?;
            let children_renderables = children.renderable_items();
        ...
        378 => {
            // "_token_tree_brace" | "token_tree_brace"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        377 => {
            // "_token_tree_bracket" | "token_tree_bracket"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        376 => {
            // "_token_tree_paren" | "token_tree_paren"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        375 => {
            // "_token_tree_pattern_brace" | "token_tree_pattern_brace"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        374 => {
            // "_token_tree_pattern_bracket" | "token_tree_pattern_bracket"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        373 => {
            // "_token_tree_pattern_paren" | "token_tree_pattern_paren"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        348 => {
            // "_visibility_modifier_crate" | "visibility_modifier_crate"
            let children = resolve_children(node, &[])?;
            let children_renderables = children.renderable_items();
        ...
        350 => {
            // "_visibility_modifier_in_path" | "visibility_modifier_in_path"
            ...
            let template = VisibilityModifierInPathTemplate {
                ...
                in_: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        349 => {
            // "_visibility_modifier_pub" | "visibility_modifier_pub"
        ...
        220 => {
            ...
            let template = ArrayTypeTemplate {
                element: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
        ...
        195 => {
            // "associated_type"
            let children =
                resolve_children(node, &["bounds", "name", "type_parameters", "where_clause"])?;
        ...
        290 => {
            ...
            let template = AsyncBlockTemplate {
                block: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
        ...
        170 => {
            ...
            let template = AttributeItemTemplate {
                attribute: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        172 => {
            ...
            let template = AttributeTemplate {
                ...
                path: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
        ...
        256 => {
            ...
            let template = CallExpressionTemplate {
                arguments: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                function: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        281 => {
            ...
            let children = resolve_children(
                node,
                &["async_marker", "move_marker", "parameters", "static_marker"],
            )?;
            ...
            let template = ClosureExpressionTemplate {
                ...
                parameters: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_2.as_scalar(),
                )),
        ...
        185 => {
            // "const_item"
            let children =
                resolve_children(node, &["name", "type", "value", "visibility_modifier"])?;
            let field_0 = resolve_field(node, "name", true)?;
        ...
        180 => {
            // "enum_variant"
            let children =
                resolve_children(node, &["body", "name", "value", "visibility_modifier"])?;
        ...
        184 => {
            ...
            let children =
                resolve_children(node, &["alias", "crate", "name", "visibility_modifier"])?;
            ...
            let template = ExternCrateDeclarationTemplate {
                ...
                crate_: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
                name: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_2.as_scalar(),
        ...
        288 => {
            ...
            let template = FieldExpressionTemplate {
                field: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                value: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
        ...
        174 => {
            ...
            let template = ForeignModItemTemplate {
                ...
                extern_modifier: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
        ...
        225 => {
            ...
            let template = GenericFunctionTemplate {
                ...
                type_arguments: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        197 => {
            ...
            let template = HigherRankedTraitBoundTemplate {
                ...
                type_parameters: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        267 => {
            ...
            let template = IfExpressionTemplate {
                ...
                condition: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
                consequence: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_2.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        286 => {
            ...
            let template = IndexExpressionTemplate {
                index: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                object: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        239 => {
            ...
            let template = MacroInvocationTemplate {
                macro_: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                token_tree: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        302 => {
            ...
            let template = MutPatternTemplate {
                ...
                mutable_specifier: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        218 => {
            ...
            let template = QualifiedTypeTemplate {
                alias: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                type_: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
        ...
        206 => {
            ...
            let template = ScopedUseListTemplate {
                list: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                path: match field_1.kind {
                    ResolvedFieldKind::Missing => OptionalNonterminalView::Missing,
        ...
        211 => {
            ...
            let children = resolve_children(
                node,
                &["lifetime", "mutable_specifier", "reference", "self"],
            )?;
            let field_0 = resolve_field(node, "lifetime", false)?;
            let field_1 = resolve_field(node, "mutable_specifier", false)?;
            ...
            let template = SelfParameterTemplate {
                ...
                self_: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_3.as_scalar(),
                )),
            };
            template.render_into(dest)
        ...
        176 => {
            // "struct_item"
            let children =
                resolve_children(node, &["name", "type_parameters", "visibility_modifier"])?;
            let field_0 = resolve_field(node, "name", true)?;
            let field_1 = resolve_field(node, "type_parameters", false)?;
            let field_2 = resolve_field(node, "visibility_modifier", false)?;
            let children_renderables = children.renderable_items();
        ...
        247 => {
            ...
            let template = UnaryExpressionTemplate {
                operand: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                operator: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
        ...
        204 => {
            ...
            let template = UseDeclarationTemplate {
                argument: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                visibility_modifier: match field_1.kind {
                    ResolvedFieldKind::Missing => OptionalNonterminalView::Missing,
        ...
        192 => {
            ...
            let template = WherePredicateTemplate {
                bounds: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_0.as_scalar(),
                )),
                left: SingleNonterminalView(::sittir_core::filters::Renderable::Text(
                    field_1.as_scalar(),
