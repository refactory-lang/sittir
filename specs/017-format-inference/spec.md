...
A maintainer running validators with `SITTIR_BACKEND=native` (the Rust Askama path) expects the same byte-equal source roundtrip US1 establishes for the TS path. The two backends produce identical output, with TS consuming `$format` directly and native restoring the same behavior through engine-owned injected/cached format state while keeping native `NodeData` format-free.
...
- **FR-005**: Both render backends — TypeScript Nunjucks (`@sittir/core`) and Rust Askama (per-grammar napi crates) — MUST produce byte-equal output for equivalent format state. TS consumes `$format` directly; native restores the same behavior through engine-owned format state associated with parse/read results and immutable engine config. A backend-parity test MUST exist that fails when the two outputs diverge.
...
- **Format Applier**: the single code path per backend (one in `@sittir/core` for the TS render; one in each native engine's render path for engine-owned format state) that replays source-derived format during render. Grammar-agnostic.
