...
- **Measurement reset from TS post-processing drop**: Cluster B surfaced that the TS-side render path was applying cosmetic post-processing (`collapse_inner_spaces` + `.trim()`) that hid walker bugs documented in `feedback_no_silent_formatting.md`. The post-processing was a Principle XI (DRY) violation in disguise — the walker emitted templates that produced stray whitespace from empty optional fields, and the post-pass collapsed it before users saw it. Native render didn't have the post-pass and exposed the walker bugs honestly. The measurement reset commit (`016/measurement-reset`) drops the post-processing on both sides of the render path so TS and native produce byte-equal raw output. Result: ~308 walker bugs surface as failing fixtures across rust + typescript + python (python is largely clean). These are NOT format-deferred items — they're template-shape failures from walker bugs. Cluster F (walker refactor) is the workstream that resolves them, lifting them from `failingKinds` straight to PASS. The corpus-validation floor tests are recalibrated downward in the same reset commit; Cluster F's 5 sub-commits each lift the floors back up incrementally.

- **Cluster F (walker refactor) — central workstream of 016**: Per `feedback_walker_refactor_blockers.md` the walker refactor is a 5-step plan: (1) freeze-template unit tests for ~15 representative kinds; (2) remove `CLAUSE_PUNCT_NAMES` hardcode; (3) walker seq case with new space model using `{% if foo is defined %}` Jinja conditionals + nested-optional fallback set; (4) handle `optional(seq(...))` properly; (5) validate against probe-kind for each frozen kind. Each step lifts pass-counts; the regression-checker passes per step because each step strictly improves counts. Cluster F is sized differently than Clusters A-E (~4-6h per the memory note for the full plan); plan to land it as 5 sub-commits, not one. Pre-existing memory notes flag two architectural blockers from the prior reverted attempt (nested-optional detection, hardcoded clause-punct); the plan addresses both.
