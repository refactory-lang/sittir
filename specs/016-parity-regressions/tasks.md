...
- [ ] T014 [US2] Verify phase-0 invariants: `pnpm -r run type-check`; `npx oxlint --deny-warnings packages/{rust,typescript,python}/src`; `npx tsx packages/tools/src/validate/jinja.ts`. Fix any regression in the same patch before committing.
...
**Goal**: Apply the documented walker refactor that lifts ~308 fixtures (at HEAD post-reset) from `failingByKind` straight to PASS across rust + typescript + python. Each sub-commit lifts counts incrementally; the regression-checker passes per step because counts only move up.
...
- [ ] T036 [US-walker] Step 4: **Handle `optional(seq(...))`.** When the optional wraps a multi-member seq, the whole enclosing emission needs to be conditional on present-detection (the canonical-field choice — usually the most-required field within the wrapped seq) using `{% if FIELD | isPresent %}...{% endif %}` with the enclosing separator (space, `else`, etc.) placed INSIDE the conditional, per the research artifact's Scenario C pattern. Spec the rule in code comments. Update freeze-tests for affected kinds (likely rust `impl_item` family — the original blocker). Commit titled `016/walker-refactor-3: optional-seq emission as enclosing isPresent conditional`.
- [ ] T037 [US-walker] Step 5: **Validate against probe-kind for each frozen kind.** For all ~15 frozen kinds, run `probe-kind --grammar X --kind Y --reparse --pretty`. Verify byte-equal source roundtrip (where the corpus source is canonical-format). Verify `compareEngines.summary` reads "TS and native engines agree on render output". Re-collect baselines for both backends. May bundle with T036 (or T035) — see cluster-header revised budget.
