...
## Phase 4: Cluster F — walker refactor (was: US3 native-only)

**Reframe (post-Cluster B)**: The original US3 ("lift native floor above zero") is structurally closed by the measurement-reset commit (`016/measurement-reset`). Once TS-side post-processing was dropped, native and TS produce byte-equal output: their `failingByKind` maps now match recursively. The remaining work is NOT native-specific; it's the shared walker bugs that surface on BOTH backends now that they're honest. Phase 4 is therefore reframed as **Cluster F (walker refactor)** per `feedback_walker_refactor_blockers.md`.

**Goal**: Apply the documented 5-step walker refactor that lifts ~308 fixtures (at HEAD post-reset) from `failingByKind` straight to PASS across rust + typescript + python. Each of the 5 steps lifts counts incrementally; the regression-checker passes per step because counts only move up.

**Independent Test**: After all 5 sub-commits land, re-run `pnpm test` and verify:
...
### Cluster F — walker refactor (5 sub-commits per `feedback_walker_refactor_blockers.md`)

- [ ] T033 [US-walker] Step 1: **Freeze-template unit tests.** Add `packages/codegen/src/__tests__/template-walker-frozen.test.ts` (or extend the existing test file) that captures the current template emission output for ~15 representative kinds across rust/typescript/python (the highest-magnitude entries in the post-reset `failingByKind`: rust `function_item`, `block`, `let_declaration`, `type_item`, `trait_item`; ts `class_declaration`, `type_alias_declaration`, `statement_block`, `interface_declaration`, `function_type`; python `dictionary`). The tests freeze CURRENT broken output; subsequent steps update the frozen expectations as they fix the bugs. Without this freeze, refactoring has no regression detection. Commit titled `016/walker-refactor-1: freeze representative templates`.
- [ ] T034 [US-walker] Step 2: **Remove `CLAUSE_PUNCT_NAMES` hardcode.** `packages/codegen/src/compiler/template-walker.ts:620` (approximate) has `const CLAUSE_PUNCT_NAMES: Record<string, string> = { '!': 'bang', '?': 'question' }`. Replace with a derivation via the existing `tokenToName(punctString)` helper. No allowlist; any optional-punct clause derives its name from the punct text. Update freeze-tests if any kinds' clause names change (likely none since `tokenToName` produces the same outputs for `!` and `?` today). Commit titled `016/walker-refactor-2: derive clause names via tokenToName`.
- [ ] T035 [US-walker] Step 3: **Walker seq case — new space model.** Per the memory note's plan: per-member, detect optional-field locally OR via the `optionalFields` set computed from `AssembledField.fields` (handles nested `optional(seq(field(...)))`). Emit `{% if foo is defined %} {{ foo }}{% endif %}` for optional fields with leading space INSIDE the conditional. Required word-like fields keep unconditional leading space. Update freeze-tests to reflect the new template shapes (each frozen kind's expected output changes from broken to correct). Commit titled `016/walker-refactor-3: jinja-conditional spacing for optional fields`.
- [ ] T036 [US-walker] Step 4: **Handle `optional(seq(...))`.** When the optional wraps a multi-member seq, the whole enclosing emission needs to be conditional on something present-detection (the canonical-field choice — usually the most-required field within the wrapped seq). Spec the rule in code comments. Update freeze-tests for affected kinds (likely rust `impl_item` family — the original blocker). Commit titled `016/walker-refactor-4: optional-seq emission as enclosing conditional`.
- [ ] T037 [US-walker] Step 5: **Validate against probe-kind for each frozen kind.** For all ~15 frozen kinds, run `probe-kind --grammar X --kind Y --reparse --pretty`. Verify byte-equal source roundtrip (where the corpus source is canonical-format). Verify `compareEngines.summary` reads "TS and native engines agree on render output". Re-collect baselines for both backends. Update freeze-tests one final time. Commit titled `016/walker-refactor-5: validate via probe-kind, recollect baselines`.
- [ ] T037a [US-walker] After step 5, run the full `pnpm test` and confirm parity-fixture pass-counts are at-or-above HEAD's pre-reset numbers per grammar. If any kind still fails, classify it: (a) genuine format-equivalence (source uses non-canonical format the grammar treats as equivalent — quote style, optional separator presence) → moves to `formatDeferredByKind` for feature 017; (b) walker bug not covered by the 5 frozen-kinds set → add to a Cluster F follow-up sub-commit covering the additional kind.
- [ ] T038 [US-walker] Update memory notes: DELETE `feedback_no_silent_formatting.md` and `feedback_walker_refactor_blockers.md` (cluster fully closed). Update `feedback_use_probe_kind.md` if the freeze-tests pattern is worth promoting. Per FR-008.
...
- [ ] T041 Run the full test suite under both backends one final time: `SITTIR_BACKEND=typescript pnpm test` (expect 0 failures), `SITTIR_BACKEND=native pnpm test` (expect identical results — both backends now byte-equal after Cluster B's measurement reset). Record final pass counts. Confirm `formatDeferredKinds` and `formatDeferredByKind` arrays are EMPTY across the entire `baselines/{ts,native}.json` (per the sharpened SC-001).
