...
- [ ] T028b [P] [US3] Convert typescript polymorphs batch 2 (5 rules) — deferred. Remaining candidates each fail a different way: `parenthesized_expression` triggers unresolvable LR conflict with `sequence_expression` (needs precedence tuning, not conflicts declaration); `export_statement` / `call_expression` have duplicate variant-name suggestions from tagVariants (`export`/`export`/`export`, `function`/`function`) that `registerPolymorphVariant`'s uniqueness guard rejects — suggested emitter needs auto-disambiguation; `arrow_function`'s `_call_signature` starts with `_` (hidden rule name) and would collide with the synthesized `_arrow_function__call_signature` path
