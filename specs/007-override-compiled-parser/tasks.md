...
- [x] T028a [P] [US3] Convert typescript polymorphs batch 1 (4 rules) — adopted copy-paste from overrides.suggested.ts: `class_heritage`, `import_clause`, `import_specifier`, `index_signature`. All compile clean; readNode kind count +7, factory pass +6
- [x] T028b [P] [US3] Convert typescript polymorphs batch 2 — partial (1/5). Suggested-emitter now disambiguates duplicate variant names (`export` → `export`/`export2`/`export3`), so `registerPolymorphVariant`'s uniqueness guard passes. `arrow_function` adopted cleanly (+2 kinds); the `_call_signature` hidden-rule-name concern turned out non-issue (variant strips leading `_` in the synthesized name). Remaining 3 held with documented reasons:
  - `export_statement`: LR conflict between `export` keyword and `primary_expression` at `export *` / `export {...}` — needs precedence tuning
  - `call_expression`: LR conflict between `_call_expression_function` and `type_arguments` on `typeof expr <` — needs precedence tuning
  - `parenthesized_expression`: LR conflict with `sequence_expression` inside `( … )` — needs precedence tuning
