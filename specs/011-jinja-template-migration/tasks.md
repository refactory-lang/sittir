...
- [x] T023 [US1] Retire `AssembledPolymorph.renderTemplate()` and related `renderParts()` / `renderTemplate()` class methods in `packages/codegen/src/compiler/node-map.ts`. Delete them. Keep `isTextTemplate` and any metadata helpers the translator needs.
