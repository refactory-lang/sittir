// Auto-generated by ir-codegen — do not edit

/** All branch (non-leaf) node kind strings. */
export const NODE_KINDS = [
	"abstract_class_declaration",
	"abstract_method_signature",
	"adding_type_annotation",
	"ambient_declaration",
	"arguments",
	"array",
	"array_pattern",
	"array_type",
	"arrow_function",
	"as_expression",
	"asserts",
	"asserts_annotation",
	"assignment_expression",
	"assignment_pattern",
	"augmented_assignment_expression",
	"await_expression",
	"binary_expression",
	"break_statement",
	"call_expression",
	"call_signature",
	"catch_clause",
	"class",
	"class_body",
	"class_declaration",
	"class_heritage",
	"class_static_block",
	"computed_property_name",
	"conditional_type",
	"constraint",
	"construct_signature",
	"constructor_type",
	"continue_statement",
	"decorator",
	"default_type",
	"do_statement",
	"else_clause",
	"enum_assignment",
	"enum_body",
	"enum_declaration",
	"export_clause",
	"export_specifier",
	"export_statement",
	"expression_statement",
	"extends_clause",
	"extends_type_clause",
	"finally_clause",
	"flow_maybe_type",
	"for_in_statement",
	"for_statement",
	"formal_parameters",
	"function_declaration",
	"function_expression",
	"function_signature",
	"function_type",
	"generator_function",
	"generator_function_declaration",
	"generic_type",
	"if_statement",
	"implements_clause",
	"import_alias",
	"import_attribute",
	"import_clause",
	"import_require_clause",
	"import_specifier",
	"import_statement",
	"index_signature",
	"index_type_query",
	"infer_type",
	"instantiation_expression",
	"interface_body",
	"interface_declaration",
	"internal_module",
	"intersection_type",
	"labeled_statement",
	"lexical_declaration",
	"literal_type",
	"lookup_type",
	"mapped_type_clause",
	"member_expression",
	"method_definition",
	"method_signature",
	"module",
	"named_imports",
	"namespace_export",
	"namespace_import",
	"nested_identifier",
	"nested_type_identifier",
	"new_expression",
	"non_null_expression",
	"object",
	"object_assignment_pattern",
	"object_pattern",
	"object_type",
	"omitting_type_annotation",
	"opting_type_annotation",
	"optional_parameter",
	"optional_type",
	"pair",
	"pair_pattern",
	"parenthesized_expression",
	"parenthesized_type",
	"program",
	"property_signature",
	"public_field_definition",
	"readonly_type",
	"regex",
	"required_parameter",
	"rest_pattern",
	"rest_type",
	"return_statement",
	"satisfies_expression",
	"sequence_expression",
	"spread_element",
	"statement_block",
	"string",
	"subscript_expression",
	"switch_body",
	"switch_case",
	"switch_default",
	"switch_statement",
	"template_literal_type",
	"template_string",
	"template_substitution",
	"template_type",
	"ternary_expression",
	"throw_statement",
	"try_statement",
	"tuple_type",
	"type_alias_declaration",
	"type_annotation",
	"type_arguments",
	"type_assertion",
	"type_parameter",
	"type_parameters",
	"type_predicate",
	"type_predicate_annotation",
	"type_query",
	"unary_expression",
	"union_type",
	"update_expression",
	"variable_declaration",
	"variable_declarator",
	"while_statement",
	"with_statement",
	"yield_expression",
] as const;

/** All leaf/terminal node kind strings. */
export const LEAF_KINDS = [
	"accessibility_modifier",
	"comment",
	"debugger_statement",
	"empty_statement",
	"escape_sequence",
	"existential_type",
	"false",
	"hash_bang_line",
	"html_comment",
	"identifier",
	"import",
	"meta_property",
	"null",
	"number",
	"optional_chain",
	"override_modifier",
	"predefined_type",
	"private_property_identifier",
	"property_identifier",
	"regex_flags",
	"regex_pattern",
	"shorthand_property_identifier",
	"shorthand_property_identifier_pattern",
	"statement_identifier",
	"string_fragment",
	"super",
	"this",
	"this_type",
	"true",
	"type_identifier",
	"undefined",
] as const;

/** All node kind strings (branch + leaf). */
export const ALL_KINDS = [...NODE_KINDS, ...LEAF_KINDS] as const;

/** Language keywords (alphabetic anonymous tokens). */
export const KEYWORDS = [
	"abstract",
	"accessor",
	"any",
	"as",
	"assert",
	"async",
	"await",
	"boolean",
	"break",
	"case",
	"catch",
	"const",
	"continue",
	"debugger",
	"declare",
	"default",
	"delete",
	"do",
	"else",
	"enum",
	"export",
	"extends",
	"finally",
	"for",
	"from",
	"function",
	"get",
	"global",
	"if",
	"implements",
	"in",
	"infer",
	"instanceof",
	"interface",
	"is",
	"keyof",
	"let",
	"meta",
	"namespace",
	"never",
	"new",
	"of",
	"override",
	"private",
	"protected",
	"public",
	"readonly",
	"require",
	"return",
	"satisfies",
	"set",
	"static",
	"switch",
	"symbol",
	"target",
	"throw",
	"try",
	"typeof",
	"unknown",
	"using",
	"var",
	"void",
	"while",
	"with",
	"yield",
] as const;

/** Operator/punctuation tokens. */
export const OPERATORS = [
	"!",
	"!=",
	"!==",
	"${",
	"%",
	"%=",
	"&",
	"&&",
	"&&=",
	"&=",
	"(",
	")",
	"*",
	"**",
	"**=",
	"*=",
	"+",
	"++",
	"+=",
	"+?:",
	",",
	"-",
	"--",
	"-=",
	"-?:",
	".",
	"...",
	"/",
	"/=",
	":",
	";",
	"<",
	"<<",
	"<<=",
	"<=",
	"=",
	"==",
	"===",
	"=>",
	">",
	">=",
	">>",
	">>=",
	">>>",
	">>>=",
	"?",
	"?.",
	"?:",
	"??",
	"??=",
	"@",
	"[",
	"]",
	"^",
	"^=",
	"`",
	"unique symbol",
	"{",
	"{|",
	"|",
	"|=",
	"||",
	"||=",
	"|}",
	"}",
	"~",
] as const;

export type NodeKind = (typeof NODE_KINDS)[number];
export type LeafKind = (typeof LEAF_KINDS)[number];
export type AnyKind = (typeof ALL_KINDS)[number];
export type Keyword = (typeof KEYWORDS)[number];
export type Operator = (typeof OPERATORS)[number];

/** Per-node-kind field metadata. */
export const FIELD_MAP: Record<
	NodeKind,
	ReadonlyArray<{
		name: string;
		required: boolean;
		multiple: boolean;
	}>
> = {
	abstract_class_declaration: [
		{ name: "body", required: true, multiple: false },
		{ name: "decorator", required: false, multiple: true },
		{ name: "name", required: true, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "classHeritage", required: false, multiple: false },
	],
	abstract_method_signature: [
		{ name: "name", required: true, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "accessibilityModifier", required: false, multiple: false },
		{ name: "overrideModifier", required: false, multiple: false },
	],
	adding_type_annotation: [],
	ambient_declaration: [
		{ name: "declaration", required: true, multiple: false },
		{ name: "typeAnnotation", required: false, multiple: false },
		{ name: "semicolon", required: false, multiple: false },
	],
	arguments: [],
	array: [],
	array_pattern: [],
	array_type: [{ name: "primaryType", required: true, multiple: false }],
	arrow_function: [
		{ name: "body", required: true, multiple: false },
		{ name: "parameter", required: false, multiple: false },
		{ name: "parameters", required: false, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	as_expression: [
		{ name: "expression", required: true, multiple: false },
		{ name: "typeAnnotation", required: false, multiple: false },
	],
	asserts: [],
	asserts_annotation: [{ name: "asserts", required: true, multiple: false }],
	assignment_expression: [
		{ name: "left", required: true, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	assignment_pattern: [
		{ name: "left", required: true, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	augmented_assignment_expression: [
		{ name: "left", required: true, multiple: false },
		{ name: "operator", required: true, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	await_expression: [{ name: "expression", required: true, multiple: false }],
	binary_expression: [
		{ name: "left", required: true, multiple: false },
		{ name: "operator", required: true, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	break_statement: [{ name: "label", required: false, multiple: false }],
	call_expression: [
		{ name: "arguments", required: true, multiple: false },
		{ name: "function", required: true, multiple: false },
		{ name: "typeArguments", required: false, multiple: false },
	],
	call_signature: [
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	catch_clause: [
		{ name: "body", required: true, multiple: false },
		{ name: "parameter", required: false, multiple: false },
		{ name: "type", required: false, multiple: false },
	],
	class: [
		{ name: "body", required: true, multiple: false },
		{ name: "decorator", required: false, multiple: true },
		{ name: "name", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "classHeritage", required: false, multiple: false },
	],
	class_body: [{ name: "decorator", required: false, multiple: true }],
	class_declaration: [
		{ name: "body", required: true, multiple: false },
		{ name: "decorator", required: false, multiple: true },
		{ name: "name", required: true, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "classHeritage", required: false, multiple: false },
		{ name: "automaticSemicolon", required: false, multiple: false },
	],
	class_heritage: [
		{ name: "extendsClause", required: true, multiple: false },
		{ name: "implementsClause", required: false, multiple: false },
	],
	class_static_block: [{ name: "body", required: true, multiple: false }],
	computed_property_name: [{ name: "expression", required: true, multiple: false }],
	conditional_type: [
		{ name: "alternative", required: true, multiple: false },
		{ name: "consequence", required: true, multiple: false },
		{ name: "left", required: true, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	constraint: [],
	construct_signature: [
		{ name: "parameters", required: true, multiple: false },
		{ name: "type", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	constructor_type: [
		{ name: "parameters", required: true, multiple: false },
		{ name: "type", required: true, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	continue_statement: [{ name: "label", required: false, multiple: false }],
	decorator: [],
	default_type: [],
	do_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "condition", required: true, multiple: false },
	],
	else_clause: [{ name: "statement", required: true, multiple: false }],
	enum_assignment: [
		{ name: "name", required: true, multiple: false },
		{ name: "value", required: true, multiple: false },
	],
	enum_body: [
		{ name: "name", required: false, multiple: true },
		{ name: "opening", required: false, multiple: false },
		{ name: "members", required: false, multiple: true },
	],
	enum_declaration: [
		{ name: "body", required: true, multiple: false },
		{ name: "name", required: true, multiple: false },
	],
	export_clause: [],
	export_specifier: [
		{ name: "alias", required: false, multiple: false },
		{ name: "name", required: true, multiple: false },
	],
	export_statement: [
		{ name: "declaration", required: false, multiple: false },
		{ name: "decorator", required: false, multiple: true },
		{ name: "source", required: false, multiple: false },
		{ name: "value", required: false, multiple: false },
	],
	expression_statement: [],
	extends_clause: [
		{ name: "typeArguments", required: false, multiple: true },
		{ name: "value", required: true, multiple: true },
	],
	extends_type_clause: [{ name: "type", required: true, multiple: true }],
	finally_clause: [{ name: "body", required: true, multiple: false }],
	flow_maybe_type: [{ name: "primaryType", required: true, multiple: false }],
	for_in_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "kind", required: false, multiple: false },
		{ name: "left", required: true, multiple: false },
		{ name: "operator", required: true, multiple: false },
		{ name: "right", required: true, multiple: false },
		{ name: "value", required: false, multiple: false },
	],
	for_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "condition", required: true, multiple: true },
		{ name: "increment", required: false, multiple: false },
		{ name: "initializer", required: true, multiple: false },
	],
	formal_parameters: [],
	function_declaration: [
		{ name: "body", required: true, multiple: false },
		{ name: "name", required: true, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	function_expression: [
		{ name: "body", required: true, multiple: false },
		{ name: "name", required: false, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	function_signature: [
		{ name: "name", required: true, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	function_type: [
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: true, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	generator_function: [
		{ name: "body", required: true, multiple: false },
		{ name: "name", required: false, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	generator_function_declaration: [
		{ name: "body", required: true, multiple: false },
		{ name: "name", required: true, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
	],
	generic_type: [
		{ name: "name", required: true, multiple: false },
		{ name: "typeArguments", required: true, multiple: false },
	],
	if_statement: [
		{ name: "alternative", required: false, multiple: false },
		{ name: "condition", required: true, multiple: false },
		{ name: "consequence", required: true, multiple: false },
	],
	implements_clause: [],
	import_alias: [
		{ name: "name", required: true, multiple: false },
		{ name: "value", required: true, multiple: false },
		{ name: "semicolon", required: true, multiple: false },
	],
	import_attribute: [{ name: "object", required: true, multiple: false }],
	import_clause: [
		{ name: "defaultImport", required: true, multiple: false },
		{ name: "namedImports", required: false, multiple: false },
	],
	import_require_clause: [
		{ name: "source", required: true, multiple: false },
		{ name: "identifier", required: true, multiple: false },
	],
	import_specifier: [
		{ name: "alias", required: false, multiple: false },
		{ name: "name", required: true, multiple: false },
	],
	import_statement: [
		{ name: "source", required: false, multiple: false },
		{ name: "importClause", required: false, multiple: false },
		{ name: "fromClause", required: false, multiple: false },
		{ name: "importAttribute", required: false, multiple: false },
		{ name: "semicolon", required: true, multiple: false },
	],
	index_signature: [
		{ name: "indexType", required: false, multiple: false },
		{ name: "name", required: false, multiple: false },
		{ name: "sign", required: false, multiple: false },
		{ name: "type", required: true, multiple: false },
		{ name: "mappedTypeClause", required: false, multiple: false },
	],
	index_type_query: [{ name: "primaryType", required: true, multiple: false }],
	infer_type: [
		{ name: "typeIdentifier", required: true, multiple: false },
		{ name: "constraint", required: false, multiple: false },
	],
	instantiation_expression: [
		{ name: "function", required: false, multiple: false },
		{ name: "typeArguments", required: true, multiple: false },
		{ name: "expression", required: true, multiple: false },
	],
	interface_body: [],
	interface_declaration: [
		{ name: "body", required: true, multiple: false },
		{ name: "name", required: true, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "extendsTypeClause", required: false, multiple: false },
	],
	internal_module: [
		{ name: "body", required: false, multiple: false },
		{ name: "name", required: true, multiple: false },
	],
	intersection_type: [
		{ name: "left", required: false, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	labeled_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "label", required: true, multiple: false },
	],
	lexical_declaration: [
		{ name: "kind", required: true, multiple: false },
		{ name: "declarators", required: true, multiple: true },
		{ name: "semicolon", required: true, multiple: false },
	],
	literal_type: [],
	lookup_type: [
		{ name: "primaryType", required: true, multiple: false },
		{ name: "indexType", required: true, multiple: false },
	],
	mapped_type_clause: [
		{ name: "alias", required: false, multiple: false },
		{ name: "name", required: true, multiple: false },
		{ name: "type", required: true, multiple: false },
	],
	member_expression: [
		{ name: "object", required: true, multiple: false },
		{ name: "optionalChain", required: false, multiple: false },
		{ name: "property", required: true, multiple: false },
	],
	method_definition: [
		{ name: "body", required: true, multiple: false },
		{ name: "name", required: true, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "accessibilityModifier", required: false, multiple: false },
		{ name: "overrideModifier", required: false, multiple: false },
	],
	method_signature: [
		{ name: "name", required: true, multiple: false },
		{ name: "parameters", required: true, multiple: false },
		{ name: "returnType", required: false, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "accessibilityModifier", required: false, multiple: false },
		{ name: "overrideModifier", required: false, multiple: false },
	],
	module: [
		{ name: "body", required: false, multiple: false },
		{ name: "name", required: true, multiple: false },
	],
	named_imports: [],
	namespace_export: [],
	namespace_import: [{ name: "identifier", required: true, multiple: false }],
	nested_identifier: [
		{ name: "object", required: true, multiple: false },
		{ name: "property", required: true, multiple: false },
	],
	nested_type_identifier: [
		{ name: "module", required: true, multiple: false },
		{ name: "name", required: true, multiple: false },
	],
	new_expression: [
		{ name: "arguments", required: false, multiple: false },
		{ name: "constructor", required: true, multiple: false },
		{ name: "typeArguments", required: false, multiple: false },
	],
	non_null_expression: [{ name: "expression", required: true, multiple: false }],
	object: [],
	object_assignment_pattern: [
		{ name: "left", required: true, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	object_pattern: [],
	object_type: [
		{ name: "opening", required: false, multiple: false },
		{ name: "members", required: false, multiple: true },
		{ name: "closing", required: false, multiple: false },
	],
	omitting_type_annotation: [],
	opting_type_annotation: [],
	optional_parameter: [
		{ name: "decorator", required: false, multiple: true },
		{ name: "name", required: false, multiple: false },
		{ name: "pattern", required: false, multiple: false },
		{ name: "type", required: false, multiple: false },
		{ name: "value", required: false, multiple: false },
		{ name: "parameterName", required: true, multiple: false },
		{ name: "initializer", required: false, multiple: false },
	],
	optional_type: [],
	pair: [
		{ name: "key", required: true, multiple: false },
		{ name: "value", required: true, multiple: false },
	],
	pair_pattern: [
		{ name: "key", required: true, multiple: false },
		{ name: "value", required: true, multiple: false },
	],
	parenthesized_expression: [{ name: "type", required: false, multiple: false }],
	parenthesized_type: [],
	program: [
		{ name: "hashBangLine", required: false, multiple: false },
		{ name: "statements", required: false, multiple: true },
	],
	property_signature: [
		{ name: "name", required: true, multiple: false },
		{ name: "type", required: false, multiple: false },
		{ name: "accessibilityModifier", required: false, multiple: false },
		{ name: "overrideModifier", required: false, multiple: false },
	],
	public_field_definition: [
		{ name: "decorator", required: false, multiple: true },
		{ name: "name", required: true, multiple: false },
		{ name: "type", required: false, multiple: false },
		{ name: "value", required: false, multiple: false },
		{ name: "accessibilityModifier", required: false, multiple: false },
		{ name: "overrideModifier", required: false, multiple: false },
		{ name: "initializer", required: false, multiple: false },
	],
	readonly_type: [],
	regex: [
		{ name: "flags", required: false, multiple: false },
		{ name: "pattern", required: true, multiple: false },
	],
	required_parameter: [
		{ name: "decorator", required: false, multiple: true },
		{ name: "name", required: false, multiple: false },
		{ name: "pattern", required: false, multiple: false },
		{ name: "type", required: false, multiple: false },
		{ name: "value", required: false, multiple: false },
		{ name: "parameterName", required: true, multiple: false },
		{ name: "initializer", required: false, multiple: false },
	],
	rest_pattern: [],
	rest_type: [],
	return_statement: [],
	satisfies_expression: [
		{ name: "expression", required: true, multiple: false },
		{ name: "typeAnnotation", required: true, multiple: false },
	],
	sequence_expression: [],
	spread_element: [{ name: "expression", required: true, multiple: false }],
	statement_block: [
		{ name: "statements", required: false, multiple: true },
		{ name: "automaticSemicolon", required: false, multiple: false },
	],
	string: [],
	subscript_expression: [
		{ name: "index", required: true, multiple: false },
		{ name: "object", required: true, multiple: false },
		{ name: "optionalChain", required: false, multiple: false },
	],
	switch_body: [],
	switch_case: [
		{ name: "body", required: false, multiple: true },
		{ name: "value", required: true, multiple: false },
	],
	switch_default: [{ name: "body", required: false, multiple: true }],
	switch_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "value", required: true, multiple: false },
	],
	template_literal_type: [],
	template_string: [],
	template_substitution: [],
	template_type: [],
	ternary_expression: [
		{ name: "alternative", required: true, multiple: false },
		{ name: "condition", required: true, multiple: false },
		{ name: "consequence", required: true, multiple: false },
	],
	throw_statement: [],
	try_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "finalizer", required: false, multiple: false },
		{ name: "handler", required: false, multiple: false },
	],
	tuple_type: [],
	type_alias_declaration: [
		{ name: "name", required: true, multiple: false },
		{ name: "typeParameters", required: false, multiple: false },
		{ name: "value", required: true, multiple: false },
	],
	type_annotation: [],
	type_arguments: [],
	type_assertion: [
		{ name: "typeArguments", required: true, multiple: false },
		{ name: "expression", required: true, multiple: false },
	],
	type_parameter: [
		{ name: "constraint", required: false, multiple: false },
		{ name: "name", required: true, multiple: false },
		{ name: "value", required: false, multiple: false },
	],
	type_parameters: [],
	type_predicate: [
		{ name: "name", required: true, multiple: false },
		{ name: "type", required: true, multiple: false },
	],
	type_predicate_annotation: [{ name: "typePredicate", required: true, multiple: false }],
	type_query: [],
	unary_expression: [
		{ name: "argument", required: true, multiple: false },
		{ name: "operator", required: true, multiple: false },
	],
	union_type: [
		{ name: "left", required: false, multiple: false },
		{ name: "right", required: true, multiple: false },
	],
	update_expression: [
		{ name: "argument", required: true, multiple: false },
		{ name: "operator", required: true, multiple: false },
	],
	variable_declaration: [
		{ name: "declarators", required: true, multiple: true },
		{ name: "semicolon", required: true, multiple: false },
	],
	variable_declarator: [
		{ name: "name", required: true, multiple: false },
		{ name: "type", required: false, multiple: false },
		{ name: "value", required: false, multiple: false },
	],
	while_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "condition", required: true, multiple: false },
	],
	with_statement: [
		{ name: "body", required: true, multiple: false },
		{ name: "object", required: true, multiple: false },
	],
	yield_expression: [{ name: "expression", required: false, multiple: false }],
};

/** Valid values for `accessibility_modifier` nodes. */
export const ACCESSIBILITY_MODIFIERS = ["private", "protected", "public"] as const;
export type AccessibilityModifierValue = (typeof ACCESSIBILITY_MODIFIERS)[number];

/** Valid values for `predefined_type` nodes. */
export const PREDEFINED_TYPES = [
	"any",
	"boolean",
	"never",
	"number",
	"object",
	"string",
	"string",
	"symbol",
	"unknown",
	"void",
] as const;
export type PredefinedTypeValue = (typeof PREDEFINED_TYPES)[number];
