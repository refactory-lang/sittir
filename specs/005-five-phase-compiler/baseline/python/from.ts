...
function _resolveSimpleStatement(v: unknown) {
	...
	if (isNodeData(v)) return v;
	if (typeof v === "string") {
		const leaf = _resolveLeafString(v, ["break_statement", "continue_statement", "pass_statement"]);
...
function _resolveParameter(v: unknown) {
	...
	if (isNodeData(v)) return v;
	if (typeof v === "string") {
		const leaf = _resolveLeafString(v, ["identifier", "keyword_separator", "positional_separator"]);
		if (leaf !== undefined) return leaf;
	}
	if (Array.isArray(v))
		throw new Error("Array value with ambiguous branch types — use {kind} to disambiguate");
...
function _resolveSimplePattern(v: unknown) {
	...
	if (isNodeData(v)) return v;
	if (typeof v === "string") {
		const leaf = _resolveLeafString(v, ["false", "float", "integer", "none", "true"]);
		if (leaf !== undefined) return leaf;
	}
	if (Array.isArray(v))
		throw new Error("Array value with ambiguous branch types — use {kind} to disambiguate");
...
function _resolveComprehensionClauses9(v: unknown) {
	...
	if (isNodeData(v)) return v;
	if (typeof v === "string") {
		if (["!=", "<", "<=", "<>", "==", ">", ">=", "in", "is", "is not", "not in"].includes(v))
			return { type: v, text: v };
		const leaf = _resolveLeafString(v, []);
		if (leaf !== undefined) return leaf;
		throw new Error("Cannot resolve string value: no leaf types accepted for this field");
...
function _resolveRightHandSide3(
	v: unknown,
): KindMap["assignment"] | KindMap["augmented_assignment"] | KindMap["yield"] | Expression;
...
function _resolveFExpression2(
	v: unknown,
): KindMap["expression_list"] | KindMap["pattern_list"] | KindMap["yield"] | Expression;
...
function _rfpc4ja(v: unknown) {
	...
	if (isNodeData(v)) return v;
	if (typeof v === "string") {
		const leaf = _resolveLeafString(v, ["false", "float", "identifier", "integer", "none", "true"]);
		if (leaf !== undefined) return leaf;
	}
	if (Array.isArray(v))
		throw new Error("Array value with ambiguous branch types — use {kind} to disambiguate");
...
function _rr83r9g(
	v: unknown,
): KindMap["list_splat"] | KindMap["parenthesized_list_splat"] | KindMap["yield"] | Expression;
...
function _resolvePattern4(
	v: unknown,
): KindMap["identifier"] | KindMap["attribute"] | KindMap["subscript"] | Expression;
...
function _resolveFExpression4(
	v: unknown,
): KindMap["list_splat"] | KindMap["parenthesized_expression"] | KindMap["yield"] | Expression;
...
function _r1qu323z(v: unknown) {
	...
	if (isNodeData(v)) return v;
	if (typeof v === "string") {
		const leaf = _resolveLeafString(v, ["escape_interpolation", "escape_sequence"]);
		if (leaf !== undefined) return leaf;
	}
	if (typeof v === "object" && v !== null) {
		if (hasKind(v)) {
...
function _resolveParameter2(
	v: unknown,
): KindMap["identifier"] | KindMap["dictionary_splat_pattern"] | KindMap["list_splat_pattern"];
	...
	return block_({
		alternative:
			obj.alternative !== undefined
				? (Array.isArray(obj.alternative) ? obj.alternative : [obj.alternative]).map(_r1ednr96)
				: undefined,
		children:
...
export function caseClauseFrom(input: RuntimeNodeOf<CaseClause> | CaseClauseFromInput) {
	...
	return case_clause_({
		...
		children:
			obj.children !== undefined
				? (Array.isArray(obj.children) ? obj.children : [obj.children]).map(_rflltaz)
				: [],
	});
}
	...
	return class_pattern_({
		...
		arguments:
			obj.arguments !== undefined
				? (Array.isArray(obj.arguments) ? obj.arguments : [obj.arguments]).map(_rflltaz)
				: undefined,
	});
}
	...
	return comparison_operator_({
		...
		comparators:
			obj.comparators !== undefined
				? (Array.isArray(obj.comparators) ? obj.comparators : [obj.comparators]).map(
						_resolveComprehensionClauses4,
					)
				: [],
	});
}
...
export function decoratedDefinitionFrom(
	...
	return decorated_definition_({
		...
		children:
			obj.children !== undefined
				? (Array.isArray(obj.children) ? obj.children : [obj.children]).map(_r1fl8679)
				: [],
	});
}
...
export function dictionaryComprehensionFrom(
	input: RuntimeNodeOf<DictionaryComprehension> | DictionaryComprehensionFromInput,
) {
...
export function exceptClauseFrom(input: RuntimeNodeOf<ExceptClause> | ExceptClauseFromInput) {
	...
	return except_clause_({
		...
		value:
			obj.value !== undefined
				? (Array.isArray(obj.value) ? obj.value : [obj.value]).map(_resolveComprehensionClauses3)
				: undefined,
...
export function expressionStatementFrom(
	...
	if (args.length === 1 && isNodeData(args[0]) && args[0].type === "expression_statement") {
		return expression_statement_(
			...((args[0].children ?? []) as (Assignment | AugmentedAssignment | Expression | Yield)[]),
		);
	}
	return expression_statement_(
...
export function forInClauseFrom(input: RuntimeNodeOf<ForInClause> | ForInClauseFromInput) {
	...
	return for_in_clause_({
		...
		right:
			obj.right !== undefined
				? (Array.isArray(obj.right) ? obj.right : [obj.right]).map(_resolveComprehensionClauses10)
				: [],
	});
}
...
export function ifStatementFrom(input: RuntimeNodeOf<IfStatement> | IfStatementFromInput) {
	...
	return if_statement_({
		alternative:
			obj.alternative !== undefined
				? (Array.isArray(obj.alternative) ? obj.alternative : [obj.alternative]).map(_rn6vv1r)
				: undefined,
		condition: _resolveComprehensionClauses3(obj.condition),
		consequence: _resolveSuite2(obj.consequence),
	});
...
export function importFromStatementNameFrom(
	input: RuntimeNodeOf<ImportFromStatementName> | ImportFromStatementNameFromInput,
) {
	if (isNodeData<"import_from_statement">(input)) {
...
export function listFrom(...args: (RuntimeNodeOf<List> | ListFromInput)[]) {
	if (args.length === 1 && isNodeData(args[0]) && args[0].type === "list") {
		return list_(
			...((args[0].children ?? []) as (Expression | ListSplat | ParenthesizedListSplat | Yield)[]),
		);
	}
	return list_(
...
export function parenthesizedExpressionFrom(
	...
	if (isNodeData(arg) && arg.type === "parenthesized_expression") {
		return parenthesized_expression_(
			arg.children?.[0] as Expression | ListSplat | ParenthesizedExpression | Yield,
		);
	}
	return parenthesized_expression_(
		_resolveFExpression4(arg) as Expression | ListSplat | ParenthesizedExpression | Yield,
	);
}
	...
	return string_({
		...
		content:
			obj.content !== undefined
				? (Array.isArray(obj.content) ? obj.content : [obj.content]).map(_r14l28kn)
				: undefined,
...
export function stringContentFrom(
	...
	return string_content_(
		...(args.map((v) => _r1qu323z(v)) as (EscapeInterpolation | EscapeSequence)[]),
	);
}
	...
	return subscript_({
		subscript:
			obj.subscript !== undefined
				? (Array.isArray(obj.subscript) ? obj.subscript : [obj.subscript]).map(_r1apoc2a)
				: [],
	...
	return try_statement_({
		...
		exceptClauses:
			obj.except_clauses !== undefined
				? (Array.isArray(obj.except_clauses) ? obj.except_clauses : [obj.except_clauses]).map(
						_r16jeonb,
					)
				: undefined,
