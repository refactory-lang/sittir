...
- [ ] **Step 1 — Update the hoist** so the `if (peeled !== null)` branch recurses into the seq body (post-order), then: `ruleMatchesEmpty(recursedBody)` → leave un-hoisted; else `isInlineSafe(recursedBody)` → hoist via `makeGroupLiftSymbol` (tagged) + `clauseHoistSynthName` (today's inline+gate path); else (inline-**unsafe**) → **leave inline, un-hoisted** (Chunk 2 emits its alias + the link pass *together*, so no parser-kind-without-IR-rule gap). Import the shared predicates; keep `setGroupLiftRuleMap` intact. **`applyAutoGroups` stays running this chunk** — it still covers inline-unsafe (inline, as today), so the gate should HOLD at floor (enrich just takes over the inline-safe hoists it was doing).
...
> **STATUS 2026-06-03 — MECHANISM DONE & CORRECT (commit `78ea43eb`).** The final mechanism is **not** the "widen `applyGroupOverrides`" sketch below but a cleaner symbol-ref alias: enrich creates every hoisted group as a hidden `_<parent>_group<N>` rule and, for inline-**unsafe** ones, wraps `alias($._name, $.name)` (NEVER alias a multi-member seq directly — tree-sitter distributes the alias across members). **tree-sitter mints the visible kindId** from the alias; **link coerces** the (nested) aliased kind to a top-level `rules` entry (`mintContentAliasKinds`, resolving through the symbol to the hidden body). `applyAutoGroups` disabled (wire ~705). group-classify `seqHasTopLevelRepeat` keeps the 6 list-kinds inline. Counts held: rust 111 / py 74 / ts 68. CST is clean for all minted groups (visibility_modifier, catch_clause, import_clause, slice — one node each).
>
> **REMAINING follow-up (NOT a render-template feature):** a visible group whose content has a **sequence nested in a choice arm** needs **polymorph registration** (existing variant mechanism), not a bespoke choice renderer. `visibility_modifier` renders `pub ()` only because its choice `self|super|crate|in_path` has one seq arm (`in_path`) that isn't polymorph-registered; ts −1 is the same class. Route visible-group seq-bearing choice arms through the polymorph pipeline → converts these to AST matches. See live state `project_content_alias_visible_group_state`.
