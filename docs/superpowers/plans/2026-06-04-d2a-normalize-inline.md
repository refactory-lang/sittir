...
- [ ] **FULL fold (v4 decision 2026-06-04):** fold the entire eligible set (~15 emitted kinds), NOT just optional-single-slot. `_extends_clause_single` folds (ts `Box`/SCC cycle gone). The v4 working-tree implementation (`normalize.ts`/`simplify.ts`/`templates.ts`) is the BASE — extend it, don't restart.
- [ ] **Spacing fix (option 2, design authority endorsed):** add to `needsSeqSpace` (`templates.ts:720`) — treat a `metadata.inlinedFrom`-tagged seq as **symbol-like at its OUTER boundaries** (restores the opaque-unit spacing the helper had before splicing; e.g. `for await (` not `for await(`). This recovers the obvious spacing cases. Residual spacing imperfection is ACCEPTED — see the correctness bar below.
- [ ] **CORRECTNESS BAR — the deep-AST gate is the arbiter, NOT template byte-equality.** Two distinct failure classes:
  - **Literal DROPPED entirely** (v2 class: `const x: T value` missing `=`) = parse-affecting = **BLOCKS.** Must never happen; v3 seq-unit multiplicity prevents it (literals gated, not dropped).
  - **Literal present, SPACING differs** (v3 `for await(` vs `for await (`) = cosmetic; round-trips to the same AST = **ACCEPTED + tracked** (strategic spacing is a separate workstream — render-time buffer look-ahead, see Deferred follow-up). NOT a Task-4 blocker.
  - The `read-render-parseAstMatchPass` gate distinguishes them automatically: dropped/fused tokens that change the parse → a DIP; pure cosmetic whitespace → no movement (tree-sitter is whitespace-insensitive there). **So the gate is sufficient — template-text byte-inspection is NO LONGER the bar** (it was the v2/v3 proxy; the real signal is the AST round-trip).
- [ ] Verify: `probe-stages` ts `extends_clause` — `_extends_clause_single` spliced. Confirm NO literal is DROPPED (token absent) for any fold target — token-PRESENT-but-spaced is fine.
- [ ] Gate: **RELEASE deep-AST `read-render-parseAstMatchPass` HOLD-or-IMPROVE at 111/69/74.** A DIP = a parse-affecting change (dropped/fused token) → bisect by per-kind-gating the fold set, report which kind. Cosmetic spacing diffs that HOLD the gate are expected and fine. ROLLBACK: revert the loop only if the gate dips and can't be recovered.
...
## Deferred follow-up — strategic render-time spacing (NOT in this execution)
The `needsSeqSpace` symbol-vs-literal boundary problem (surfaced by inlining `_for_header`: `for await (` → `for await(`) is a symptom of a brittle codegen-time spacing model. **Design authority direction (2026-06-04):** spacing needs a strategic approach — adjust **at render time** via **buffer look-ahead** (the renderer decides inter-token spacing from the actual adjacent emitted text, not from codegen-time word-likeness heuristics on rule nodes). The Task-4 `metadata.inlinedFrom`-symbol-like patch is a stopgap; the strategic render-time-spacing rework supersedes it and the whole `needsSeqSpace` family (`project_template_walker_adjacency`, `project_walker_hotspot`, `project_multi_separator_templates`). Cosmetic spacing diffs from Task 4 are accepted until this lands.
