...
**Keyword tokens (user ruling).** Every keyword token is named `<text>_keyword` — `class_keyword`, `fn_keyword`, `u8_keyword`, `TSKindId.ClassKeyword` — whether or not its text collides with a named kind. The predicate is parser.c's own naming, read in `compiler/generated-metadata.ts` where the symbol names are derived: an anonymous symbol whose C name is `anon_sym_` followed by the literal text verbatim is a keyword; a symbolic C name (`anon_sym_COMMA`, `anon_sym_macro_rules_BANG`) is not and keeps its derived name (`comma`, `macro_rules_bang`); a text made only of underscores (`_`, the one identifier-class character tree-sitter never escapes) is punctuation and derives `underscore` (`underscore2` when doubled), never a keyword. The `anon_<text>` collision scheme is deleted; a collision that survives the suffix is a codegen error naming both symbols. The nested options key, the Rust struct name and every generated spelling follow from the one name. Every reference to an old `anon_*` name outside generated output (tests, examples, docs, `grammar.sittir.ts` overrides) is updated; no alias or shim is left behind. Rendered bytes and validator numbers do not move.
