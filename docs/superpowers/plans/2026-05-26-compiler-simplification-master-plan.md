...
| **PR-O** | Structural de-dup (M1/MO2/P1 — non-behavioral) + **`separator-canonical`** (normalize `separator` → a single `Rule`; **Phase 1 = diagnostic** surfacing multi/non-terminal separators via PR-G's sink — doable now; Phase 2 = full render gated on the future **non-slot-variables** design; spec `2026-05-30-separator-canonical-design.md`) | B, H (files) | ‖ | no | `pr-o-structural-dedup` | ⬜ |
