...
| **PR-Q** | Enum recursive-widening (count-gated) | **P** | 11 | **yes** | `pr-q-enum-recursive-widening` | ✅ SUBSUMED BY PR-P — documented deliverable (`widening delta rust 0/ts 1/py 0`, the ts `predefined_type` case) was banked by PR-P step 1 (flat predicate caught it). PROBE (recursive `isEnumChoiceRule`, flat=false ∧ recursive-enum=true): rust **1** / ts **1** / py **0** — both are binary-operator precedence cascades (string ops + nested choices); the 18 "mixed string+symbol" choices are identifier/keyword unions (real SYMBOL members), NOT enums → invalid targets. No meaningful widening target remains; reclassifying the 2 operator choices is marginal + risks operator-modeling change. CLOSED without a separate PR. |
