...
**Carried in:** the native reader delivers a field-tagged separator into a mixedEnum array slot (python `for_in_clause.right`, and the singular sibling typescript `for_statement.condition`, which drops its `;` terminator the same way) because wrap's `resolveSlotDrillExprs` drops wire delimiters by separator id for every separated `many` slot but never runs on the native read path. The validator's read-render-parse deep-read only walks a chosen kind set, so it does not see the drop either — that blind spot is carried in along with the defect. Witness: `packages/python/tests/for-in-clause-separator-witness.test.ts` (`it.fails`). This plan's coordinate/gap-classification work is the first place a fix has the right shape to land. Also carried in: blank lines between statements render dropped in all three grammars (python module statements, rust items, typescript statements) even where a `(_)/after: blankline` declaration exists — trivia this plan's gap classification is meant to carry. Also carried in: a transport-text fast path bug in `render-module.ts` (an unconditional early `return w.text(text)` for a leaf's own text skips a depth-holding body's `w.dedent()`, leaking +1 depth for the rest of the render — see `render_block`/`render_suite_block` in the generated python transport), pre-existing and orthogonal to coordinates but touching the same render call surface this plan restructures; fixing it here avoids restructuring the fast path twice. Also carried in: the depth-break derivation is unified as of the typed-render-sink round (`dsl/primitives/spacing.ts::DEPTH_BREAK`/`depthBreakOf`), but `SpacingWriter::site(DEDENT)` and the generated literal `w.dedent()` route still diverge on whether a trailing break seam is unconditional — this plan's coordinate-carrying render is expected to be the first case where that divergence becomes observable, so reconciling the two dedent routes belongs in this plan's scope.
