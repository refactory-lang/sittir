...
- **Data-model threading (LANDED 2026-05-16):**
  ...
  - `AssembledPunctuation` exposes `.immediate` and `.tokenized` getters that report the underlying rule's wrapper status — distinct from the `modelType === 'token'` classification (an `AssembledPunctuation` exists for every classified token kind whether or not its rule was wrapped in a `TokenRule`).
  - Verification: `packages/codegen/src/__tests__/token-immediate-threading.test.ts`.
- **Gap (remaining):** the metadata reaches `TerminalValue` and `AssembledPunctuation`, but the template walker (`compiler/template-walker.ts`) and render-module template emission don't yet consume it. The walker returns `string[]` from `walkRuleForTemplate`; adjacency annotation requires a structured return.
- **Forward note (2026-05-19):** the planned rule-attribute / template-emitter refactor (see [`docs/superpowers/specs/2026-05-18-rule-attributes-and-template-emitter-design.md`](../specs/2026-05-18-rule-attributes-and-template-emitter-design.md)) deletes `compiler/template-walker.ts` in PR2 and replaces it with `emitters/templates.ts`, which reads PR0 modifier attributes (including `nonterminal`, `multiplicity`, structured `separator`) directly from the rule. Adjacency for scanner-delimited kinds (memory entry `template_walker_adjacency`) is absorbed there — the new emitter outputs literals exactly as captured on rules, with no spacing absorbers — so this requirement's structural fix lives in that refactor rather than as a separate walker change here.
