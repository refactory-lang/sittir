...
**Sequencing correction (the P1s' shared root):** "defer ALL render to non-slot-variables" was too broad. The **single-literal** projection (the common case) migrates to the singular-`Rule` shape **in this sub-item** so lists still render; only **choice/multi** render (per-occurrence replay) stays gated on non-slot-variables — exactly the cases the `warning` flags. The sub-item's own gate (generated byte-diff *classified*) is what proves the single-literal migration is render-neutral.
