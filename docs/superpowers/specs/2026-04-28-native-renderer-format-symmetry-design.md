...
That split violates the intended contract from the earlier format work. It also makes the public API drift: JS still behaves like a format-aware renderer, while native behaves like a stateless canonical printer.
...
Adopt **strict public API symmetry** between JS and native renderers, backed by **persistent renderer state** on the native side.

The key rules are:

1. native `NodeData` stays format-free; no per-node `$format` returns to the boundary
2. renderer instances own format state
...
1. Restore format-aware rendering symmetry between JS and native backends.
2. Keep `NodeData` free of per-node format state on the native side.
3. Make renderer lifecycle and handle behavior match across JS and native.
...
1. **JS contract**
   - renderer creation can carry format config
...
That mismatch is the real bug. The issue is not merely where formatting is applied, but that the two backends no longer present the same renderer model.
...
- `renderer.parseAndRead(...) -> treeHandle`
- `renderer.render(input, options?) -> string`
- `renderer.dispose()`
- `treeHandle.render(options?) -> string` or equivalent renderer-mediated render entrypoint

The important part is not the exact method spelling but the **same object model and the same semantics** on both backends:

- renderer instances hold engine-level defaults
- parsed handles remain associated with the renderer that created them
...
Existing convenience helpers may remain, but they should delegate to this shared renderer/handle contract instead of preserving backend-specific behavior differences.
...
1. engine-level renderer config
...
If native render is expected to be format-aware without putting format on every node, then the renderer must retain state across parse/read/render calls. That state is what nodes and tree handles "borrow" from during render.

The native backend therefore needs a real renderer/engine instance rather than only a stateless `render(node_json)` surface.

### Fast path for parsed handles

Once native has persistent renderer state, parsed-handle render can be cheaper and cleaner:

- render can use the renderer's association to the parsed handle
...
- it may render with engine-level defaults from the renderer
...
- same renderer creation surface
- same handle association model
- same precedence rules
- same detached-node behavior
- same lifecycle/error semantics

If JS currently has helper paths that bypass this shape, they should either be preserved only as wrappers or tightened so the shared renderer/handle API becomes the canonical contract.

## Error Handling

The symmetric API should also align failure modes:

- rendering with a disposed renderer is an explicit error
...
1. Add contract tests that exercise the same renderer/handle API on JS and native.
...
- feature 017 expectations for both-backend format parity should be restored through renderer behavior rather than test harness compensation
- the spec 012 native API contract will need to describe a stateful renderer/handle model rather than only a stateless canonical render call

The boundary may still expose low-level primitives internally, but the supported public contract should be the symmetric renderer API.
...
Implement the follow-up as a strict renderer-contract alignment change:

1. define the canonical symmetric renderer/handle API
2. preserve format-free native `NodeData`
3. add persistent native renderer state and renderer-associated tree handles
