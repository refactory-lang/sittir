...
- **A level's members are the union over its descendants**, and a member is **required only when the level's own claim carries it in every claiming grammar and every child by path declares it required**; otherwise it is optional. A member only a refinement carries (`accessorKind` on a getter, `operator` on a comparison) is therefore optional on the ancestor, and an ordinary function or call satisfies its base interface with the members every claiming grammar has. `T | T[]` where descendants disagree on multiplicity. A base member is typed by a namespace set (`G['expression']`) where a namespace contributes several kinds, and by the specific interface (`V.Identifier.Label<G>`) where it contributes one.
