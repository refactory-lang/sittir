...
#0.4), plus the end-state per-value kind refs **`kind` (render/source — also the per-value
`storageKind`: the value is keyed under it via `drillAs`; distinct from `storageName`,
which `parseKind` drives)** + **`parseKind` (parse-as)**. The render/source ref is the
