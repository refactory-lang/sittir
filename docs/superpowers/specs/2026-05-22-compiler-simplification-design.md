...
| **PR-S** | `separator-canonical` — collapse the 3-way `RuleBase.separator` union → one canonical `separator?: Rule` (extracted from PR-O's behavioral sub-item into its own standalone PR; **independent** — deps PR-B✅/PR-H✅ both landed, does NOT depend on the M→I→K chain). Spec: `2026-05-30-separator-canonical-design.md` | **Stop the eager, lossy evaluate-side lift** (`extractRepeatSeparator`/`extractFirstStringFromChoice` `evaluate.ts:389-427` discards all-but-first choice arm); let `seq(content, SEP)` ride structurally through evaluate→link→normalize. **Lift ONCE in `applyWrapperDeletion`** (push-down, the home PR-H settled) assigning `.separator` as a **single `Rule`** (the `ChoiceRule` preserved, not flattened) — one assignment site, one detection site. The **single-literal projection migrates in this PR** so ordinary `,`/`;`/`\n` lists render byte-identical. Emit a **`warning`** (detect via `rule.type !== 'string'`; **NOT** `propose-*`, **never** `fail`) at that single site when the separator is non-literal, surfaced via the printed grammar-diagnostics batch (`collectGrammarDiagnosticsForGrammar`), NOT the not-yet-consumed PR-G sink. **OUT OF SCOPE — choice/multi-separator RENDER:** gated on the separator-role child-slot design (`2026-05-26-non-slot-separator-rules-design.md`; `role:'separator'` child slot + `delimits` edge, verbatim pass-through — **NOT** a "non-slot variables" model; post-strangler follow-up → **PR-T**). | **own gate (behavioral, unlike PR-O's structural half):** `validate:native` hold (rust 125 / ts 72 / py 76) **+ generated byte-diff CLASSIFIED, not assumed empty** — relocating the lift evaluate→push-down can shift rendered bytes; every drift explained | one canonical separator shape stops every walker/render consumer branching on separator *shape* (same shape-variance-reduction class as PR-C); push-down is the single non-lossy assignment site (both the eager-lift and post-fanout classes have converged by Phase 3); single-literal migration is render-neutral by construction; deps met, no polymorph-chain coupling |
| **PR-T** | Separator-role child slots + `delimits` — RENDER + round-trip for choice / multi / per-gap separators (the render half of the separator model; clears exactly the cases PR-S's `warning` flags). **Depends on PR-S** (the single-`Rule` separator must land first). Its own brainstorm→spec→plan. Spec: `2026-05-26-non-slot-separator-rules-design.md` | Model a rule-shaped separator (`choice`/`optional`/token — not a fixed literal) as a **`role:'separator'` child slot** with a **`delimits: <contentSlotId>`** slot-to-slot edge (`infix` placement; `trailingPermitted?`/`leadingPermitted?` = static grammar *permission*; per-instance flank **presence captured**, not a static flag). Value is **verbatim pass-through** (`project_verbatim_transport_pattern` text carrier — "don't know which arm; preserve + re-emit"), **stored on the content slot** it delimits (no standalone `_separator` Config key — one home, no parallel list to keep aligned). Read buckets interleaved CST children by `role` (content vs separator), aligns each via `delimits`; render walks the content slot's values and replays each gap's preserved token, so even genuinely-mixed lists round-trip faithfully. **A separator IS a CST child** → a `role`-tagged child slot, **NOT** a "non-slot variables" / render-time-state model (that framing was rejected — DRY). | `validate:native` hold-or-improve; choice / multi / per-gap separator kinds render + round-trip; single-literal lists unchanged; the PR-S `warning` set clears | a separator is a real CST child, so a `role`-tagged child slot is the faithful (not parallel) model; verbatim pass-through avoids classifying choice arms; one storage home on the content slot; the relation is intrinsic + grammar-derived → modeled explicitly, not via `propose-*` |
...
3. **Delete `factory-map.json5` + `emitters/factory-map.ts`.** One serialized
   ...
   ⚠️ **PR-K φ2 remainder (filed 2026-06-10).** #69 landed steps 1–2 and deleted
   `factory-map.json5`, but **`emitters/factory-map.ts` was NOT deleted** —
   `buildFactoryMap` survives as `node-model.ts`'s producer (`node-model.ts:238`
   still calls it; validators read its output via `loadNodeModel`). Finishing
   step 3 is **PR-K φ2**: fold `buildFactoryMap` into `buildNodeModel`
   (`node-model.ts`) per the absorb-not-getter rule in step 1 above; move
   `FactoryShape`/`FactorySlotMeta` to a types home (validators import only the
   types); relocate `expandRuntimeDiscriminatorKinds` to its sole caller
   (`wrap.ts`); delete the file. **Byte-neutral**, gated by the §6 strict-subset
   proof (counts hold). **NOT folded into PR-L** — PR-L is the `propose-*`
   heuristic-flip (override-authored, count-at-each-removal); the emitter
   absorption is a structural/DRY consolidation that proves out under PR-K's gate,
   not PR-L's. Cheap wins to bank with it: remove the already-dead
   `factorySlots.children` branch (`factory-map.ts:102–108`, unreachable since the
   kind-named-slots refactor retired `AssembledBranch.children`) and fix the stale
   `factory-map.json5` reference still in `.claude/codegen-conventions.md`.
...
| `AssembledPunctuation` | REFINED — see `node-map.ts:3463` | `text` getter; overrides `isParameterless`/`stampExpression` (base case) + `stampChildExpression` |
...
| `collectAnonymousNodes(nodeMap, ctx)` | `collectAnonymousNodes(rules, nodes, wordMatcher, kindEntries)` (`assemble.ts:208`) | **Re-target to NodeMap + ctx.** The `rules`/`wordMatcher`/`kindEntries` args fold into `AssembleCtx`; it remains a whole-map collection pass (creates `AssembledKeyword`/`AssembledPunctuation` entries). |
