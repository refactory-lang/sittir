...
- Honest non-conforming leaves are utilities/comparators (`setsEqual`, `rulesEqualForVariant`, rename helpers, classify leaf predicates) and single-grammar-wide-arg passes (the `kindEntries` canonicalize trio — folding ONE arg into a fabricated ctx adds indirection without navigability gain). (`nameVariant` deleted, Track 1 commit `48292a847` — see the Link-phase entry above.)
