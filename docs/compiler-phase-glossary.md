...
4. Threads top-level **alias bodies** and **polymorph-form** contents through the same `applyNormalizationPasses` → `applyWrapperDeletion` → `computeSimplifiedRules` pipeline and merges them into `normalizedRules` / `simplifiedRules` (so `assemble.ts` reads snapshots, never re-simplifies per call).
**Output:** `SimplifiedGrammar` with `linkRules` (RawRule — renamed from `rules` in the phase-visibility-tightening pass; the pre-simplify, post-normalization-passes/pre-wrapper-deletion view), `normalizedRules` (RenderRule), `simplifiedRules` (SimplifiedRule).
...
**Output:** `NodeMap` with `nodes`, `signatures`, `derivations`, `linkRules` (renamed from `rules` in the phase-visibility-tightening pass — same pre-simplify `Rule<'link'>` view as `SimplifiedGrammar.linkRules`), `externals`.
