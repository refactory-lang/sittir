...
- **python deep AST mismatches** (1): `for_in_clause` drops its trailing comma ("Generator expression"). (The former `string` drops `string_content` rows are FIXED — read-layer text leaves pass through the wrap so the render's all-slots-empty `$text` fast-path fires; the former `simple_pattern_negative` drops `-` row is FIXED — the `_simple_pattern: { '11/0': field('sign') }` transform models the optional `-` the way `complex_pattern`'s position-0 field does.)
