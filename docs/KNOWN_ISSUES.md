...
- **S1 — remaining alias-identity rows.** The former "native coords unresolved for alias target" from() cluster (~34 rows) was a locator artifact for scalar-materialized text leaves and is FIXED — those kinds now compare via the string route (`from(text)` vs `factory(text)`); the refusal survives only for non-text kinds without native coords (rust `block_comment`). What remains under S1 is the true raw-context-id-vs-canonical-id class: the parsekind-noninjective diagnostics (3 rust / 3 python / 9 typescript) plus the typescript rrp transport alias-unwrap pair below and the `_static_marker` rows further down.
