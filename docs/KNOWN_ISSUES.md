...
The REAL gap: `mintContentAliasKinds` (`link.ts`) never gives an alias-minted kind (`pattern_group`, `element_list`, `parameter_list`, …) its own `TSKindId`/`KIND_NAMES` entry in generated `types.ts` — confirmed via `packages/python/src/types.ts`: `kindIdFromName` has no `case 'pattern_group'` (its underlying runtime id, 176, stays tagged as `_patterns` in `KIND_NAMES`). This means at runtime a promoted node's `$type` is still the raw hidden id, and any code that resolves templates BY NAME off that id (the deprecated JS/Nunjucks `createRenderer`, used only by `collect-baseline.test.ts`'s js/native parity check) looks up `_patterns.jinja` — which was never emitted, since all codegen output for this kind is correctly named after the promoted alias instead. **Native rendering is unaffected** (id-indexed dispatch, not name-based) — confirmed via `validate:native` holding baseline with zero regressions.
...
**Fix, if/when prioritized:** same fix as the `factories.ts`/`wrap.ts` entry above — make `link.ts`'s `mintContentAliasKinds` (or a downstream catalog-collection step) register alias-minted kinds with their own `TSKindId`, not just a `node-model.json5` entry.
